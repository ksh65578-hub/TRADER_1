...
PATCH_BASENAME = "MVP4_DASHBOARD_STABILITY_SCALEUP_LOCK"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-STABILITY-SCALEUP-LOCK"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_stability_scaleup_lock_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_DASHBOARD_STABILITY_SCALEUP_LOCK.md",
]
...
def regenerate_launcher_dashboards() -> dict[str, Any]:
    ...
    for launcher_name in sorted(ROOT_LAUNCHER_SPECS):
        ...
        safe = (
            ...
            and stability.get("scale_up_allowed") is False
        )
        item = {
            "launcher_name": launcher_name,
            "dashboard_shell": rel(paths["dashboard_shell"]),
            "dashboard_html": rel(paths["dashboard_html"]),
            "stability_scale_up_allowed": stability.get("scale_up_allowed") if isinstance(stability, dict) else None,
        ...
        if not safe:
            failed.append(item)
    return {
        "command": "safe local launcher dashboard regeneration for stability scale-up lock",
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_DASHBOARD_STABILITY_SCALEUP_LOCK.md",
        f"""# MVP4_DASHBOARD_STABILITY_SCALEUP_LOCK
...
Dashboard Stability Trends now carries the same explicit scale-up lock as the root dashboard and long-run panels. Any drift to scale_up_allowed=true in that panel blocks validation.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard stability scale-up lock",
            "full_text_marker": f"{REQUIREMENT_ID}: stability trend display panels must not omit or drift scale_up_allowed=false",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard Stability Trends scale-up lock",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_DASHBOARD_SHELL", "SECTION_LIVE_FINAL_GUARD", "SECTION_RISK_SCALE_UP_BLOCKER"],
            "depends_on": [
            ...
            "source_text_sha256": sha256_bytes(
                b"stability trend display panels must not omit or drift scale_up_allowed=false"
            ),
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_DASHBOARD_LEGACY_RUNTIME_ARTIFACT_HYGIENE.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "next_required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_LIVE_FINAL_GUARD", "SECTION_RISK_SCALE_UP_BLOCKER"],
            ...
            "active_read_surface_used": [
                ...
                "dashboard stability trends panel",
                "read_only_dashboard_shell schema",
                "live final guard slice",
                "risk scale-up blocker slice",
            ],
            "task_class": "DASHBOARD_UX",
            "required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_LIVE_FINAL_GUARD", "SECTION_RISK_SCALE_UP_BLOCKER"],
            "expanded_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_LIVE_FINAL_GUARD", "SECTION_RISK_SCALE_UP_BLOCKER"],
            ...
            "optimizer_stage": "NOT_CHANGED_DASHBOARD_STABILITY_SCALEUP_LOCK_ONLY",
            ...
            "optimizer_guardrail_result": "PASS_DASHBOARD_STABILITY_TRENDS_CANNOT_CREATE_SCALE_UP",
            ...
            "convergence_guardrail_result": "PASS_STABILITY_PANEL_SCALE_UP_DRIFT_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_STABILITY_TRENDS_SCALE_UP_LOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Stability Scale-Up Lock Audit
