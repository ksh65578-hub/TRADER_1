...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-RESOLUTION-CURRENT-EVIDENCE-CLOSURE-DASHBOARD-BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_resolution_current_evidence_closure_dashboard_binding_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING.md",
]
...
def load_closure_report() -> dict[str, Any]:
    report = load_json(runtime_closure_path())
    result = validate_upbit_paper_post_rerun_resolution_current_evidence_closure_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun resolution current-evidence closure validation failed: "
...
def validate_dashboard_projection(dashboard: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if dashboard.get("blocking_reason") != "POST_RERUN_RECONCILIATION_REQUIRED":
        raise RuntimeError("closure dashboard binding did not keep POST_RERUN_RECONCILIATION_REQUIRED")
    if reconciliation.get("source") != "upbit_paper_post_rerun_resolution_current_evidence_closure_report.json":
        raise RuntimeError("dashboard did not prefer the post-rerun resolution closure source")
    if reconciliation.get("post_rerun_resolution_closure_status") != "CURRENT_EVIDENCE_CLOSED_RESOLUTION_UNRESOLVED":
        raise RuntimeError("dashboard did not surface the current-evidence closure status")
    if reconciliation.get("post_rerun_resolution_closure_validation_status") != "PASS":
        raise RuntimeError("dashboard did not surface closure validation PASS")
    if reconciliation.get("post_rerun_resolution_closure_source_resolution_audit_file_load_status") != "PASS":
        raise RuntimeError("dashboard did not keep closure source audit binding PASS")
    if reconciliation.get("post_rerun_resolution_closure_source_resolution_audit_file_hash_match") is not True:
        raise RuntimeError("dashboard did not keep closure source audit hash binding")
    if reconciliation.get("post_rerun_resolution_closure_closed_item_count") <= 0:
        raise RuntimeError("dashboard closure closed count is missing")
    if (
        reconciliation.get("post_rerun_resolution_closure_current_evidence_write_authorized_count") != 0
        or reconciliation.get("post_rerun_resolution_closure_current_evidence_write_allowed_count") != 0
        or reconciliation.get("post_rerun_resolution_closure_candidate_current_evidence_usable_count") != 0
    ):
        raise RuntimeError("dashboard exposed closure current-evidence write allowance")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    closure_source = source_ids.get("POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE")
    if not isinstance(closure_source, dict) or closure_source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish the closure source artifact as PASS")
    if operator_action.get("status") != "BLOCKED" or operator_action.get("safe_to_continue_paper") is not False:
        raise RuntimeError("operator action did not fail closed for post-rerun resolution closure")
...
def write_context(now: str, trader_hash: str, agents_hash: str, dashboard: dict[str, Any], closure: dict[str, Any]) -> None:
    base.write_text(
        ROOT
        / "contracts"
        / "generated"
        / "context_pack"
        / "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING
task_class: MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING
...
included_schema_ids: ["trader1.read_only_dashboard_shell.v1", "trader1.upbit_paper_post_rerun_resolution_current_evidence_closure_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The Upbit PAPER dashboard loads the validated post-rerun resolution current-evidence closure report.
- The closure report is displayed as dashboard truth only, not current evidence or execution truth.
- POST_RERUN_RECONCILIATION_REQUIRED remains the dashboard blocking reason and operator primary blocker.
- Closure status, source audit binding, closed counts, and zero current-evidence write counts are visible.
...
- It does not resolve POST_RERUN_RECONCILIATION_REQUIRED; it makes the closure guard operator-visible.
...
- source_resolution_audit_file_load_status: {closure["source_resolution_audit_file_load_status"]}
- source_resolution_audit_file_hash_match: {closure["source_resolution_audit_file_hash_match"]}
- source_unresolved_item_count: {closure["source_unresolved_item_count"]}
- closed_item_count: {closure["closed_item_count"]}
- current_evidence_closed_count: {closure["current_evidence_closed_count"]}
- current_evidence_write_allowed_count: {closure["current_evidence_write_allowed_count"]}
...
Upbit PAPER post-rerun resolution current-evidence closure is now visible in the read-only dashboard. The dashboard remains blocked by POST_RERUN_RECONCILIATION_REQUIRED and cannot create current evidence, live orders, or scale-up.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun resolution current-evidence closure dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: current-evidence closure must be visible in the operator dashboard "
                "without creating current evidence, live order, or scale-up permission"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun resolution current-evidence closure dashboard binding",
            ...
            "depends_on": [
                ...
                "REQ-MVP4-UPBIT-PAPER-POST-RERUN-OPERATOR-RESOLUTION-DASHBOARD-BINDING",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"post rerun resolution current evidence closure visible in operator dashboard without current evidence live order or scale up permission"
            ),
    ...
    rows.append(
        {
            ...
            "fixture_files": [
                "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_post_rerun_resolution_current_evidence_closure_report.json"
            ],
            ...
            "dashboard_artifacts": [
                "reconciliation_recovery_summary.post_rerun_resolution_closure_status",
                "reconciliation_recovery_summary.post_rerun_resolution_closure_source_resolution_audit_file_load_status",
                "reconciliation_recovery_summary.post_rerun_resolution_closure_closed_item_count",
                "reconciliation_recovery_summary.post_rerun_resolution_closure_current_evidence_write_allowed_count",
                "source_artifacts.POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE",
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "post-rerun resolution current-evidence closure report",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_CLOSURE_VISIBLE_IN_DASHBOARD",
            ...
            "convergence_state_after": "POST_RERUN_RESOLUTION_CLOSURE_DASHBOARD_VISIBLE_LIVE_BLOCKED",
            ...
            "post_rerun_resolution_current_evidence_closure_status": closure["closure_status"],
            "post_rerun_resolution_closure_source_resolution_audit_file_load_status": closure[
                "source_resolution_audit_file_load_status"
            ],
            "post_rerun_resolution_closure_source_resolution_audit_file_hash_match": closure[
                "source_resolution_audit_file_hash_match"
            ],
            "post_rerun_resolution_closure_source_unresolved_item_count": closure["source_unresolved_item_count"],
            "post_rerun_resolution_closure_source_resolved_item_count": closure["source_resolved_item_count"],
            "post_rerun_resolution_closure_closed_item_count": closure["closed_item_count"],
            "post_rerun_resolution_closure_current_evidence_closed_count": closure["current_evidence_closed_count"],
            "post_rerun_resolution_closure_controls_satisfied_count": closure["closure_controls_satisfied_count"],
            "post_rerun_current_evidence_write_authorized_count": closure["current_evidence_write_authorized_count"],
            "post_rerun_current_evidence_write_allowed_count": closure["current_evidence_write_allowed_count"],
            "candidate_current_evidence_usable_count": closure["candidate_current_evidence_usable_count"],
        }
    )
    if dashboard.get("blocking_reason") != "POST_RERUN_RECONCILIATION_REQUIRED":
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_VISIBLE_POST_RERUN_RESOLUTION_CLOSURE_LIVE_BLOCKED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            "operator_action_status": operator_action["status"],
            "operator_primary_blocker_code": operator_action["primary_blocker_code"],
            "reconciliation_status": reconciliation["status"],
            "post_rerun_resolution_closure_status": reconciliation["post_rerun_resolution_closure_status"],
            "post_rerun_resolution_closure_closed_item_count": reconciliation[
                "post_rerun_resolution_closure_closed_item_count"
            ],
            "post_rerun_resolution_closure_current_evidence_write_allowed_count": reconciliation[
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Resolution Current-Evidence Closure Dashboard Binding Audit
...
def write_patch_artifacts(
    ...
    write_evidence(now, trader_hash, agents_hash, patch_result, dashboard, closure, launcher_artifacts)
...
def main() -> int:
    ...
    closure = load_closure_report()
    dashboard, reconciliation, operator_action, launcher_artifacts = write_launcher_artifacts()
    write_context(now, trader_hash, agents_hash, dashboard, closure)
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, dashboard, closure)
    write_patch_artifacts(now, trader_hash, agents_hash, patch_result, dashboard, closure, launcher_artifacts)
