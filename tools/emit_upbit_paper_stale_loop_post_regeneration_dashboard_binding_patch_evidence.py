...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-POST-REGENERATION-DASHBOARD-BINDING"
...
from trader1.runtime.paper.upbit_paper_stale_loop_post_regeneration_reconciliation import (  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_stale_loop_post_regeneration_dashboard_binding_patch_evidence.py",
...
def runtime_post_regeneration_path() -> Path:
    return (
        ROOT
        / "system"
        / "runtime"
...
def load_post_regeneration_report() -> dict[str, Any]:
    report = load_json(runtime_post_regeneration_path())
    result = validate_upbit_paper_stale_loop_post_regeneration_reconciliation_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "stale-loop post-regeneration reconciliation validation failed: "
...
def validate_dashboard_projection(dashboard: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if dashboard.get("blocking_reason") != "STALE_LOOP_RECONCILIATION_AFTER_REGENERATION_REQUIRED":
        raise RuntimeError("dashboard did not keep stale-loop post-regeneration as the primary blocker")
    if reconciliation.get("source") != "upbit_paper_stale_loop_post_regeneration_reconciliation_report.json":
        raise RuntimeError("dashboard did not prefer the stale-loop post-regeneration reconciliation source")
    if reconciliation.get("stale_loop_post_regeneration_reconciliation_status") != "BLOCKED":
        raise RuntimeError("dashboard did not surface stale-loop post-regeneration reconciliation BLOCKED")
    if reconciliation.get("stale_loop_post_regeneration_reconciliation_validation_status") != "PASS":
        raise RuntimeError("dashboard did not surface stale-loop post-regeneration reconciliation validation PASS")
    if (
        reconciliation.get("stale_loop_post_regeneration_item_count") <= 0
        or reconciliation.get("stale_loop_post_regeneration_accepted_count") <= 0
        or reconciliation.get("stale_loop_post_regeneration_blocked_reconciliation_count") <= 0
        or reconciliation.get("stale_loop_post_regeneration_current_evidence_usable_count")
        != reconciliation.get("stale_loop_post_regeneration_accepted_count")
        or reconciliation.get("stale_loop_post_regeneration_excluded_from_current_evidence_count")
        != reconciliation.get("stale_loop_post_regeneration_blocked_reconciliation_count")
    ):
        raise RuntimeError("dashboard did not preserve stale-loop post-regeneration blocked counts")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    source = source_ids.get("STALE_LOOP_POST_REGENERATION_RECONCILIATION")
    if not isinstance(source, dict) or source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish stale-loop post-regeneration source artifact as PASS")
    if portfolio.get("status") != "UNVERIFIED" or portfolio.get("source_snapshot_status") != "BLOCKED":
        raise RuntimeError("portfolio was incorrectly promoted while stale-loop reconciliation is blocked")
    if operator_action.get("status") != "BLOCKED" or operator_action.get("safe_to_continue_paper") is not False:
        raise RuntimeError("operator action did not fail closed for stale-loop post-regeneration reconciliation")
...
def write_context(now: str, trader_hash: str, agents_hash: str, dashboard: dict[str, Any], post_regeneration: dict[str, Any]) -> None:
    base.write_text(
...
- post_reconciliation_status: {post_regeneration["post_reconciliation_status"]}
- post_reconciliation_item_count: {post_regeneration["post_reconciliation_item_count"]}
- planned_regeneration_item_count: {post_regeneration["planned_regeneration_item_count"]}
- regenerated_current_accepted_count: {post_regeneration["regenerated_current_accepted_count"]}
- regenerated_current_blocked_reconciliation_count: {post_regeneration["regenerated_current_blocked_reconciliation_count"]}
- current_evidence_usable_count: {post_regeneration["current_evidence_usable_count"]}
- excluded_from_current_evidence_count: {post_regeneration["excluded_from_current_evidence_count"]}
...
Upbit PAPER stale-loop post-regeneration reconciliation is now visible in the read-only dashboard. Accepted regenerated artifacts remain display-only while blocked replacements still require ledger/recovery reconciliation before any portfolio truth or current-evidence claim.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER stale-loop post-regeneration reconciliation dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: stale-loop post-regeneration accepted artifacts and blocked replacements must be "
                "visible in the operator dashboard without creating portfolio truth, live order, or scale-up permission"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER stale-loop post-regeneration reconciliation dashboard binding",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"stale loop post regeneration reconciliation visible in operator dashboard without portfolio truth live order or scale up permission"
            ),
    ...
    rows.append(
        {
            ...
            "dashboard_artifacts": [
                "reconciliation_recovery_summary.stale_loop_post_regeneration_reconciliation_status",
                "reconciliation_recovery_summary.stale_loop_post_regeneration_accepted_count",
                "reconciliation_recovery_summary.stale_loop_post_regeneration_blocked_reconciliation_count",
                "reconciliation_recovery_summary.stale_loop_post_regeneration_current_evidence_usable_count",
                "reconciliation_recovery_summary.stale_loop_post_regeneration_excluded_from_current_evidence_count",
                "source_artifacts.STALE_LOOP_POST_REGENERATION_RECONCILIATION",
...
def build_patch_result(
    ...
    post_regeneration: dict[str, Any],
) -> dict[str, Any]:
    template = load_json(
        ROOT
        / "system"
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_REPAIR_RECONCILIATION_DASHBOARD_BINDING.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "stale-loop post-regeneration reconciliation report",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_STALE_LOOP_POST_REGENERATION_VISIBLE_IN_DASHBOARD",
            "optimizer_guardrail_result": "PASS_DASHBOARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "STALE_LOOP_POST_REGENERATION_DASHBOARD_VISIBLE_LIVE_BLOCKED",
            ...
            "stale_loop_post_regeneration_reconciliation_status": post_regeneration["post_reconciliation_status"],
            "stale_loop_post_regeneration_item_count": post_regeneration["post_reconciliation_item_count"],
            "stale_loop_post_regeneration_accepted_count": post_regeneration["regenerated_current_accepted_count"],
            "stale_loop_post_regeneration_blocked_reconciliation_count": post_regeneration[
                "regenerated_current_blocked_reconciliation_count"
            ],
            "stale_loop_post_regeneration_current_evidence_usable_count": post_regeneration[
                "current_evidence_usable_count"
            ],
            "stale_loop_post_regeneration_excluded_from_current_evidence_count": post_regeneration[
                "excluded_from_current_evidence_count"
            ],
            "candidate_current_evidence_usable_count": post_regeneration["current_evidence_usable_count"],
        }
    )
    if dashboard.get("blocking_reason") != "STALE_LOOP_RECONCILIATION_AFTER_REGENERATION_REQUIRED":
        raise RuntimeError("patch_result cannot be emitted without dashboard stale-loop blocker projection")
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_VISIBLE_STALE_LOOP_POST_REGENERATION_RECONCILIATION_LIVE_BLOCKED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            ...
            "stale_loop_post_regeneration_reconciliation_status": reconciliation[
                "stale_loop_post_regeneration_reconciliation_status"
            ],
            "stale_loop_post_regeneration_item_count": reconciliation[
                "stale_loop_post_regeneration_item_count"
            ],
            "stale_loop_post_regeneration_accepted_count": reconciliation[
                "stale_loop_post_regeneration_accepted_count"
            ],
            "stale_loop_post_regeneration_blocked_reconciliation_count": reconciliation[
                "stale_loop_post_regeneration_blocked_reconciliation_count"
            ],
            "stale_loop_post_regeneration_current_evidence_usable_count": reconciliation[
                "stale_loop_post_regeneration_current_evidence_usable_count"
            ],
            "stale_loop_post_regeneration_excluded_from_current_evidence_count": reconciliation[
                "stale_loop_post_regeneration_excluded_from_current_evidence_count"
            ],
            "live_order_ready": False,
            "live_order_allowed": False,
    ...
    base.write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "planned_regeneration_item_count": post_regeneration["planned_regeneration_item_count"],
            "regenerated_current_accepted_count": post_regeneration["regenerated_current_accepted_count"],
            "regenerated_current_blocked_reconciliation_count": post_regeneration[
                "regenerated_current_blocked_reconciliation_count"
            ],
            "current_evidence_usable_count": post_regeneration["current_evidence_usable_count"],
            "excluded_from_current_evidence_count": post_regeneration["excluded_from_current_evidence_count"],
...
def main() -> int:
    ...
    post_regeneration = load_post_regeneration_report()
    dashboard, _, _, _, launcher_artifacts = write_launcher_artifacts()
    write_context(now, trader_hash, agents_hash, dashboard, post_regeneration)
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, dashboard, post_regeneration)
    write_evidence(now, trader_hash, agents_hash, patch_result, dashboard, post_regeneration, launcher_artifacts)
