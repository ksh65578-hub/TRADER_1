...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_RUNTIME_CANDIDATE_LINKAGE_COST_MODEL"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-RUNTIME-CANDIDATE-LINKAGE-COST-MODEL"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_RUNTIME_LONG_RUN_AND_REAL_DATA_EVIDENCE_HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_runtime_candidate_linkage_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_RUNTIME_CANDIDATE_LINKAGE_COST_MODEL.md",
]
...
def run_bounded_paper_loop(now: str) -> tuple[dict[str, Any], dict[str, Any]]:
    loop = run_upbit_paper_persistent_loop(
        root=ROOT,
        loop_id="mvp4-candidate-linkage-cost-model",
        session_id="mvp1_upbit_paper_launcher",
        requested_cycle_count=2,
    )
    result = validate_upbit_paper_persistent_loop_report(loop)
    report = {
        "command": "run_upbit_paper_persistent_loop(root=ROOT, loop_id=mvp4-candidate-linkage-cost-model, requested_cycle_count=2)",
...
def upsert_requirement_index(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    entry = {
        ...
        "source_heading": "Upbit PAPER runtime candidate selection, cost model, and sizing linkage",
        "full_text_marker": f"{REQUIREMENT_ID}:SECTION_UPBIT_PAPER_RUNTIME:candidate linkage, net EV after cost, sizing strategy unit binding",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "PAPER runtime selected candidate must be cost-backed, highest net EV, and sizing-bound",
        ...
        "source_text_sha256": sha256_json({"requirement_id": REQUIREMENT_ID, "title": "candidate linkage and cost model"}),
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_RUNTIME_CANDIDATE_LINKAGE_COST_MODEL.md",
        f"""# MVP4_UPBIT_PAPER_RUNTIME_CANDIDATE_LINKAGE_COST_MODEL
...
Upbit PAPER runtime now validates candidate-to-sizing linkage and net EV after cost arithmetic. PAPER candidates carry fee, slippage, spread, impact, and latency components. The bounded PAPER loop was rerun locally and remains public/static-data only.
""",
    )
    ...
    patch_result.update(
        {
            ...
            "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-UPBIT-PAPER-RUNTIME-ENGINE-0-TO-1-FOUNDATION"],
            ...
            "next_required_section_ids": ["SECTION_UPBIT_PAPER_RUNTIME", "SECTION_STRATEGY_PROFITABILITY_LOOP"],
            "next_optional_section_ids": ["SECTION_LONG_RUN_STABILITY", "SECTION_BINANCE_ADAPTER_BOUNDARY"],
            ...
            "profit_convergence_patch": "PAPER_RUNTIME_COST_EVIDENCE_STRENGTHENED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_text(
        ...
        f"""# MVP4 Upbit PAPER Runtime Candidate Linkage Cost Model Audit
