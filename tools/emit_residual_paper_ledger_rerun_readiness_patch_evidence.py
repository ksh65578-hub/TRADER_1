...
PATCH_BASENAME = "MVP4_RESIDUAL_PAPER_LEDGER_RERUN_READINESS"
...
from trader1.reports.residual_paper_ledger_rerun_readiness import (  # noqa: E402
...
CHANGED_ARTIFACTS = [
    "trader1/reports/residual_paper_ledger_rerun_readiness.py",
    "contracts/schema/residual_paper_ledger_rerun_readiness_report.schema.json",
    "contracts/registry.yaml",
    "tests/contract/test_residual_paper_ledger_rerun_readiness.py",
    "tools/emit_residual_paper_ledger_rerun_readiness_patch_evidence.py",
...
def build_report(now: str, trader_hash: str, agents_hash: str, state_before: dict[str, Any] | None = None) -> dict[str, Any]:
    state = state_before or load_json(ROOT / "contracts" / "generated" / "current_implementation_state.json")
    source_reports = load_runtime_source_reports(ROOT)
    ...
    errors = validate_residual_paper_ledger_rerun_readiness_report(report, source_reports, state)
    if errors:
        raise RuntimeError("residual PAPER ledger rerun readiness failed: " + "; ".join(errors))
    for field in FALSE_FIELDS:
...
- bounded_staging_status: {report["bounded_staging_status"]}
...
- current_evidence_bridge_status: {report["current_evidence_bridge_status"]}
...
PAPER ledger rerun evidence is staged and candidate-only. Post-rerun ledger rollup is present, but reconciliation and current evidence bridge remain blocked until operator reconciliation completes. The open gaps remain open and cannot be closed by inference.
    ...
    requirements.append(
        {
            ...
            "source_heading": "residual PAPER ledger rerun readiness",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: residual PAPER ledger rerun gaps must expose staged candidate readiness, "
                "post-rerun reconciliation blockers, and current-evidence write denial without closing gaps by inference"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Residual PAPER ledger rerun readiness evidence",
            "requirement_kind": "EVIDENCE_PATCH",
            "schema_ids": [SCHEMA_ID],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": artifacts,
            "test_ids": ["tests/contract/test_residual_paper_ledger_rerun_readiness.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": sha256_bytes(b"residual PAPER ledger rerun readiness evidence live blocked"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_RESIDUAL_PAPER_LEDGER_RERUN_READINESS",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/residual_paper_ledger_rerun_readiness_report.schema.json"],
            "validator_files": [
                "trader1/reports/residual_paper_ledger_rerun_readiness.py",
                "trader1/validation/mvp0_validators.py",
            ],
            "test_files": ["tests/contract/test_residual_paper_ledger_rerun_readiness.py"],
            "fixture_files": [
                "contracts/generated/current_implementation_state.json",
                *[str(spec["relative_path"]).format(session_id="mvp1_upbit_paper_launcher") for spec in RUNTIME_SOURCE_SPECS.values()],
            ],
            "runtime_modules": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_RESIDUAL_PAPER_LEDGER_RERUN_READINESS",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_RESIDUAL_OPEN_GAP_OPERATOR_ACTION_PLAN.patch_result.json")
    patch_result = dict(template)
    remaining_blockers = sorted(set(state_before.get("open_contract_gap_ids", [])))
    patch_result.update(
        {
            "patch_id": PATCH_ID,
            "created_at_utc": now,
            ...
            "new_registry_items": [REQUIREMENT_ID, "residual_paper_ledger_rerun_readiness_report"],
            ...
            "active_read_surface_used": [
                ...
                "PAPER rerun runtime source reports",
                "requirement_index",
            ...
            "current_implementation_state_status": "UPDATED_RESIDUAL_PAPER_LEDGER_RERUN_READINESS",
            "retained_archive_read": False,
            "full_document_read": False,
            "read_cache_invalidated": False,
            "optimizer_patch": "false",
            "optimizer_stage": "NOT_CHANGED_RESIDUAL_PAPER_LEDGER_RERUN_READINESS_ONLY",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    assert_false_fields("readiness report", report, "")
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_RESIDUAL_PAPER_LEDGER_RERUN_READINESS_LIVE_BLOCKED",
    ...
    write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "runtime_source_paths": [source["relative_path"] for source in report["runtime_sources"]],
    ...
    write_text(
        ...
        f"""# MVP4 Residual PAPER Ledger Rerun Readiness Audit
        ...
        json.dumps(
            {
                ...
                "readiness_status": report["readiness_status"],
                "post_rerun_reconciliation_status": report["post_rerun_reconciliation_status"],
                "current_evidence_write_allowed": report["current_evidence_write_allowed"],
                "next_allowed_task_class": NEXT_TASK_CLASS,
                "live_order_ready": False,
                "live_order_allowed": False,
                "can_live_trade": False,
