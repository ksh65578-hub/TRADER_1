...
PATCH_BASENAME = "MVP4_PAPER_CURRENT_TRUTH_REFRESH_WRITER_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260506_001"
REQUIREMENT_ID = "REQ-MVP4-PAPER-CURRENT-TRUTH-REFRESH-DASHBOARD-BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_paper_current_truth_refresh_dashboard_binding_patch_evidence.py",
    CONTEXT_PACK_PATH,
]
...
def build_report(now: str, trader_hash: str, agents_hash: str) -> dict[str, Any]:
    ...
    return {
        "schema_id": "trader1.paper_current_truth_refresh_dashboard_binding_audit_report.v1",
        "patch_id": PATCH_ID,
        "generated_at_utc": now,
        "authority": {"trader1_sha256": trader_hash, "agents_sha256": agents_hash},
        "finding": (
            "The dashboard listed paper_current_truth_refresh_report.json as a source artifact, but portfolio truth "
            "selection could still fall back to stale summary display before consuming the fresh PAPER refresh output."
        ),
        "patch": (
            "Portfolio truth selection now prefers audited current evidence first, then a scoped PAPER current-truth "
            "refresh report, then stale summary display. Invalid refresh permission drift is converted to BLOCKED "
            "display truth with live and scale flags false."
        ),
        "acceptance_conditions": [
            "Fresh scoped PAPER current-truth refresh can drive portfolio cash/equity/PnL display when summary is stale",
            "Stale refresh remains stale display-only and does not prove runtime continuity",
            "Refresh permission drift fails closed as BLOCKED without live/scale permission",
            "Audited writer and LIVE_READY remain blocked",
            "Open contract gap count remains 13 and live/scale flags remain false",
        ],
        "blockers_removed": [
            "Dashboard no longer ignores fresh PAPER current-truth refresh when summary.json is stale",
            "Refresh permission drift now maps to an explicit blocked portfolio truth state",
        ],
...
The dashboard can consume a fresh PAPER current-truth refresh report as portfolio display truth even when summary.json is stale. This is not audited continuous writer proof and cannot enable live orders, LIVE_READY, or scale-up.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "Continuous PAPER current truth dashboard binding",
            "full_text_marker": f"{REQUIREMENT_ID}: bind fresh PAPER current-truth refresh report into dashboard portfolio truth while keeping live blocked",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "PAPER current truth refresh dashboard binding",
            "requirement_kind": "DASHBOARD_UX_RUNTIME_SAFETY_PATCH",
            "schema_ids": ["trader1.read_only_dashboard_shell.v1", "trader1.paper_current_truth_refresh_report.v1"],
            ...
            "live_affecting": True,
            "read_when": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": [
            ...
            "source_text_sha256": base.sha256_bytes(
                b"bind fresh PAPER current-truth refresh report into dashboard portfolio truth while keeping live blocked"
            ),
...
def build_patch_result(
    ...
    patch_result = base.build_patch_result(now, tests_run, validators_run)
    patch_result.update(
        {
            "patch_id": PATCH_ID,
            "created_at_utc": now,
            ...
            "next_required_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_LIVE_FINAL_GUARD"],
            "next_optional_section_ids": ["SECTION_RUNTIME_RECOVERY_RTO_ACTIVE", "SECTION_STRATEGY_PROFITABILITY"],
            ...
            "active_read_surface_used": [
                ...
                "dashboard portfolio truth projection",
                "paper current truth refresh report",
                "live final guard",
            ],
            "task_class": "PAPER_CURRENT_TRUTH_REFRESH_DASHBOARD_BINDING_PATCH",
            "required_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_LIVE_FINAL_GUARD"],
...
""",
    ...
    base.write_json(
        SESSION_DIR / "PAPER_RUN_SUMMARY.json",
        {
            ...
            "paper_current_truth_refresh_dashboard_binding_implemented": True,
    ...
    base.write_json(
        SESSION_DIR / "DASHBOARD_READINESS_SUMMARY.json",
        {
            ...
            "fresh_refresh_can_drive_portfolio_display": True,
            "permission_drift_blocks_display_truth": True,
...
Current state: the dashboard can now show fresh PAPER current-truth refresh values even if summary.json is stale. This is still PAPER display truth only, not live readiness.
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_PAPER_CURRENT_TRUTH_REFRESH_DASHBOARD_BINDING_LIVE_BLOCKED",
            "gap_closure_allowed_by_this_patch": False,
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 PAPER Current Truth Refresh Dashboard Binding Audit
    ...
    tests_run: list[dict[str, Any]] = [
        run_command(
            [
                ...
                "-p",
                "no:cacheprovider",
                "tests/dashboard/test_read_only_dashboard.py::ReadOnlyDashboardTest::test_dashboard_uses_fresh_paper_current_truth_refresh_when_summary_stale",
                "tests/dashboard/test_read_only_dashboard.py::ReadOnlyDashboardTest::test_dashboard_blocks_paper_current_truth_refresh_permission_drift",
