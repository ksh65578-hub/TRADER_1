...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-RECONCILIATION-DECISION-AUDIT"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP"
...
CHANGED_ARTIFACTS = [
    ...
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    operator_queue = load_json(runtime_path("upbit_paper_post_rerun_operator_reconciliation_queue_report.json"))
    ...
    if queue_result.status != "PASS":
        raise RuntimeError(
            "source post-rerun operator reconciliation queue validation failed: "
    ...
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun reconciliation decision audit validation failed: "
            f"{result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_post_rerun_reconciliation_decision_audit_report(root=ROOT, report=report)
    return report
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT
task_class: MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT
...
- The audit consumes only the validated post-rerun operator reconciliation queue report.
- Every queue item receives a write-denied decision while POST_RERUN_RECONCILIATION_REQUIRED remains open.
- Current ledger JSONL, latest runtime pointers, persistent loops, source artifacts, live readiness, and scale-up remain immutable.
- Current evidence write authorized, write allowed, and candidate current-evidence usable counts remain zero.

known_omissions_by_design:
- This patch is not a reconciliation writer, current-evidence writer, or promotion patch.
- POST_RERUN_RECONCILIATION_REQUIRED remains open for later review-only blocker rollup work.
...
- write_denied_count: {report["write_denied_count"]}
...
Upbit PAPER post-rerun operator reconciliation queue items now have a review-only decision audit. The audit records that current evidence writes are denied while reconciliation remains required, and it does not write current ledger evidence or live/scale artifacts.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun reconciliation decision audit",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun operator queue items must receive a review-only write-denied "
                "decision audit before any future current evidence writer can be considered"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun reconciliation decision audit",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_post_rerun_reconciliation_decision_audit.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(b"post rerun decision audit denies current evidence writes while reconciliation remains required"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REVIEW_ONLY_DECISION_AUDIT_CURRENT_EVIDENCE_WRITE_DENIED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_post_rerun_reconciliation_decision_audit.py"],
            "fixture_files": [],
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REVIEW_ONLY_DECISION_AUDIT_CURRENT_EVIDENCE_WRITE_DENIED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_QUEUE.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_post_rerun_reconciliation_decision_audit_validator"],
            ...
            "active_read_surface_used": [
                ...
                "PAPER post-rerun decision audit",
                "live final guard",
            ],
            "task_class": "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_DECISION_AUDIT_WRITE_DENIED",
            "optimizer_guardrail_result": "PASS_DECISION_AUDIT_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "POST_RERUN_RECONCILIATION_DECISION_AUDIT_WRITE_DENIED_RECONCILIATION_REQUIRED",
            ...
            "post_rerun_reconciliation_decision_audit_status": report["decision_audit_status"],
            "post_rerun_reconciliation_decision_item_count": report["decision_item_count"],
            "post_rerun_reconciliation_write_denied_count": report["write_denied_count"],
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REVIEW_ONLY_DECISION_AUDIT_CURRENT_EVIDENCE_WRITE_DENIED",
            "decision_audit_status": report["decision_audit_status"],
            "decision_item_count": report["decision_item_count"],
            "write_denied_count": report["write_denied_count"],
            "operator_reconciliation_required_count": report["operator_reconciliation_required_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Reconciliation Decision Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_post_rerun_reconciliation_decision_audit_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_post_rerun_reconciliation_decision_audit_validator")
