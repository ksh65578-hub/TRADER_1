...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_STALE_LOOP_ISOLATED_EVENT_ID_SCOPE_REPAIRED_ROLLUP_REBUILD"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-ISOLATED-EVENT-ID-SCOPE-REPAIRED-ROLLUP-REBUILD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_STALE_LOOP_ISOLATED_EVENT_ID_SCOPE_REPAIRED_DUPLICATE_RECHECK"
...
from trader1.runtime.paper.upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild import (  # noqa: E402
...
REPAIRED_ROLLUP_REBUILD_PATH = (
    RUNTIME_BASE / "upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_report.json"
)
...
CHANGED_ARTIFACTS = [
    ...
    "contracts/schema/upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_report.schema.json",
    "trader1/runtime/paper/upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild.py",
    "trader1/validation/mvp0_validators.py",
    "tests/runtime/test_upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild.py",
    "tools/emit_upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_patch_evidence.py",
    rel(REPAIRED_ROLLUP_REBUILD_PATH),
    f"contracts/generated/context_pack/{PATCH_BASENAME}.md",
]

BLOCKERS = [
    "ISOLATED_EVENT_ID_SCOPE_REPAIRED_ROLLUP_REBUILD_CURRENT_EVIDENCE_BLOCKED",
    "REPAIRED_ROLLUP_DUPLICATE_RECHECK_REQUIRED",
...
def report_repaired_ledger_paths(report: dict[str, Any]) -> list[str]:
    paths: list[str] = []
    for item in report.get("items", []):
        ...
        for cycle in item.get("cycles", []):
            if isinstance(cycle, dict):
                path = cycle.get("repaired_ledger_path")
...
def write_runtime_report() -> dict[str, Any]:
    report = build_upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_report(
        root=ROOT,
        event_id_scope_repair_executor_report=load_json(EVENT_ID_REPAIR_EXECUTOR_PATH),
        event_id_scope_repaired_rollup_rebuild_id=(
            "upbit-paper-stale-loop-isolated-event-id-scope-repaired-rollup-rebuild-20260502"
        ),
    ...
    if result.status != "PASS":
        raise RuntimeError(
            f"isolated event-id repaired rollup rebuild failed: {result.status} {result.blocker_code} {result.message}"
        )
    if (
        report.get("rebuild_status") != "REPAIRED_CANDIDATE_ROLLUPS_READY_CURRENT_EVIDENCE_BLOCKED"
        ...
        or report.get("candidate_rollup_artifact_ready_count") != 3
        or report.get("candidate_rollup_written_count", 0) + report.get("candidate_rollup_reused_existing_count", 0) != 3
        ...
        raise RuntimeError("isolated event-id repaired rollup rebuild did not preserve expected safe counts")
    for field in (
        ...
        if report.get(field) is not False:
            raise RuntimeError(f"isolated event-id repaired rollup rebuild attempted forbidden permission: {field}")
    write_upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_report(root=ROOT, report=report)
    return report
...
Upbit PAPER isolated event-id repaired rollup rebuild wrote/reused {report.get("candidate_rollup_artifact_ready_count")} candidate-only rollup artifact(s) from {report.get("ledger_jsonl_count")} repaired ledger file(s). These artifacts are not current evidence.
...
def update_requirement_artifacts(
    ...
    agents_hash: str,
    report: dict[str, Any],
    repaired_ledger_paths: list[str],
    candidate_rollup_paths: list[str],
) -> None:
    req_path = ROOT / "contracts" / "generated" / "requirement_index.json"
    matrix_path = ROOT / "contracts" / "generated" / "requirement_artifact_matrix.json"
    req_index = load_json(req_path)
    artifact_ids = sorted(set([*CHANGED_ARTIFACTS, *repaired_ledger_paths, *candidate_rollup_paths]))
    ...
    reqs.append(
        {
            ...
            "source_heading": "Upbit PAPER stale-loop isolated event-id repaired rollup rebuild",
            "full_text_marker": f"{REQUIREMENT_ID}: event-id repaired candidate rollups must stay candidate-only",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Rebuild candidate rollups from event-id repaired ledgers without current evidence promotion",
            "requirement_kind": "RUNTIME_RECONCILIATION_ROLLUP_PATCH",
            "schema_ids": ["trader1.upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_report.v1"],
            ...
            "depends_on": [
                "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-ISOLATED-EVENT-ID-SCOPE-REPAIR-EXECUTOR",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": sha256_bytes(
                b"event-id repaired candidate rollup rebuild candidate-only current evidence blocked"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REPAIRED_CANDIDATE_ROLLUPS_CURRENT_EVIDENCE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "fixture_files": [rel(EVENT_ID_REPAIR_EXECUTOR_PATH), *repaired_ledger_paths],
            "runtime_modules": [
                "trader1/runtime/paper/upbit_paper_stale_loop_isolated_event_id_scope_repaired_rollup_rebuild.py"
            ],
            "evidence_artifacts": [
            ...
            "dashboard_artifacts": [],
            "patch_result_fields": [
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_status",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_pass_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_blocked_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_artifact_ready_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_ledger_jsonl_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_ledger_event_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_filled_order_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_duplicate_event_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_event_id_updated_count",
                "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_current_evidence_write_allowed_count",
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REPAIRED_CANDIDATE_ROLLUPS_CURRENT_EVIDENCE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_STALE_LOOP_ISOLATED_EVENT_ID_SCOPE_REPAIR_EXECUTOR.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_EVENT_ID_REPAIRED_ROLLUP_REBUILD",
            ...
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_status": report["rebuild_status"],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_count": report[
                "candidate_rollup_attempt_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_pass_count": report[
                "candidate_rollup_pass_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_blocked_count": report[
                "candidate_rollup_blocked_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_candidate_artifact_ready_count": report[
                "candidate_rollup_artifact_ready_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_ledger_jsonl_count": report[
                "ledger_jsonl_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_ledger_event_count": report[
                "ledger_event_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_filled_order_count": report[
                "filled_order_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_duplicate_event_count": report[
                "duplicate_event_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_event_id_updated_count": report[
                "event_id_updated_count"
            ],
            "stale_loop_isolated_event_id_scope_repaired_rollup_rebuild_current_evidence_write_allowed_count": report[
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_EVENT_ID_REPAIRED_ROLLUP_REBUILD_CURRENT_EVIDENCE_BLOCKED",
...
def main() -> int:
    ...
    repaired_ledger_paths = report_repaired_ledger_paths(report)
    candidate_rollup_paths = report_candidate_rollup_paths(report)
    artifact_paths = sorted(set([*repaired_ledger_paths, *candidate_rollup_paths]))
    write_context(now, trader_hash, agents_hash, report)
    update_requirement_artifacts(now, trader_hash, agents_hash, report, repaired_ledger_paths, candidate_rollup_paths)
