...
PATCH_BASENAME = "MVP4_STALE_LOOP_REGENERATION_EXECUTION_REQUIRED_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260504_001"
REQUIREMENT_ID = "REQ-MVP4-STALE-LOOP-REGENERATION-EXECUTION-REQUIRED-RECHECK"
PREVIOUS_REQUIREMENT_ID = "REQ-MVP4-STALE-LOOP-REGENERATION-REQUIRED-RECHECK"
...
PREVIOUS_PATCH_RESULT = "system/evidence/patch_results/MVP4_STALE_LOOP_REGENERATION_REQUIRED_RECHECK.patch_result.json"
GUARD_REPORT = (
...
CLOSED_GAPS = {"STALE_LOOP_REGENERATION_REQUIRED", "STALE_LOOP_REGENERATION_EXECUTION_REQUIRED"}
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_stale_loop_regeneration_execution_required_recheck_patch_evidence.py",
...
def load_summary() -> dict[str, Any]:
    ...
    if state.get("next_allowed_task_class") not in {PATCH_BASENAME, NEXT_TASK_CLASS}:
        raise RuntimeError("state is not routed to stale loop regeneration execution required recheck")
    if previous.get("next_task_class") != PATCH_BASENAME:
        raise RuntimeError("previous recheck does not route to the execution required recheck")
...
included_requirement_ids: ["{REQUIREMENT_ID}", "{PREVIOUS_REQUIREMENT_ID}", "{EXECUTOR_REQUIREMENT_ID}", "{POST_REGENERATION_REQUIREMENT_ID}"]
...
Upbit PAPER stale-loop safe regeneration has already created 16 current-schema replacement copies while retaining sources. The result is still PAPER-only evidence and remains blocked on post-regeneration reconciliation; it is not long-run, live, promotion, or scale-up evidence.
    ...
    requirements.append(
        {
            ...
            "source_heading": "stale loop regeneration execution required recheck",
            "full_text_marker": (
            ...
            "requirement_title": "Stale loop regeneration execution required recheck",
            ...
            "source_text_sha256": sha256_bytes(
                b"safe regeneration execution recheck closes execution-required gap without live permission"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_RECHECK_EXECUTION_COMPLETED_RECONCILIATION_REQUIRED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/patch_result.schema.json"],
            "validator_files": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_RECHECK_EXECUTION_COMPLETED_RECONCILIATION_REQUIRED",
        }
    )
    matrix.update(
        {
    ...
    patch_result.update(
        {
            ...
            "current_implementation_state_status": "UPDATED_NEXT_TASK_ADVANCED_EXECUTION_GAP_CLOSED",
            "retained_archive_read": False,
            "full_document_read": False,
            "read_cache_invalidated": False,
            "optimizer_patch": "NOT_OPTIMIZER_PATCH",
            "optimizer_stage": "MVP-4_PAPER_SCORECARD_INPUT_ONLY",
            "optimizer_status_before": "STALE_LOOP_REGENERATION_PLAN_CONFIRMED_EXECUTION_REQUIRED",
            "optimizer_status_after": "STALE_LOOP_REGENERATION_EXECUTED_RECONCILIATION_REQUIRED_NOT_LIVE_READY",
            ...
            "optimizer_guardrail_result": "PASS_SAFE_REGENERATION_EXECUTION_EVIDENCE_REMAINS_PAPER_ONLY",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "STATE_RECHECK_ONLY",
            "convergence_layer_changed": False,
            "convergence_state_before": "STALE_LOOP_REGENERATION_PLAN_CONFIRMED_EXECUTION_REQUIRED",
            "convergence_state_after": "STALE_LOOP_SAFE_REGENERATION_EXECUTED_RECONCILIATION_REQUIRED",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_STALE_LOOP_REGENERATION_EXECUTION_GAP_CLOSED_RECONCILIATION_REQUIRED",
            **summary,
    ...
    write_text(
        ...
        f"""# MVP4 Stale Loop Regeneration Execution Required Recheck Audit
