...
PATCH_BASENAME = "MVP4_STALE_LOOP_REGENERATION_REQUIRED_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260504_001"
REQUIREMENT_ID = "REQ-MVP4-STALE-LOOP-REGENERATION-REQUIRED-RECHECK"
NEXT_TASK_CLASS = "MVP4_STALE_LOOP_REGENERATION_EXECUTION_REQUIRED_RECHECK"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_stale_loop_regeneration_required_recheck_patch_evidence.py",
...
def load_summary() -> dict[str, Any]:
    ...
    if PREVIOUS_REQUIREMENT_ID not in state.get("completed_requirement_ids", []):
        raise RuntimeError("regenerated-current repair reconciliation recheck is not completed")
    if state.get("next_allowed_task_class") not in {PATCH_BASENAME, NEXT_TASK_CLASS}:
        raise RuntimeError("state is not routed to the stale loop regeneration required recheck")
    if previous.get("next_task_class") != PATCH_BASENAME:
        raise RuntimeError("previous recheck does not route to the stale loop regeneration required recheck")
...
included_requirement_ids: ["{REQUIREMENT_ID}", "{PREVIOUS_REQUIREMENT_ID}", "{STALE_LOOP_RECONCILIATION_REQUIREMENT_ID}", "{STALE_LOOP_REGENERATION_POLICY_REQUIREMENT_ID}"]
...
The stale-loop regeneration plan is ready as PAPER-only source-preserving intent, but no regeneration has been executed. It creates no current evidence, live permission, long-run evidence, source deletion, or scale-up permission.
    ...
    requirements.append(
        {
            ...
            "source_heading": "stale loop regeneration required recheck",
            "full_text_marker": (
            ...
            "requirement_title": "Stale loop regeneration required recheck",
            ...
            "source_text_sha256": sha256_bytes(
                b"stale loop regeneration required recheck remains execution and live blocked"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_RECHECK_STALE_LOOP_REGENERATION_PLAN_EXECUTION_REQUIRED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "fixture_files": [PREVIOUS_PATCH_RESULT, STALE_LOOP_RECONCILIATION_REPORT, STALE_LOOP_REGENERATION_PLAN],
            "runtime_modules": [
                "trader1/runtime/paper/upbit_paper_stale_loop_regeneration.py",
                "trader1/runtime/paper/upbit_paper_stale_loop_reconciliation.py",
            ],
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_RECHECK_STALE_LOOP_REGENERATION_PLAN_EXECUTION_REQUIRED",
        }
    )
    matrix.update(
        {
    ...
    patch_result.update(
        {
            ...
            "optimizer_status_before": "STALE_LOOP_REGENERATION_RECHECK_NEXT_LIVE_BLOCKED",
            "optimizer_status_after": "STALE_LOOP_REGENERATION_PLAN_CONFIRMED_EXECUTION_REQUIRED",
            ...
            "convergence_state_before": "STALE_LOOP_REGENERATION_RECHECK_NEXT_LIVE_BLOCKED",
            "convergence_state_after": "STALE_LOOP_REGENERATION_PLAN_CONFIRMED_EXECUTION_REQUIRED",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_STALE_LOOP_REGENERATION_PLAN_REMAINS_EXECUTION_REQUIRED",
            **summary,
    ...
    write_text(
        ...
        f"""# MVP4 Stale Loop Regeneration Required Recheck Audit
...
Patch:
- Added a dedicated route/evidence recheck for {STALE_LOOP_REGENERATION_BLOCKER}.
