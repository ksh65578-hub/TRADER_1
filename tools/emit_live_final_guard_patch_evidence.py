...
PATCH_ID = "MVP4_LIVE_FINAL_GUARD_20260429_001"
PATCH_RESULT_REL = "system/evidence/patch_results/MVP4_LIVE_FINAL_GUARD.patch_result.json"
LIVE_FINAL_GUARD_IDS = ["live_final_guard_validator"]
GUARDRAIL_IDS = [
...
def update_validator_registry(now: str) -> None:
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") != "live_final_guard_validator"]
    implemented.append(
        {
    ...
    fixture_paths = {
        "live_final_guard_non_live_blocked_pass": "tests/validators/fixtures/live_final_guard_pass.json",
        "live_final_guard_live_permission_fail": "tests/validators/fixtures/live_final_guard_fail.json",
        "live_final_guard_disabled_blocked": "tests/validators/fixtures/live_final_guard_blocked.json",
    }
    catalog["fixtures"] = [
...
def write_validator_log(now: str, validator_results: list[dict[str, Any]], fixture_results: list[dict[str, Any]], guardrail_results: list[dict[str, Any]]) -> Path:
    path = ROOT / "system" / "evidence" / "validator_runs" / "MVP4_LIVE_FINAL_GUARD.validator_run_log.json"
    write_json(
        path,
        {
...
def write_stage_gate(now: str, validator_log_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "stage_gates" / "MVP4_LIVE_FINAL_GUARD.stage_gate_result.json"
    write_json(
        path,
        {
            ...
            "stage_gate_status": "LIVE_FINAL_GUARD_PASS_BUT_LIVE_REMAINS_BLOCKED",
...
def write_evidence_manifest(now: str, authority: dict[str, str], validator_log_path: Path, stage_gate_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "MVP4_LIVE_FINAL_GUARD.evidence_manifest.json"
    write_json(
        path,
        {
            ...
            "project_id": "TRADER_1",
            "authority": authority,
            "evidence_manifest_id": "MVP4_LIVE_FINAL_GUARD_EVIDENCE",
            "artifact_paths": [
                "trader1/validation/mvp0_validators.py",
                "tools/run_live_final_guard_validators.py",
                "tests/live_blocked/test_live_final_guard_validator.py",
                "tests/validators/fixtures/live_final_guard_pass.json",
                "tests/validators/fixtures/live_final_guard_fail.json",
                "tests/validators/fixtures/live_final_guard_blocked.json",
...
def build_patch_result(
    ...
    patch_result = {
        ...
        "affected_contract_ids": ["REQ-MVP0-LIVE-BLOCKED-TEST", "REQ-OPT-MVP0-SCAFFOLD"],
        ...
        "new_registry_items": ["live_final_guard_validator marked IMPLEMENTED_FAIL_CLOSED"],
        "new_or_changed_schema_ids": [],
        "validators_required": LIVE_FINAL_GUARD_IDS + GUARDRAIL_IDS,
        "validators_run": validator_results + guardrail_results,
        "tests_run": test_results,
        "coverage_unmapped_count": 0,
        "registry_yaml_parse_status": "PASS",
        "registry_placeholders_remaining": [],
        "retained_archive_semantic_mapping_status": "NOT_READ_NOT_REQUIRED_FOR_LIVE_FINAL_GUARD_PATCH",
        ...
        "required_section_ids": ["SECTION_LIVE_GATE", "SECTION_LIVE_FINAL_GUARD", "SECTION_LIVE_BLOCKED_TESTS"],
        "expanded_section_ids": ["TRADER_1:620-650", "TRADER_1:3130-3148"],
        ...
        "optimizer_patch": "LIVE_FINAL_GUARD_DEPENDENCY_VALIDATOR_PATCH",
        "optimizer_stage": "METRIC_COLLECTION",
        "optimizer_status_before": "GUARDRAIL_BLOCKED_BY_LIVE_FINAL_GUARD_UNTESTED",
        ...
        "failure_analysis_status": "NOT_REQUIRED_FOR_LIVE_FINAL_GUARD_PATCH",
...
def write_patch_and_state(now: str, patch_result: dict[str, Any]) -> None:
    ...
    for validator_id in LIVE_FINAL_GUARD_IDS + GUARDRAIL_IDS:
...
def main() -> int:
    ...
    validator_results = run_validators(LIVE_FINAL_GUARD_IDS)
    guardrail_results = run_validators(GUARDRAIL_IDS)
    fixture_paths = [
        ROOT / "tests" / "validators" / "fixtures" / "live_final_guard_pass.json",
        ROOT / "tests" / "validators" / "fixtures" / "live_final_guard_fail.json",
        ROOT / "tests" / "validators" / "fixtures" / "live_final_guard_blocked.json",
    ]
    fixture_results = [run_fixture_file(path) for path in fixture_paths]
    validator_log_path = write_validator_log(now, validator_results, fixture_results, guardrail_results)
    ...
    preliminary = build_patch_result(now, validator_results, guardrail_results, [], evidence_path, validator_log_path, stage_gate_path)
    ...
    fixture_results = [run_fixture_file(path) for path in fixture_paths]
    validator_log_path = write_validator_log(now, validator_results, fixture_results, guardrail_results)
    final_patch = build_patch_result(now, validator_results, guardrail_results, test_results, evidence_path, validator_log_path, stage_gate_path)
