...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-RECONCILIATION-BLOCKER-ROLLUP"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_BLOCKER_DASHBOARD_VISIBILITY"
...
CHANGED_ARTIFACTS = [
    ...
    "contracts/schema/upbit_paper_post_rerun_reconciliation_blocker_rollup_report.schema.json",
    "trader1/runtime/paper/upbit_paper_post_rerun_reconciliation_blocker_rollup.py",
    ...
    "tools/emit_upbit_paper_post_rerun_reconciliation_blocker_rollup_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    source_path = runtime_path("upbit_paper_post_rerun_reconciliation_decision_audit_report.json")
    decision_audit = load_json(source_path)
    ...
    if decision_result.status != "PASS":
        raise RuntimeError(
            "source post-rerun reconciliation decision audit validation failed: "
    ...
    report = build_upbit_paper_post_rerun_reconciliation_blocker_rollup_report(
        ...
        source_decision_audit_path=base.rel(source_path),
        rollup_id="mvp4-upbit-paper-post-rerun-reconciliation-blocker-rollup",
    )
    result = validate_upbit_paper_post_rerun_reconciliation_blocker_rollup_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun reconciliation blocker rollup validation failed: "
            f"{result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_post_rerun_reconciliation_blocker_rollup_report(root=ROOT, report=report)
    return report


def referenced_runtime_paths(report: dict[str, Any]) -> list[str]:
    paths = [str(report.get("source_decision_audit_path") or "")]
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP
task_class: MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP
...
included_schema_ids: ["trader1.patch_result.v1", "trader1.upbit_paper_post_rerun_reconciliation_blocker_rollup_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The rollup consumes only a validated post-rerun reconciliation decision audit report.
- POST_RERUN_RECONCILIATION_REQUIRED remains the primary blocker.
- All blocker counts preserve current-evidence, live-order, and scale-up blocking semantics.
- Current evidence write authorized, write allowed, and candidate current-evidence usable counts remain zero.
- No current ledger JSONL, latest runtime pointer, live readiness, live order, or scale-up artifact is written by this rollup.

known_omissions_by_design:
- This patch is not a reconciliation writer, current-evidence writer, promotion patch, LIVE_READY patch, or scale-up patch.
- POST_RERUN_RECONCILIATION_REQUIRED remains open for operator review and later visibility work.
- No private exchange/account/API call, credential, live order, live config mutation, or risk scale-up was used.

runtime_summary:
- blocker_rollup_status: {report["blocker_rollup_status"]}
- primary_blocker_code: {report["primary_blocker_code"]}
- rollup_item_count: {report["rollup_item_count"]}
- unique_blocker_count: {report["unique_blocker_count"]}
- unresolved_blocker_count: {report["unresolved_blocker_count"]}
- primary_blocker_item_count: {report["primary_blocker_item_count"]}
...
Upbit PAPER post-rerun reconciliation decisions now have a review-only blocker rollup. The rollup aggregates unresolved blocker counts for operator visibility while keeping current evidence writes, live readiness, live orders, and scale-up blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun reconciliation blocker rollup",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun reconciliation write-denied decisions must be rolled up into "
                "operator-visible unresolved blocker counts without becoming current evidence"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun reconciliation blocker rollup",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_post_rerun_reconciliation_blocker_rollup.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"post rerun reconciliation write denied decisions roll up unresolved blockers without current evidence writes"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REVIEW_ONLY_BLOCKER_ROLLUP_CURRENT_EVIDENCE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_post_rerun_reconciliation_blocker_rollup.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_post_rerun_reconciliation_blocker_rollup.py"],
            "evidence_artifacts": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REVIEW_ONLY_BLOCKER_ROLLUP_CURRENT_EVIDENCE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_post_rerun_reconciliation_blocker_rollup_validator"],
            ...
            "active_read_surface_used": [
                ...
                "PAPER post-rerun blocker rollup",
                "live final guard",
            ],
            "task_class": "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_BLOCKER_ROLLUP_CURRENT_EVIDENCE_BLOCKED",
            "optimizer_guardrail_result": "PASS_BLOCKER_ROLLUP_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "true",
            "convergence_layer_changed": True,
            "convergence_state_after": "POST_RERUN_RECONCILIATION_BLOCKER_ROLLUP_CURRENT_EVIDENCE_BLOCKED",
            ...
            "post_rerun_reconciliation_blocker_rollup_status": report["blocker_rollup_status"],
            "post_rerun_reconciliation_blocker_rollup_item_count": report["rollup_item_count"],
            "post_rerun_reconciliation_unique_blocker_count": report["unique_blocker_count"],
            "post_rerun_reconciliation_primary_blocker_item_count": report["primary_blocker_item_count"],
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REVIEW_ONLY_BLOCKER_ROLLUP_CURRENT_EVIDENCE_BLOCKED",
            "blocker_rollup_status": report["blocker_rollup_status"],
            "primary_blocker_code": report["primary_blocker_code"],
            "rollup_item_count": report["rollup_item_count"],
            "unique_blocker_count": report["unique_blocker_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Reconciliation Blocker Rollup
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_post_rerun_reconciliation_blocker_rollup_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_post_rerun_reconciliation_blocker_rollup_validator")
