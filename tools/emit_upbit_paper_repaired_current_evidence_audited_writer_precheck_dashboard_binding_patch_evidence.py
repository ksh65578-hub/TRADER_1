...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_PRECHECK_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-REPAIRED-CURRENT-EVIDENCE-AUDITED-WRITER-PRECHECK-DASHBOARD-BINDING"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DESIGN"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_repaired_current_evidence_audited_writer_precheck_dashboard_binding_patch_evidence.py",
...
def write_runtime_precheck_report() -> dict[str, Any]:
    report = build_upbit_paper_repaired_current_evidence_audited_writer_precheck_report(
        ...
        audited_writer_precheck_id="upbit-paper-repaired-current-evidence-audited-writer-precheck-dashboard-binding-20260502",
    )
...
def validate_dashboard_projection(dashboard: dict[str, Any], report: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not all(isinstance(item, dict) for item in (reconciliation, portfolio, operator_action, workflow)):
        raise RuntimeError("dashboard missing audited writer precheck operator summaries")
    if reconciliation.get("source") != "upbit_paper_repaired_current_evidence_audited_writer_precheck_report.json":
        raise RuntimeError("dashboard did not prefer audited writer precheck source")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_precheck_status")
        ...
        raise RuntimeError("dashboard did not expose audited writer precheck blocker")
    expected_counts = {
        "gate_count": report["audit_gate_count"],
        "gate_pass_count": report["audit_gate_pass_count"],
        "gate_blocked_count": report["audit_gate_blocked_count"],
        "candidate_ready_count": report["audited_writer_candidate_ready_count"],
    }
    actual_counts = {
        "gate_count": reconciliation["upbit_paper_repaired_current_evidence_audited_writer_precheck_gate_count"],
        "gate_pass_count": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_precheck_gate_pass_count"
        ],
        "gate_blocked_count": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_precheck_gate_blocked_count"
        ],
        "candidate_ready_count": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_precheck_candidate_ready_count"
        ],
    }
    if actual_counts != expected_counts:
        raise RuntimeError(f"dashboard audited writer precheck counts drifted: {actual_counts}")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_enabled") is not False
        or reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_precheck_passed") is not False
        ...
        raise RuntimeError("dashboard attempted to enable audited writer or current-evidence writes")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    source = source_ids.get("UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_PRECHECK")
    if not isinstance(source, dict) or source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish audited writer precheck source artifact as PASS")
    if portfolio.get("status") != "UNVERIFIED" or portfolio.get("source_snapshot_status") != "BLOCKED":
        raise RuntimeError("portfolio was incorrectly promoted by audited writer precheck display")
    if portfolio.get("blocking_reason") != AUDITED_CURRENT_EVIDENCE_WRITER_NOT_IMPLEMENTED_BLOCKER_CODE:
        raise RuntimeError("portfolio did not expose audited writer precheck blocker")
    if operator_action.get("primary_action_label") != "Inspect audited writer precheck":
        raise RuntimeError("operator action did not point to audited writer precheck")
    if "audited current-evidence writer is not implemented" not in str(operator_action.get("one_line_blocker")):
        raise RuntimeError("operator action did not explain audited writer blocker")
    if "Audited current-evidence writer is not implemented" not in str(workflow.get("summary")):
        raise RuntimeError("workflow did not explain audited writer blocker")
...
- audited_writer_candidate_ready_count: {report["audited_writer_candidate_ready_count"]}
...
The read-only dashboard now surfaces the Upbit PAPER repaired current-evidence audited writer precheck. It shows clean precheck inputs but keeps the audited writer, current-evidence writes, portfolio truth writes, live orders, and scale-up blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER audited writer precheck dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: dashboard must surface the audited writer precheck blocker while current evidence, "
                "portfolio truth, live orders, and scale-up remain blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER audited writer precheck dashboard binding",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"upbit paper repaired current evidence audited writer precheck dashboard binding no writes no live no scale"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_DASHBOARD_BOUND_WRITER_DISABLED_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_DASHBOARD_BOUND_WRITER_DISABLED_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_PRECHECK.patch_result.json"
    )
    ...
    patch_result.update(
        ...
        "patch_class": "RUNTIME_SAFETY_PATCH",
            ...
            "optimizer_status_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_PRECHECK_DASHBOARD_BOUND",
            "optimizer_guardrail_result": "PASS_DASHBOARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_PRECHECK_DASHBOARD_BOUND_LIVE_BLOCKED",
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_AUDITED_WRITER_PRECHECK_DASHBOARD_BOUND_WRITER_DISABLED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            "precheck_status": report["audited_writer_precheck_status"],
            "primary_blocker_code": report["primary_blocker_code"],
...
def main() -> int:
    ...
    dashboard, _reconciliation, _portfolio, dashboard_paths = write_launcher_artifacts(report)
