...
PATCH_BASENAME = "MVP4_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-MARKET-DATA-POINTER-SCHEMA-GUARD"
...
CHANGED_ARTIFACTS = [
    ...
    "contracts/schema/upbit_public_market_data_latest_pointer.schema.json",
    "contracts/schema/upbit_public_market_data_collection_writer_report.schema.json",
    "trader1/runtime/paper/upbit_public_collector.py",
    "trader1/validation/mvp0_validators.py",
    "tests/integration/test_upbit_public_collection_persistent_loop.py",
    "tools/emit_upbit_market_data_pointer_schema_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD.md",
]
VALIDATORS_REQUIRED = [
    "registry_validator",
    "schema_validator",
...
def run_runtime_artifacts() -> list[str]:
    loop = run_upbit_paper_persistent_loop(
        root=ROOT,
        loop_id="mvp4-upbit-market-data-pointer-schema-guard",
    ...
    paths.append("system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/mvp4-upbit-market-data-pointer-schema-guard.persistent_loop_report.json")
...
def build_audit() -> dict[str, Any]:
    ...
    latest_schema = load_json(ROOT / "contracts" / "schema" / "upbit_public_market_data_latest_pointer.schema.json")
    writer_schema = load_json(ROOT / "contracts" / "schema" / "upbit_public_market_data_collection_writer_report.schema.json")
    schemas = registry.get("schemas", {})
    checks = {
        "latest_pointer_schema_registered": "upbit_public_market_data_latest_pointer" in schemas,
        "collection_writer_schema_registered": "upbit_public_market_data_collection_writer_report" in schemas,
        "latest_pointer_schema_strict": latest_schema.get("additionalProperties") is False,
        "collection_writer_schema_strict": writer_schema.get("additionalProperties") is False,
        "latest_pointer_has_scope_fields": {"exchange", "market_type", "mode", "session_id", "symbol"}.issubset(set(latest_schema.get("required", []))),
        "writer_report_has_scope_and_hash_fields": {"exchange", "market_type", "mode", "session_id", "symbol", "public_market_data_hash"}.issubset(set(writer_schema.get("required", []))),
        "collector_writes_latest_scope_fields": '"exchange": report["exchange"]' in collector_text and '"symbol": report["symbol"]' in collector_text,
        "validator_checks_latest_pointer_hash_mismatch": "latest pointer hash mismatch was not failed closed" in validator_text,
        "validator_checks_writer_live_mutation": "collection writer live flag mutation was not blocked" in validator_text,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    return {
        "audit_schema_id": "trader1.upbit_market_data_pointer_schema_guard_audit.v1",
        "status": "PASS" if not blockers else "BLOCKED",
        "blockers": blockers,
        "checks": checks,
        "hidden_defect": {
            "classification": "schema_coverage_gap",
            "condition": "Upbit market-data latest pointer and collection writer artifacts were emitted without first-class schema/registry coverage",
            "impact": "runtime_schema_instance_validator could miss drift in operator-visible market-data pointer/writer artifacts",
            "fix": "added strict schemas, registry entries, runtime instance validation, and negative fixtures for pointer hash drift and writer live-flag mutation",
        },
...
def update_context(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD.md",
        f"""# MVP4_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD
...
Upbit public market-data latest pointer and collection writer artifacts are now strict schema-covered, registry-mapped, runtime-instance validated, scoped to UPBIT/KRW_SPOT/PAPER, and hash-bound to their source collection report.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER market data pointer schema closure",
            "full_text_marker": f"{REQUIREMENT_ID}:latest pointer and collection writer artifacts must be strict schema-covered and live-blocked",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit market-data pointer/writer artifacts must be schema and registry closed",
            "requirement_kind": "SCHEMA_VALIDATOR_PATCH",
            "schema_ids": [
                "trader1.upbit_public_market_data_latest_pointer.v1",
                "trader1.upbit_public_market_data_collection_writer_report.v1",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_UPBIT_PAPER_RUNTIME", "SECTION_DATA_FRESHNESS", "SECTION_NAMESPACE_SEPARATION", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": ["REQ-MVP4-UPBIT-PAPER-RUNTIME-SOURCE-HASH-GUARD", "REQ-MVP0-LIVE-DEFAULT-FALSE"],
            "source_text_sha256": sha256_bytes(b"latest pointer and collection writer artifacts must be strict schema-covered and live-blocked"),
    ...
    rows.append(
        {
            ...
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_public_collector.py"],
            "evidence_artifacts": [
...
def build_patch_result(
    ...
    patch_result: dict[str, Any] = {
        ...
        "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-UPBIT-PAPER-RUNTIME-SOURCE-HASH-GUARD", "REQ-MVP4-LIVE-FINAL-GUARD"],
        ...
        "new_registry_items": ["upbit_public_market_data_latest_pointer", "upbit_public_market_data_collection_writer_report"],
        ...
        "active_read_surface_used": ["SECTION_UPBIT_PAPER_RUNTIME", "SECTION_DATA_FRESHNESS", "SECTION_NAMESPACE_SEPARATION", "SECTION_LIVE_FINAL_GUARD"],
        "task_class": "MVP4_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD",
        "required_section_ids": ["SECTION_UPBIT_PAPER_RUNTIME", "SECTION_DATA_FRESHNESS", "SECTION_NAMESPACE_SEPARATION", "SECTION_LIVE_FINAL_GUARD"],
        ...
        "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD",
        ...
        "failure_analysis_status": "NOT_REQUIRED_POINTER_SCHEMA_GUARD",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_UPBIT_MARKET_DATA_POINTER_SCHEMA_GUARD_NO_LIVE_ORDERS",
            "pointer_schema_guard_audit": audit,
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Upbit Market Data Pointer Schema Guard
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["implemented_validator_ids"] = sorted(set(state.get("implemented_validator_ids", []) + ["upbit_public_market_data_collection_validator", "runtime_schema_instance_validator"]))
