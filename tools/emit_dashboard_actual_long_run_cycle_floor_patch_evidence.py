...
PATCH_BASENAME = "MVP4_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-ACTUAL-LONG-RUN-CYCLE-FLOOR"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_actual_long_run_cycle_floor_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR.md",
]
...
DASHBOARD_ARTIFACTS = [
    ...
    "system/runtime/binance/spot/paper/mvp1_binance_paper_launcher/launcher/root_launcher_report.json",
...
def configure_base() -> None:
    ...
    base.DASHBOARD_ARTIFACTS = DASHBOARD_ARTIFACTS
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR.md",
        f"""# MVP4_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR

context_pack_id: MVP4_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR
...
- Dashboard actual long-run status requires both the 86400s duration floor and the 2880 stable-sample floor.
- A sparse 24h history with only two stable samples remains ACTUAL_LONG_RUN_COLLECTING.
- The duration checklist item may pass independently, but cycle, evidence-window, and recovery proof stay collecting until repeated samples exist.
- A forged ACTUAL_LONG_RUN_VALIDATED boundary is blocked by validator logic.
...
- ACTUAL_LONG_RUN_RUNTIME_EVIDENCE_MISSING and ACTUAL_PERSISTENT_RUNTIME_EXECUTION_MISSING remain open
...
The dashboard now separates long-run duration from repeated runtime samples. A sparse day-long history can show duration progress, but it cannot validate actual long-run evidence until the 2880 stable-sample floor is also met.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard actual long-run cycle floor",
            "full_text_marker": f"{REQUIREMENT_ID}: dashboard must not validate actual long-run evidence before duration and stable-sample floors",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard actual long-run stable-sample floor",
            ...
            "depends_on": [
                "REQ-MVP4-DASHBOARD-ACTUAL-LONG-RUN-FLOOR-UX",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"dashboard must not validate actual long-run evidence before duration and stable-sample floors"
            ),
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_DASHBOARD_ACTUAL_LONG_RUN_CYCLE_FLOOR_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Dashboard Actual Long-Run Cycle Floor Audit
...
def main() -> int:
    ...
    patch_result = base.build_patch_result(now, tests_run, validators_run, regenerated)
    patch_result.update(
        {
            "patch_id": PATCH_ID,
            "created_at_utc": now,
            ...
            "optimizer_guardrail_result": "PASS_DASHBOARD_LONG_RUN_CYCLE_FLOOR_BLOCKED_BEFORE_2880_STABLE_SAMPLES",
            "convergence_state_after": "LONG_RUN_EVIDENCE_BLOCKED_DASHBOARD_DURATION_AND_CYCLE_FLOORS_ENFORCED",
            "convergence_guardrail_result": "PASS_DASHBOARD_LONG_RUN_CYCLE_FLOOR_BLOCKED_BEFORE_2880_STABLE_SAMPLES",
        }
    )
