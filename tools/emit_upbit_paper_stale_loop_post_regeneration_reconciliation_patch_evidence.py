...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_RECONCILIATION"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-POST-REGENERATION-RECONCILIATION"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_REGENERATED_BLOCKED_REPAIR_RECONCILIATION"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_stale_loop_post_regeneration_reconciliation_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_RECONCILIATION.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    ...
    report = build_upbit_paper_stale_loop_post_regeneration_reconciliation_report(
        ...
        reconciliation_id="mvp4-upbit-paper-stale-loop-post-regeneration-reconciliation",
    )
    result = validate_upbit_paper_stale_loop_post_regeneration_reconciliation_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "post-regeneration reconciliation validation failed: "
            f"{result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_stale_loop_post_regeneration_reconciliation_report(root=ROOT, report=report)
    return report


def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_RECONCILIATION.md",
        f"""# MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_RECONCILIATION
...
- planned_regeneration_item_count: {report["planned_regeneration_item_count"]}
...
Upbit PAPER stale loop schema-repair output has post-regeneration reconciliation. PASS replacements are separable from blocked repairs; 6 regenerated replacements still require ledger/recovery reconciliation and cannot be used as current evidence.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-regeneration reconciliation",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: after safe regeneration, only PASS regenerated current-schema PAPER replacements "
                "may be used as current evidence; blocked repairs and retained sources remain excluded"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER stale loop post-regeneration reconciliation",
            ...
            "depends_on": [
                "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-SAFE-REGENERATION-EXECUTOR",
            ...
            "source_text_sha256": base.sha256_bytes(b"post regeneration reconciliation accepts only PASS regenerated current schema paper replacements"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_BLOCKED_REPAIRS_EXCLUDED_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_BLOCKED_REPAIRS_EXCLUDED_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PAPER_STALE_LOOP_SAFE_REGENERATION_EXECUTOR.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_stale_loop_post_regeneration_reconciliation_validator"],
            ...
            "active_read_surface_used": [
                ...
                "safe regeneration executor report",
                "post-regeneration reconciliation runtime artifact",
                "stale-loop tests and validators",
            ],
            "task_class": "MVP4_UPBIT_PAPER_STALE_LOOP_POST_REGENERATION_RECONCILIATION",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_REGENERATION_RECONCILED_NOT_LIVE_READY",
            "optimizer_guardrail_result": "PASS_POST_REGENERATION_RECONCILIATION_EXCLUDES_BLOCKED_REPAIRS_FROM_EVIDENCE",
            "convergence_state_after": "STALE_RUNTIME_LOOP_POST_REGENERATION_RECONCILED_BLOCKED_REPAIRS_EXCLUDED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_POST_REGENERATION_RECONCILIATION_CREATED_BLOCKED_REPAIRS_EXCLUDED",
            "post_reconciliation_status": report["post_reconciliation_status"],
            "regenerated_current_accepted_count": report["regenerated_current_accepted_count"],
            "regenerated_current_blocked_reconciliation_count": report["regenerated_current_blocked_reconciliation_count"],
            "current_evidence_usable_count": report["current_evidence_usable_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Stale Loop Post-Regeneration Reconciliation Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_stale_loop_post_regeneration_reconciliation_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_stale_loop_post_regeneration_reconciliation_validator")
    completed = set(state.get("completed_requirement_ids", []))
    completed.add(REQUIREMENT_ID)
    gaps = set(state.get("open_contract_gap_ids", []))
    gaps.update({"REGENERATED_CURRENT_BLOCKED_REPAIRS_REQUIRE_LEDGER_RECOVERY_RECONCILIATION", "ACTUAL_LONG_RUN_RUNTIME_EVIDENCE_BOUNDARY"})
