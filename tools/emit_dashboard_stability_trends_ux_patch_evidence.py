...
PATCH_BASENAME = "MVP4_DASHBOARD_STABILITY_TRENDS_UX"
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "DASHBOARD_STABILITY_TRENDS_UX.md",
        f"""# DASHBOARD_STABILITY_TRENDS_UX
...
Dashboard now includes Stability Trends for current heartbeat/source/resource/latency/queue/rate-limit checks. It uses green for stable current checks, yellow for attention, and red only for error severity. It explicitly marks trend evidence as current-snapshot-only until a validated history artifact exists.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_DASHBOARD_RECENT_EVENTS_UX.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "next_task_class": "MVP4_RUNTIME_STABILITY_HISTORY_SCAFFOLD",
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_DASHBOARD_STABILITY_TRENDS_UX",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_DASHBOARD_STABILITY_TRENDS_UX_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "artifact_paths": [
                ...
                "tools/emit_dashboard_stability_trends_ux_patch_evidence.py",
                "contracts/generated/context_pack/DASHBOARD_STABILITY_TRENDS_UX.md",
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Stability Trends UX Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_RUNTIME_STABILITY_HISTORY_SCAFFOLD"
