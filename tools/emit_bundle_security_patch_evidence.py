...
PATCH_ID = "MVP0_BUNDLE_SECURITY_20260428_001"
...
def update_validator_registry(now: str) -> None:
    ...
    for validator_id in ["source_bundle_hygiene_validator", "secret_scan_validator"]:
    ...
    registry["source_bundle_manifest_path"] = "contracts/security/source_bundle_manifest.json"
    registry["source_bundle_denylist_path"] = "contracts/security/source_bundle_denylist.json"
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_section(
        section_map,
        {
            "section_id": "SECTION_BUNDLE_SECURITY",
            "source_file": "TRADER_1.md",
            "source_sha256": trader_hash,
            "source_heading": "bundle hygiene and source identity",
            "line_start": 10146,
            "line_end": 10224,
            "source_section_sha256": source_hash(trader_lines, 10146, 10224),
    ...
    requirement = {
        "requirement_id": "REQ-MVP0-BUNDLE-SECURITY",
        "source_section_id": "SECTION_BUNDLE_SECURITY",
        "source_file": "TRADER_1.md",
        "source_heading": "bundle hygiene and source identity",
        "full_text_marker": "source archive requires forbidden_count=0 and contains_secret=true blocks release/external share",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "MVP-0 source bundle denylist and secret scan hygiene",
        "requirement_kind": "BUNDLE_SECURITY",
        "schema_ids": ["trader1.validator_result.v1"],
        "validator_ids": ["source_bundle_hygiene_validator", "secret_scan_validator"],
        "artifact_ids": [
            "contracts/security/source_bundle_denylist.json",
            "contracts/security/source_bundle_manifest.json",
            "trader1/security/source_bundle.py",
            "tools/build_source_bundle_manifest.py",
        ],
        ...
        "live_affecting": True,
        "read_when": ["BUNDLE_SECURITY"],
        "depends_on": ["REQ-MVP0-LIVE-DEFAULT-FALSE"],
        "source_text_sha256": sha256_bytes(b"source archive requires forbidden_count=0 and contains_secret blocks release"),
    ...
    ensure_matrix_row(
        matrix,
        {
            ...
            "validator_files": ["trader1/validation/mvp0_validators.py", "trader1/security/source_bundle.py"],
            "test_files": ["tests/security/test_source_bundle_security.py"],
            "fixture_files": [],
            "runtime_modules": [],
            "evidence_artifacts": [
                "system/evidence/validator_runs/MVP0_BUNDLE_SECURITY.validator_run_log.json",
                "system/evidence/patch_results/MVP0_BUNDLE_SECURITY.patch_result.json",
            ],
            "dashboard_artifacts": [],
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "BUNDLE_SECURITY.md",
        f"""# BUNDLE_SECURITY
...
def update_state_and_evidence(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    validator_results = run_validators(["source_bundle_hygiene_validator", "secret_scan_validator"])
    validator_log = {
    ...
    write_json(ROOT / "system" / "evidence" / "validator_runs" / "MVP0_BUNDLE_SECURITY.validator_run_log.json", validator_log)
    ...
    stage_gate = {
        ...
        "source_bundle_hygiene": "PASS",
        "secret_scan": "PASS",
        "source_forbidden_count": 0,
        "contains_secret": False,
        "live_order_allowed": False,
        "stage_gate_status": "PASS_FOR_MVP0_BUNDLE_SECURITY_ONLY",
    }
    write_json(ROOT / "system" / "evidence" / "stage_gates" / "MVP0_BUNDLE_SECURITY.stage_gate_result.json", stage_gate)
    ...
    evidence_manifest = {
        ...
        "evidence_manifest_id": "MVP0_BUNDLE_SECURITY_EVIDENCE",
        "artifact_paths": [
            "contracts/security/source_bundle_denylist.json",
            "contracts/security/source_bundle_manifest.json",
            "trader1/security/source_bundle.py",
            ...
            "system/evidence/validator_runs/MVP0_BUNDLE_SECURITY.validator_run_log.json",
            "system/evidence/stage_gates/MVP0_BUNDLE_SECURITY.stage_gate_result.json",
        ],
        "known_blockers": [
            "LIVE_READY_MISSING",
    ...
    write_json(ROOT / "system" / "evidence" / "MVP0_BUNDLE_SECURITY.evidence_manifest.json", evidence_manifest)
    ...
    patch_result = {
        ...
        "affected_contract_ids": ["REQ-MVP0-BUNDLE-SECURITY"],
        ...
        "validators_required": ["source_bundle_hygiene_validator", "secret_scan_validator"],
        "validators_run": validator_results,
        "tests_run": [
            {"command": "python tools/build_source_bundle_manifest.py", "status": "PASS"},
            {"command": "python tools/run_bundle_security_validators.py", "status": "PASS"},
            {"command": "python -m unittest tests.security.test_source_bundle_security -v", "status": "PASS"},
            {"command": "python -m unittest discover -s tests -v", "status": "PASS"},
        ],
        "coverage_unmapped_count": 0,
        "registry_yaml_parse_status": "PASS",
        "registry_placeholders_remaining": [],
        "retained_archive_semantic_mapping_status": "NOT_READ_NOT_REQUIRED_FOR_BUNDLE_SECURITY",
        "read_cache_update_required": False,
        "context_pack_update_required": False,
        "current_implementation_state_updated": True,
        "next_task_class": "MVP0_CONTRACT_BASELINE",
        "next_required_section_ids": ["SECTION_NAMESPACE", "SECTION_TRUTH_HIERARCHY", "SECTION_PATCH_RESULT"],
        ...
        "evidence_manifest_path": "system/evidence/MVP0_BUNDLE_SECURITY.evidence_manifest.json",
        "validator_run_log_path": "system/evidence/validator_runs/MVP0_BUNDLE_SECURITY.validator_run_log.json",
        "stage_gate_result_path": "system/evidence/stage_gates/MVP0_BUNDLE_SECURITY.stage_gate_result.json",
        "token_navigation_patch": True,
        "active_read_surface_used": ["SECTION_BUNDLE_SECURITY", "SECTION_PATCH_RESULT"],
        "task_class": "BUNDLE_SECURITY",
        "required_section_ids": ["SECTION_BUNDLE_SECURITY", "SECTION_PATCH_RESULT"],
        "expanded_section_ids": ["SECTION_BUNDLE_SECURITY", "SECTION_PATCH_RESULT"],
        ...
        "failure_analysis_status": "NOT_REQUIRED_FOR_BUNDLE_SECURITY",
    ...
    write_json(ROOT / "system" / "evidence" / "patch_results" / "MVP0_BUNDLE_SECURITY.patch_result.json", patch_result)

    state_path = ROOT / "contracts" / "generated" / "current_implementation_state.json"
    state = load_json(state_path)
    if "REQ-MVP0-BUNDLE-SECURITY" not in state["completed_requirement_ids"]:
        state["completed_requirement_ids"].append("REQ-MVP0-BUNDLE-SECURITY")
    for validator_id in ["source_bundle_hygiene_validator", "secret_scan_validator"]:
    ...
    ledger["patches"].append(
        {
            ...
            "patch_class": "VALIDATOR_PATCH",
            "target_mvp_level": "MVP-0",
            "patch_result_path": "system/evidence/patch_results/MVP0_BUNDLE_SECURITY.patch_result.json",
