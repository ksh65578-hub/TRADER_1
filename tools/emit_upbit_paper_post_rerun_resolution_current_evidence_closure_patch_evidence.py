...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-RESOLUTION-CURRENT-EVIDENCE-CLOSURE"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_DASHBOARD_BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_resolution_current_evidence_closure_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    source_path = runtime_path("upbit_paper_post_rerun_operator_resolution_audit_report.json")
    source = load_json(source_path)
    source_result = validate_upbit_paper_post_rerun_operator_resolution_audit_report(source)
    if source_result.status != "PASS":
        raise RuntimeError(
            "source post-rerun operator resolution audit validation failed: "
    ...
    report = build_upbit_paper_post_rerun_resolution_current_evidence_closure_report(
        ...
        source_resolution_audit_path=base.rel(source_path),
        closure_guard_id="mvp4-upbit-paper-post-rerun-resolution-current-evidence-closure",
    )
    ...
    write_upbit_paper_post_rerun_resolution_current_evidence_closure_report(root=ROOT, report=report)
    return report


def referenced_runtime_paths(report: dict[str, Any]) -> list[str]:
    paths = [str(report.get("source_resolution_audit_path") or "")]
    for item in report.get("closure_items", []):
...
- source_resolution_audit_file_load_status: {report["source_resolution_audit_file_load_status"]}
- source_resolution_audit_file_hash_match: {report["source_resolution_audit_file_hash_match"]}
- source_unresolved_item_count: {report["source_unresolved_item_count"]}
- source_resolved_item_count: {report["source_resolved_item_count"]}
- closed_item_count: {report["closed_item_count"]}
- current_evidence_closed_count: {report["current_evidence_closed_count"]}
...
Upbit PAPER post-rerun operator resolution audit now has a closure guard that keeps unresolved resolution outputs out of current evidence. POST_RERUN_RECONCILIATION_REQUIRED remains open, and live orders and scale-up remain blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun resolution current-evidence closure",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: unresolved post-rerun operator resolution outputs must be closed "
                "as non-current evidence until validated reconciliation writer evidence exists"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun resolution current-evidence closure",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_post_rerun_resolution_current_evidence_closure.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"unresolved post rerun operator resolution outputs closed as non current evidence until validated reconciliation writer evidence exists"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REVIEW_ONLY_RESOLUTION_CURRENT_EVIDENCE_CLOSED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_post_rerun_resolution_current_evidence_closure.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_post_rerun_resolution_current_evidence_closure.py"],
            "evidence_artifacts": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REVIEW_ONLY_RESOLUTION_CURRENT_EVIDENCE_CLOSED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RESOLUTION_DASHBOARD_BINDING.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_post_rerun_resolution_current_evidence_closure_validator"],
            ...
            "active_read_surface_used": [
                ...
                "post-rerun current evidence promotion guard",
                "live final guard",
            ],
            "task_class": "MVP4_UPBIT_PAPER_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSED",
            "optimizer_guardrail_result": "PASS_RESOLUTION_CLOSURE_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "true",
            "convergence_layer_changed": True,
            "convergence_state_after": "POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSED",
            ...
            "post_rerun_resolution_closure_source_resolution_audit_file_load_status": report["source_resolution_audit_file_load_status"],
            "post_rerun_resolution_closure_source_resolution_audit_file_hash_match": report["source_resolution_audit_file_hash_match"],
            "post_rerun_resolution_closure_source_unresolved_item_count": report["source_unresolved_item_count"],
            "post_rerun_resolution_closure_source_resolved_item_count": report["source_resolved_item_count"],
            "post_rerun_resolution_closure_closed_item_count": report["closed_item_count"],
            "post_rerun_resolution_closure_current_evidence_closed_count": report["current_evidence_closed_count"],
            "post_rerun_resolution_closure_controls_satisfied_count": report["closure_controls_satisfied_count"],
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REVIEW_ONLY_RESOLUTION_CURRENT_EVIDENCE_CLOSED",
            "closure_status": report["closure_status"],
            "source_resolution_audit_file_load_status": report["source_resolution_audit_file_load_status"],
            "source_resolution_audit_file_hash_match": report["source_resolution_audit_file_hash_match"],
            "primary_blocker_code": report["primary_blocker_code"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Resolution Current-Evidence Closure
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_post_rerun_resolution_current_evidence_closure_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_post_rerun_resolution_current_evidence_closure_validator")
