...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_SOURCE_RECONCILIATION_BINDING_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-SOURCE-RECONCILIATION-BINDING-GUARD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_EVIDENCE"

if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

import tools.emit_persistent_runtime_resource_boundary_patch_evidence as base  # noqa: E402
import tools.emit_upbit_paper_post_rerun_current_evidence_promotion_guard_patch_evidence as previous  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_source_reconciliation_binding_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_SOURCE_RECONCILIATION_BINDING_GUARD.md",
]

BLOCKERS = [
    "POST_RERUN_RECONCILIATION_REQUIRED",
    "POST_RERUN_SOURCE_RECONCILIATION_FILE_BINDING_REQUIRED",
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_SOURCE_RECONCILIATION_BINDING_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_SOURCE_RECONCILIATION_BINDING_GUARD
...
- source_file_load_status: {report["source_post_rerun_reconciliation_file_load_status"]}
- source_file_hash_match: {str(report["source_post_rerun_reconciliation_file_hash_match"]).lower()}
...
Upbit PAPER post-rerun current-evidence promotion review is now bound to the persisted source reconciliation artifact hash. Missing or mismatched source files block the guard while all live and scale-up flags remain false.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun source reconciliation binding guard",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun promotion review must bind source reconciliation evidence to the "
                "persisted source artifact hash before any candidate can be review-ready"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun source reconciliation binding guard",
            ...
            "source_text_sha256": base.sha256_bytes(b"post rerun source reconciliation file hash binding guard"),
    ...
    rows.append(
        {
            ...
            "patch_result_fields": [
                ...
                "post_rerun_source_reconciliation_file_load_status",
                "post_rerun_source_reconciliation_file_hash_match",
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "persisted post-rerun reconciliation artifact",
                "live final guard",
            ],
            ...
            "post_rerun_source_reconciliation_file_load_status": report["source_post_rerun_reconciliation_file_load_status"],
            "post_rerun_source_reconciliation_file_hash_match": report["source_post_rerun_reconciliation_file_hash_match"],
            "optimizer_guardrail_result": "PASS_SOURCE_RECONCILIATION_ARTIFACT_HASH_BOUND_REVIEW_ONLY",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Source Reconciliation Binding Guard Audit
...
- promotion_guard_status: {report["promotion_guard_status"]}
- source_file_load_status: {report["source_post_rerun_reconciliation_file_load_status"]}
- source_file_hash_match: {report["source_post_rerun_reconciliation_file_hash_match"]}
- current_evidence_write_allowed_count: {report["current_evidence_write_allowed_count"]}
    ...
    tests_run = [
        base.run_command(
            [
                ...
                "-m",
                "unittest",
                "tests.runtime.test_upbit_paper_post_rerun_current_evidence_promotion_guard",
