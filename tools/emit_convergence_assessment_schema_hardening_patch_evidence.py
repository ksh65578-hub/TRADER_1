...
PATCH_BASENAME = "MVP4_CONVERGENCE_ASSESSMENT_SCHEMA_HARDENING"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-CONVERGENCE-ASSESSMENT-SCHEMA-HARDENING"
...
FIXTURE_FILES = [
    ...
    "tests/validators/fixtures/convergence_assessment_live_flag_fail.json",
    "tests/validators/fixtures/convergence_assessment_missing_dependency_fail.json",
    "tests/validators/fixtures/convergence_assessment_untested_dependency_fail.json",
    "tests/validators/fixtures/convergence_assessment_drift_unblocked_fail.json",
    "tests/validators/fixtures/convergence_assessment_writer_input_fail.json",
    "tests/validators/fixtures/convergence_assessment_scale_up_fail.json",
]
...
CHANGED_ARTIFACTS = [
    ...
    "tests/validators/test_convergence_assessment_validator.py",
    *FIXTURE_FILES,
    "tools/run_convergence_assessment_validators.py",
    "tools/emit_convergence_assessment_schema_hardening_patch_evidence.py",
    "contracts/generated/context_pack/CONVERGENCE_ASSESSMENT_SCHEMA.md",
    "system/evidence/audit_reports/MVP4_CONVERGENCE_ASSESSMENT_SCHEMA_HARDENING_20260429.md",
]
...
def update_registry_artifacts(now: str) -> None:
    ...
    validator_registry["implemented_validators"] = ensure_unique(
        ...
        "validator_id",
        {
            "validator_id": "convergence_assessment_validator",
...
def update_fixture_catalog(now: str) -> None:
    ...
    rows = [
        ("convergence_assessment_pass", "PASS", "convergence_assessment_pass.json"),
        ("convergence_assessment_live_flag_fail", "FAIL", "convergence_assessment_live_flag_fail.json"),
        ("convergence_assessment_missing_dependency_fail", "FAIL", "convergence_assessment_missing_dependency_fail.json"),
        ("convergence_assessment_untested_dependency_fail", "FAIL", "convergence_assessment_untested_dependency_fail.json"),
        ("convergence_assessment_drift_unblocked_fail", "FAIL", "convergence_assessment_drift_unblocked_fail.json"),
        ("convergence_assessment_writer_input_fail", "FAIL", "convergence_assessment_writer_input_fail.json"),
        ("convergence_assessment_scale_up_fail", "FAIL", "convergence_assessment_scale_up_fail.json"),
    ]
    ...
    catalog["scale_up_allowed"] = False
...
def update_gap_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    if audit_path.exists():
        ...
        for gap in audit.get("gaps", []):
            if gap.get("component") in {
                ...
                gap["fix"] = (
                    "convergence_assessment_report now requires the full parent dependency chain, dependency PASS counts, "
                    "fresh input, model drift blocking, false live/scale fields, no writer input eligibility, and operator wording "
                    "that separates display truth from live execution truth."
                )
        ...
        actions.append(
            {
                "action_id": PATCH_ID,
                "status": "APPLIED",
                "summary": "Hardened convergence assessment schema and validator dependency chain; added negative fixtures for live flag, dependency, drift, writer-input, and scale-up failures.",
    ...
    if gap_path.exists():
        ...
        gap["notes"] = (
            "Convergence assessment is now strict and fail-closed for dependency PASS count, failure analysis, paper/shadow evidence, "
            "model drift, stale data, LIVE_READY writer input attempts, and scale-up attempts. The broader maturity gap remains open "
            "until long-run evidence and external live-review inputs exist."
        )
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "CONVERGENCE_ASSESSMENT_SCHEMA.md",
        f"""# CONVERGENCE_ASSESSMENT_SCHEMA
...
Convergence assessment now requires explicit PASS dependency statuses for objective profile, optimizer memory, strategy memory, overfit diagnostics, execution feedback, failure analysis, paper/shadow evidence accumulation, market regime adaptation, model drift, and coverage index. The report cannot create live permission, LIVE_READY writer input eligibility, active snapshot mutation, or scale-up.
""",
    )
...
def update_requirement_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    requirements.append(
        {
            ...
            "source_section_id": "SECTION_CONVERGENCE_ASSESSMENT",
            "source_file": "TRADER_1.md",
            "source_heading": "Convergence assessment parent validator must not pass over dependency gaps",
            "full_text_marker": f"{REQUIREMENT_ID}:SECTION_CONVERGENCE_ASSESSMENT:full dependency chain and no live permission",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Harden convergence assessment report and validator dependency closure",
            "requirement_kind": "OPTIMIZER_CONVERGENCE_GUARDRAIL",
            "schema_ids": ["trader1.convergence_assessment_report.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/validators/test_convergence_assessment_validator.py"],
            "mvp_stage": "MVP-4",
            ...
            "depends_on": [
                "REQ-MVP4-MODEL-DRIFT-SCHEMA-HARDENING",
    ...
    rows.append(
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_CONVERGENCE_ASSESSMENT",
            "schema_files": ["contracts/schema/convergence_assessment_report.schema.json"],
            "validator_files": ["trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/validators/test_convergence_assessment_validator.py"],
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_MODEL_DRIFT_SCHEMA_HARDENING.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "new_or_changed_schema_ids": ["trader1.convergence_assessment_report.v1"],
            ...
            "next_task_class": "MVP4_DASHBOARD_CONVERGENCE_ASSESSMENT_VISIBILITY",
            "next_required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_CONVERGENCE_ASSESSMENT", "SECTION_LIVE_FINAL_GUARD"],
            "next_optional_section_ids": ["SECTION_MODEL_DRIFT", "SECTION_PROFIT_CONVERGENCE_CYCLE"],
            ...
            "active_read_surface_used": ["SECTION_CONVERGENCE_ASSESSMENT", "SECTION_MODEL_DRIFT", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "MVP4_CONVERGENCE_ASSESSMENT_SCHEMA_HARDENING",
            "required_section_ids": ["SECTION_CONVERGENCE_ASSESSMENT", "SECTION_MODEL_DRIFT", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": [
                "TRADER_1:convergence-assessment-active-surface",
                "TRADER_1:model-drift-active-surface",
                "AGENTS:convergence-assessment-validator-dependencies",
            ],
            ...
            "optimizer_patch": "CONVERGENCE_ASSESSMENT_SCHEMA_HARDENING",
            "optimizer_stage": "MVP4_ANALYSIS_ONLY",
            "optimizer_status_before": "CONVERGENCE_ASSESSMENT_SCHEMA_SCAFFOLD_ONLY",
            "optimizer_status_after": "CONVERGENCE_ASSESSMENT_FAIL_CLOSED_VALIDATED",
            "optimizer_maturity_level_before": "MVP4_SCAFFOLD",
            "optimizer_maturity_level_after": "MVP4_NEGATIVE_FIXTURE_VALIDATED",
            "optimizer_output_type": "CONVERGENCE_ASSESSMENT_REPORT_ONLY",
            "optimizer_guardrail_result": "PASS_NO_LIVE_PERMISSION",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "CONVERGENCE_ASSESSMENT_HARDENING",
            "convergence_layer_changed": True,
            "convergence_state_before": "CONVERGENCE_ASSESSMENT_SCHEMA_SCAFFOLD_ONLY",
            "convergence_state_after": "CONVERGENCE_ASSESSMENT_FAIL_CLOSED_VALIDATED_NO_LIVE_PERMISSION",
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": True,
            "failure_analysis_status": "REQUIRED_AND_PARENT_DEPENDENCY_ENFORCED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_CONVERGENCE_ASSESSMENT_SCHEMA_HARDENING_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Convergence Assessment Schema Hardening Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["implemented_schema_ids"] = sorted(set(state.get("implemented_schema_ids", []) + ["trader1.convergence_assessment_report.v1"]))
    state["implemented_validator_ids"] = sorted(set(state.get("implemented_validator_ids", []) + ["convergence_assessment_validator"]))
    state["untested_validator_ids"] = [item for item in state.get("untested_validator_ids", []) if item != "convergence_assessment_validator"]
    state["last_patch_id"] = PATCH_ID
    state["last_patch_result_hash"] = patch_result["result_hash"]
    state["next_allowed_task_class"] = "MVP4_DASHBOARD_CONVERGENCE_ASSESSMENT_VISIBILITY"
...
def main() -> int:
    ...
    tests_run = [
        ...
        run_command([sys.executable, "-m", "unittest", "tests.validators.test_convergence_assessment_validator", "-v"]),
        ...
        run_command([sys.executable, "tools/run_convergence_assessment_validators.py"]),
