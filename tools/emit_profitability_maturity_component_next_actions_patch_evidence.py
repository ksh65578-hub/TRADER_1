...
PATCH_BASENAME = "MVP4_PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS"
REQUIREMENT_ID = "REQ-MVP4-PROFITABILITY-MATURITY-COMPONENT-NEXT-ACTIONS"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_profitability_maturity_component_next_actions_patch_evidence.py",
    ...
    "system/evidence/audit_reports/MVP4_PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS_20260429.md",
    "contracts/generated/context_pack/PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS.md",
]
...
def update_registry_timestamp(now: str) -> None:
    ...
    members = validators.setdefault("supplemental_mvp4", [])
    for validator_id in ("read_only_dashboard_validator", "profitability_optimizer_evidence_gap_validator"):
...
def update_contract_gap(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    gap["status"] = "OPEN"
    gap["notes"] = (
        "Rechecked in MVP-4. Dashboard profitability maturity components now include next_required_evidence "
        "so each gap tells the operator what evidence to collect next. Gap remains OPEN because long-run evidence, "
        "external API verification, read-only burn-in, manual order evidence, and operator approval are absent."
    )
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_section_id": "SECTION_PROFITABILITY_EVIDENCE_MATURITY",
            "source_file": "TRADER_1.md",
            "source_heading": "profitability evidence maturity gap operator actions",
            "full_text_marker": f"{REQUIREMENT_ID}:component-level next evidence must be visible and fail closed",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Profitability maturity component next required evidence",
            "requirement_kind": "DASHBOARD_UX_VALIDATOR_PATCH",
            "schema_ids": ["trader1.read_only_dashboard_shell.v1"],
            "validator_ids": ["read_only_dashboard_validator", "profitability_optimizer_evidence_gap_validator"],
            ...
            "live_affecting": True,
            "read_when": [
                "SECTION_PROFITABILITY_EVIDENCE_MATURITY",
                "SECTION_DASHBOARD_OPERATOR_UX",
                "SECTION_OPTIMIZER_CONVERGENCE_GUARDRAIL",
                "SECTION_LIVE_FINAL_GUARD",
            ],
            "depends_on": [
                "REQ-MVP4-PROFITABILITY-MATURITY-GAP-RECHECK",
                "REQ-MVP4-DASHBOARD-PROFITABILITY-MATURITY-UX-HARDENING",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": sha256_bytes(b"component-level next evidence must be visible and fail closed"),
    ...
    ensure_matrix_row(
        ...
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_PROFITABILITY_EVIDENCE_MATURITY",
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS.md",
        f"""# PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS

context_pack_id: PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS
task_class: MVP4_PROFITABILITY_OPTIMIZER_EVIDENCE_GAP_RECHECK_CONTINUE
source_trader1_sha256: {trader_hash}
source_agents_sha256: {agents_hash}
included_section_ids: ["SECTION_PROFITABILITY_EVIDENCE_MATURITY", "SECTION_DASHBOARD_OPERATOR_UX", "SECTION_LIVE_FINAL_GUARD"]
...
- Every profitability maturity component includes next_required_evidence.
- Missing next evidence fails closed.
- Dashboard renders the next evidence action below each maturity component.
...
Read-only dashboards now show per-component profitability next_required_evidence. This is operator guidance only. The profitability optimizer maturity contract gap remains OPEN and live-blocking until external live-review evidence exists.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(
        ROOT / "system" / "evidence" / "patch_results" / "MVP4_PROFITABILITY_MATURITY_GAP_RECHECK.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "next_task_class": "MVP4_RISK_EXPOSURE_QUALITY_GAP_RECHECK",
            "next_required_section_ids": [
                "SECTION_RISK_SIZING_EXPOSURE",
                "SECTION_PROFITABILITY_EVIDENCE_MATURITY",
                "SECTION_LIVE_FINAL_GUARD",
            ],
            "next_optional_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_OPTIMIZER_CONVERGENCE_GUARDRAIL"],
            ...
            "expanded_section_ids": [
                "TRADER_1:profitability-evidence-maturity-active-surface",
            ...
            "optimizer_patch": "PROFITABILITY_COMPONENT_NEXT_ACTION_DASHBOARD_RECHECK",
            "optimizer_stage": "MVP4_REVIEW_PREP",
            "optimizer_status_before": "PAPER_SCORECARD_VISIBLE_WITH_COMPONENT_GAPS",
            "optimizer_status_after": "PAPER_SCORECARD_VISIBLE_WITH_COMPONENT_NEXT_ACTIONS",
            "optimizer_maturity_level_before": "MVP4_PARTIAL_PATCHED_GAP_VISIBLE",
            "optimizer_maturity_level_after": "MVP4_PARTIAL_PATCHED_NEXT_ACTION_VISIBLE",
            "optimizer_output_type": "DASHBOARD_DISPLAY_ONLY_COMPONENT_NEXT_REQUIRED_EVIDENCE",
            ...
            "optimizer_validators_run": [
                ...
                in {"read_only_dashboard_validator", "profitability_optimizer_evidence_gap_validator", "live_final_guard_validator"}
            ],
            ...
            "profit_convergence_patch": "PROFITABILITY_MATURITY_COMPONENT_NEXT_ACTIONS_NO_LIVE_MUTATION",
            "convergence_layer_changed": False,
            "convergence_state_before": "PROFITABILITY_GAP_OPEN_COMPONENT_DISPLAY",
            "convergence_state_after": "PROFITABILITY_GAP_OPEN_COMPONENT_NEXT_ACTION_DISPLAY",
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": False,
            "failure_analysis_status": "NOT_CHANGED_DASHBOARD_NEXT_ACTION_PATCH_ONLY",
            ...
            "convergence_validators_run": [
                result
                for result in validators_run
                if result.get("validator_id") in {"read_only_dashboard_validator", "live_final_guard_validator"}
            ],
            "convergence_guardrail_result": next(
                (result.get("status") for result in validators_run if result.get("validator_id") == "live_final_guard_validator"),
                "UNTESTED",
            ),
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_OPERATOR_NEXT_EVIDENCE_VISIBILITY_LIVE_REVIEW_STILL_BLOCKED",
            "next_allowed_task_class": "MVP4_RISK_EXPOSURE_QUALITY_GAP_RECHECK",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Profitability Maturity Component Next Actions
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_RISK_EXPOSURE_QUALITY_GAP_RECHECK"
    ...
    state["implemented_validator_ids"] = sorted(
        set(state.get("implemented_validator_ids", []) + ["read_only_dashboard_validator", "profitability_optimizer_evidence_gap_validator"])
    )
    state["untested_validator_ids"] = []
    gaps = set(state.get("open_contract_gap_ids", []))
    gaps.add("PROFITABILITY_OPTIMIZER_EVIDENCE_MATURITY")
