...
PATCH_BASENAME = "MVP4_UPBIT_PUBLIC_REST_SAMPLE_EVIDENCE"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PUBLIC-REST-SAMPLE-EVIDENCE"
...
CHANGED_ARTIFACTS = [
    "contracts/schema/upbit_public_rest_sample_report.schema.json",
    "contracts/registry.yaml",
    "contracts/validators/validator_registry.json",
    "trader1/runtime/paper/upbit_public_rest_sample.py",
    "trader1/validation/mvp0_validators.py",
    "tests/integration/test_upbit_public_rest_sample.py",
    "tools/run_upbit_public_rest_read_only_sample.py",
    "tools/emit_upbit_public_rest_sample_evidence_patch.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PUBLIC_REST_SAMPLE_EVIDENCE.md",
]
...
def upsert_requirement_index(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    entry = {
        ...
        "source_heading": "Upbit PAPER public REST sample evidence boundary",
        "full_text_marker": f"{REQUIREMENT_ID}:SECTION_MARKET_DATA:public-rest-sample-paper-only-not-live-ready",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "Public Upbit REST sample evidence remains PAPER-only and cannot become live readiness",
        "requirement_kind": "RUNTIME_DATA_EVIDENCE_BOUNDARY_PATCH",
        "schema_ids": ["trader1.upbit_public_rest_sample_report.v1"],
        "validator_ids": ["upbit_public_rest_sample_validator"],
        "artifact_ids": CHANGED_ARTIFACTS,
        "test_ids": ["tests/integration/test_upbit_public_rest_sample.py"],
        "mvp_stage": "MVP-4",
        ...
        "source_text_sha256": sha256_json({"requirement_id": REQUIREMENT_ID, "title": "public rest sample evidence boundary"}),
...
def upsert_requirement_artifact_matrix(now: str) -> None:
    ...
    row = {
        ...
        "schema_files": ["contracts/schema/upbit_public_rest_sample_report.schema.json"],
        "validator_files": ["trader1/validation/mvp0_validators.py"],
        "test_files": ["tests/integration/test_upbit_public_rest_sample.py"],
        "fixture_files": [],
        "runtime_modules": ["trader1/runtime/paper/upbit_public_rest_sample.py", "trader1/adapters/upbit/market_data.py"],
        "evidence_artifacts": [
        ...
        "dashboard_artifacts": ["system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/market_data/public/rest_sample_report.json"],
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PUBLIC_REST_SAMPLE_EVIDENCE.md",
        f"""# MVP4_UPBIT_PUBLIC_REST_SAMPLE_EVIDENCE
...
Upbit public REST sampling is now an operator-visible PAPER-only evidence surface. A successful sample can support PAPER input quality checks only; it cannot satisfy LIVE_READY, official API verification, account evidence, burn-in, manual order test, or operator approval.
""",
    )
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    for key, value in {
        ...
        "implemented_schema_ids": "trader1.upbit_public_rest_sample_report.v1",
        "implemented_validator_ids": "upbit_public_rest_sample_validator",
    }.items():
        values = state.setdefault(key, [])
        if value not in values:
            values.append(value)
            values.sort()
    state["untested_validator_ids"] = [item for item in state.get("untested_validator_ids", []) if item != "upbit_public_rest_sample_validator"]
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_FOR_PUBLIC_REST_SAMPLE_EVIDENCE_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    artifact_paths = [
        *CHANGED_ARTIFACTS,
        "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/market_data/public/rest_sample_report.json",
    ...
    write_text(
        ...
        f"""# MVP4 Upbit Public REST Sample Evidence Audit
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PUBLIC_DATA_SEQUENCE_REGIME_GUARD.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-UPBIT-PUBLIC-REST-READ-ONLY-TRANSFORM"],
            ...
            "new_registry_items": [REQUIREMENT_ID, "upbit_public_rest_sample_validator"],
            "new_or_changed_schema_ids": ["trader1.upbit_public_rest_sample_report.v1"],
            ...
            "expanded_section_ids": ["SECTION_MARKET_DATA", "SECTION_UPBIT_PAPER_RUNTIME"],
            ...
            "profit_convergence_patch": "PUBLIC_REST_SAMPLE_BOUNDARY_NOT_LIVE_READY",
            "convergence_layer_changed": False,
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": False,
            "failure_analysis_status": "NOT_REQUIRED_FOR_PUBLIC_REST_SAMPLE_BOUNDARY_PATCH",
...
def main() -> int:
    ...
    tests_run = [
        run_command([sys.executable, "-m", "unittest", "tests.integration.test_upbit_public_rest_sample", "-v"], timeout_seconds=120),
        run_command([sys.executable, "tools/run_upbit_public_rest_read_only_sample.py", "--no-network"], timeout_seconds=120),
        run_command([sys.executable, "tools/run_upbit_public_rest_read_only_sample.py", "--timeout-seconds", "2.5"], timeout_seconds=120),
    ]
