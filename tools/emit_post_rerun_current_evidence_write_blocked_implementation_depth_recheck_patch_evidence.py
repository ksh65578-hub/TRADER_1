...
PATCH_BASENAME = "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260504_001"
REQUIREMENT_ID = "REQ-MVP4-POST-RERUN-CURRENT-EVIDENCE-WRITE-BLOCKED-IMPLEMENTATION-DEPTH-RECHECK"
GAP_ID = "POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED"
NEXT_TASK_CLASS = "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK"
...
RUNTIME_REPORTS = {
    ...
    "review_guidance": RUNTIME_BASE / "upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.json",
...
CHANGED_ARTIFACTS = [
    ...
    "tests/contract/test_post_rerun_current_evidence_write_blocked_recheck.py",
    *ROUTE_TEST_ARTIFACTS,
    "tools/emit_post_rerun_current_evidence_write_blocked_implementation_depth_recheck_patch_evidence.py",
...
def false_field_errors(name: str, report: dict[str, Any]) -> list[str]:
    ...
    if GAP_ID not in set(report.get("blocker_codes") or []):
        errors.append(f"{name} no longer carries {GAP_ID}")
    return errors
...
def build_depth_report(now: str, trader_hash: str, agents_hash: str) -> dict[str, Any]:
    ...
    promotion = reports["promotion_guard"]
    operator_queue = reports["operator_queue"]
    review_guidance = reports["review_guidance"]
    resolution = reports["resolution_audit"]
    blocker_rollup = reports["blocker_rollup"]
    decision = reports["decision_audit"]
    closure = reports["closure"]

    if promotion.get("promotion_guard_status") != "BLOCKED":
        errors.append("promotion guard must remain BLOCKED")
    if int(promotion.get("promotion_review_ready_count") or 0) <= 0:
        errors.append("promotion guard lost review-ready candidates")
    if int(promotion.get("candidate_rollup_verified_count") or 0) <= 0:
        errors.append("promotion guard lost verified candidate rollups")
    ...
    if int(operator_queue.get("operator_reconciliation_required_count") or 0) <= 0:
        errors.append("operator reconciliation is no longer required")
    if review_guidance.get("review_guidance_status") != "BLOCKED_RECONCILIATION_REVIEW_REQUIRED":
        errors.append("review guidance must remain blocked/review-required")
    if resolution.get("resolution_audit_status") != "UNRESOLVED_RECONCILIATION_REVIEW_ONLY":
    ...
    if resolution.get("resolved_item_count") != 0:
    ...
    if decision.get("decision_audit_status") != "BLOCKED":
        errors.append("decision audit must remain BLOCKED")
    if decision.get("decision_item_count") != decision.get("write_denied_count"):
    ...
    if closure.get("closure_status") != "CURRENT_EVIDENCE_CLOSED_RESOLUTION_UNRESOLVED":
        errors.append("closure must remain current-evidence closed/unresolved")
    if closure.get("resolved_item_count") != 0:
        errors.append("closure unexpectedly has resolved items")

    previous_state_sync = load_json(ROOT / PREVIOUS_STATE_SYNC_PATCH_RESULT)
    if previous_state_sync.get("patch_id") != "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_STATE_SYNC_RECHECK_20260504_001":
        errors.append("previous current evidence write blocked state-sync patch_id drifted")
    if GAP_ID not in previous_state_sync.get("remaining_blockers", []):
        errors.append("previous current evidence write blocked state-sync lost blocker")
    if previous_state_sync.get("live_order_allowed_after") is not False:
        errors.append("previous current evidence write blocked state-sync no longer stays live-blocked")

    current_evidence_write_allowed_count = max(int(item.get("current_evidence_write_allowed_count") or 0) for item in reports.values())
    current_evidence_write_authorized_count = max(
        int(item.get("current_evidence_write_authorized_count") or 0) for item in reports.values()
    )
    candidate_current_evidence_usable_count = max(
        int(item.get("candidate_current_evidence_usable_count") or 0) for item in reports.values()
    )

    status = "PASS_DEPTH_5_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_LIVE_BLOCKING" if not errors else "BLOCKED_DEPTH_RECHECK_ERROR"
    return {
        "schema_id": "trader1.post_rerun_current_evidence_write_blocked_implementation_depth_recheck.v1",
        ...
        "review_guidance_status": review_guidance.get("review_guidance_status"),
        "review_guidance_item_count": review_guidance.get("review_guidance_item_count"),
        "resolution_audit_status": resolution.get("resolution_audit_status"),
        ...
        "blocker_rollup_item_count": blocker_rollup.get("rollup_item_count"),
        "decision_audit_status": decision.get("decision_audit_status"),
        "decision_item_count": decision.get("decision_item_count"),
        "write_denied_count": decision.get("write_denied_count"),
        "closure_status": closure.get("closure_status"),
        "current_evidence_closed_count": closure.get("current_evidence_closed_count"),
        "candidate_current_evidence_usable_count": candidate_current_evidence_usable_count,
        "current_evidence_write_allowed_count": current_evidence_write_allowed_count,
        "current_evidence_write_authorized_count": current_evidence_write_authorized_count,
    ...
    write_json(
        CONTRACT_GAP_PATH,
        {
            ...
            "blockers": [
                {
                    "code": GAP_ID,
                    "message": "Post-rerun current-evidence write-blocked evidence reaches depth-5 review coverage, but candidates remain review-only and current evidence writes remain denied.",
                }
            ],
...
task_class: MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP4-POST-RERUN-CURRENT-EVIDENCE-WRITE-BLOCKED-STATE-SYNC-RECHECK", "REQ-MVP4-UPBIT-PAPER-POST-RERUN-CURRENT-EVIDENCE-PROMOTION-GUARD"]
included_schema_ids: ["trader1.patch_result.v1", "trader1.contract_gap.v1", "trader1.upbit_paper_post_rerun_current_evidence_promotion_guard_report.v1", "trader1.upbit_paper_post_rerun_operator_reconciliation_queue_report.v1", "trader1.upbit_paper_post_rerun_resolution_current_evidence_closure_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS + runtime_artifacts())}
source_section_hashes: see contracts/generated/authority_section_map.json

acceptance_checklist:
- Confirm promotion guard, operator queue, review guidance, resolution audit, blocker rollup, decision audit, and closure reports remain present.
- Confirm review-ready candidates remain review-only and not current-usable evidence.
- Confirm current evidence write authorization, current ledger JSONL write, latest pointer write, live order, and scale-up remain blocked.
- Keep {GAP_ID} open and live-affecting.
- Route to {NEXT_TASK_CLASS}.

depth_snapshot:
- status: {report["status"]}
- promotion_review_ready_count: {report["promotion_review_ready_count"]}
- promotion_candidate_verified_count: {report["promotion_candidate_verified_count"]}
...
- {GAP_ID} remains live-blocking until independent repaired current-evidence writer and reconciliation evidence passes.
    ...
    requirements.append(
        {
            ...
            "source_heading": "post-rerun current evidence write blocked implementation depth recheck",
            "full_text_marker": f"{REQUIREMENT_ID}: verify post-rerun current-evidence write-blocked runtime evidence depth without current evidence or live permission",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Post-rerun current evidence write blocked implementation depth recheck",
            ...
            "source_text_sha256": sha256_bytes(b"post rerun current evidence write blocked implementation depth recheck"),
...
def build_patch_result(
    ...
    if report["status"] != "PASS_DEPTH_5_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_LIVE_BLOCKING":
        raise RuntimeError("cannot emit patch_result while post-rerun current evidence write-blocked depth recheck is blocked")
    ...
    patch_result.update(
        {
            ...
            "scale_up_allowed_after": False,
            "post_rerun_current_evidence_promotion_guard_status": report["promotion_guard_status"],
            "post_rerun_promotion_review_ready_count": report["promotion_review_ready_count"],
            "post_rerun_promotion_candidate_verified_count": report["promotion_candidate_verified_count"],
            "post_rerun_operator_reconciliation_queue_status": report["operator_queue_status"],
            ...
            "post_rerun_resolution_current_evidence_closure_status": report["closure_status"],
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_DEPTH_RECHECK_LIVE_BLOCKING",
    ...
    write_text(
        ...
        f"""# MVP4 Post-Rerun Current Evidence Write Blocked Implementation Depth Recheck
