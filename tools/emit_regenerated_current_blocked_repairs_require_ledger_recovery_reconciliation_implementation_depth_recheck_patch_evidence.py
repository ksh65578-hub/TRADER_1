...
CONTRACT_GAP_PATH = (
    ROOT / "system" / "evidence" / "contract_gaps" / f"{REGENERATED_BLOCKER}.contract_gap.json"
)
...
CHANGED_ARTIFACTS = [
    ...
    "tests/contract/test_regenerated_current_blocked_repairs_require_ledger_recovery_reconciliation_recheck.py",
    *ROUTE_TEST_ARTIFACTS,
    "tools/emit_regenerated_current_blocked_repairs_require_ledger_recovery_reconciliation_implementation_depth_recheck_patch_evidence.py",
...
def load_summary() -> dict[str, Any]:
    ...
    if PREVIOUS_REQUIREMENT_ID not in state.get("completed_requirement_ids", []):
        raise RuntimeError("blocked repair plan operator reconciliation depth recheck is not completed")
    if state.get("next_allowed_task_class") not in {PATCH_BASENAME, NEXT_TASK_CLASS}:
        raise RuntimeError("state is not routed to the regenerated-current blocked repairs depth recheck")
    if previous.get("next_task_class") != PATCH_BASENAME:
        raise RuntimeError("previous depth recheck does not route to the regenerated-current blocked repairs depth recheck")
    if previous_recheck.get("next_task_class") != "MVP4_STALE_LOOP_REGENERATION_REQUIRED_RECHECK":
        raise RuntimeError("previous regenerated-current recheck no longer routes to stale loop regeneration recheck")
...
def build_depth_report(now: str, trader_hash: str, agents_hash: str, summary: dict[str, Any]) -> dict[str, Any]:
    status = "PASS_DEPTH_5_REGENERATED_CURRENT_REPAIRS_LEDGER_RECOVERY_RECONCILIATION_LIVE_BLOCKING"
    return {
        "schema_id": (
            "trader1.regenerated_current_blocked_repairs_require_ledger_recovery_reconciliation_"
            "implementation_depth_recheck.v1"
        ),
        ...
        "contract_gap_id": REGENERATED_BLOCKER,
        "status": status,
        ...
        "blocked_repair_plan_report_path": BLOCKED_REPAIR_PLAN_REPORT,
        "repair_operator_queue_report_path": REPAIR_OPERATOR_QUEUE_REPORT,
...
def write_contract_gap(now: str, trader_hash: str, agents_hash: str) -> None:
    write_json(
        CONTRACT_GAP_PATH,
        {
            ...
            "blockers": [
                {
                    "code": REGENERATED_BLOCKER,
                    "message": (
                        "Regenerated current repair candidates remain blocked by ledger/recovery reconciliation: "
                        "one candidate requires hash operator reconciliation, four require PAPER runtime cycle rerun, "
                        "and one requires recovery guard rerun before any current evidence use."
                    ),
                }
            ],
            "notes": (
                "Depth recheck only; no repair execution, current evidence write, source deletion, "
                "live order, credentialed API call, live config mutation, or scale-up permission is created."
            ),
            "contract_gap_id": REGENERATED_BLOCKER,
            "severity": "HIGH",
    ...
    artifacts = sorted(
        set(
            ...
            + [
                ...
                f"system/evidence/audit_reports/{PATCH_BASENAME}.json",
                f"system/evidence/audit_reports/{PATCH_BASENAME}_20260504.md",
                f"system/evidence/contract_gaps/{REGENERATED_BLOCKER}.contract_gap.json",
    ...
    requirements.append(
        {
            ...
            "source_heading": (
                "regenerated current blocked repairs require ledger recovery reconciliation implementation depth recheck"
            ),
            "full_text_marker": (
                f"{REQUIREMENT_ID}: regenerated current repair candidates must remain depth-verified, "
                "repair-queue-blocked, "
            ...
            "source_text_sha256": sha256_bytes(
                b"regenerated current blocked repairs require ledger recovery reconciliation implementation depth recheck remains live blocked"
            ),
...
def write_evidence(
    ...
    write_text(
        ...
        f"""# MVP4 Regenerated Current Blocked Repairs Require Ledger Recovery Reconciliation Implementation Depth Recheck
