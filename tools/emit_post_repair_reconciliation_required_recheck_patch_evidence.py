...
PATCH_BASENAME = "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260504_001"
REQUIREMENT_ID = "REQ-MVP4-POST-REPAIR-RECONCILIATION-REQUIRED-RECHECK"
NEXT_TASK_CLASS = "MVP4_REPAIR_CANDIDATE_HASH_MISMATCH_RECONCILIATION_REQUIRED_RECHECK"
PREVIOUS_ROUTE_REQUIREMENT_ID = "REQ-MVP4-PATCH-RESULT-VALIDATOR-RUN-GAP-NEXT-TASK-RESTORE"
...
PREVIOUS_ROUTE_PATCH_RESULT = (
    "system/evidence/patch_results/MVP4_PATCH_RESULT_VALIDATOR_RUN_GAP_NEXT_TASK_RESTORE.patch_result.json"
)
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_post_repair_reconciliation_required_recheck_patch_evidence.py",
...
def load_summary() -> dict[str, Any]:
    ...
    previous_route = load_json(ROOT / PREVIOUS_ROUTE_PATCH_RESULT)
    ...
    if PREVIOUS_ROUTE_REQUIREMENT_ID not in state.get("completed_requirement_ids", []):
        raise RuntimeError("previous route restore requirement is not completed")
    if state.get("next_allowed_task_class") not in {
        "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_RECHECK",
        NEXT_TASK_CLASS,
    }:
        raise RuntimeError("current state is not routed to post-repair recheck")
    if previous_route.get("next_task_class") != "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_RECHECK":
        raise RuntimeError("previous route patch does not target post-repair recheck")
    if post_repair.get("post_repair_reconciliation_status") != "BLOCKED":
        raise RuntimeError("post-repair reconciliation is no longer BLOCKED")
    ...
    if post_repair.get("candidate_current_evidence_usable_count") != 0:
        raise RuntimeError("post-repair unexpectedly permits current evidence")
    if post_repair.get("source_loop_expected_rollup_hash_mismatch_count", 0) <= 0:
        raise RuntimeError("post-repair hash mismatch evidence is missing")
    if "REPAIR_CANDIDATE_HASH_MISMATCH_RECONCILIATION_REQUIRED" not in post_repair.get("blocker_codes", []):
        raise RuntimeError("post-repair hash mismatch blocker is missing")

    for path_text, artifact in (
        (PREVIOUS_ROUTE_PATCH_RESULT, previous_route),
    ...
    return {
        ...
        "previous_route_patch_result_hash": previous_route.get("result_hash"),
        ...
        "post_repair_primary_item_blocker_code": post_repair["items"][0]["item_blocker_code"],
...
included_requirement_ids: ["{REQUIREMENT_ID}", "{PREVIOUS_ROUTE_REQUIREMENT_ID}", "{POST_REPAIR_REQUIREMENT_ID}", "{POST_REPAIR_OPERATOR_UX_REQUIREMENT_ID}"]
...
- post_repair_primary_item_blocker_code: {summary["post_repair_primary_item_blocker_code"]}
...
Post-repair reconciliation remains blocked because the repair candidate has a source expected rollup hash mismatch. The candidate is not usable as current evidence.
    ...
    requirements.append(
        {
            ...
            "source_heading": "post-repair reconciliation required recheck",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: blocked post-repair reconciliation must route to the specific repair "
                "candidate hash mismatch reconciliation gap without current-evidence, live, or scale-up mutation"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Post-repair reconciliation required recheck",
            ...
            "depends_on": [
                PREVIOUS_ROUTE_REQUIREMENT_ID,
                POST_REPAIR_REQUIREMENT_ID,
                POST_REPAIR_OPERATOR_UX_REQUIREMENT_ID,
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": sha256_bytes(
                b"post repair reconciliation required recheck routes to hash mismatch gap live blocked"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_RECHECK_HASH_MISMATCH_GAP_OPEN",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_RECHECK_HASH_MISMATCH_GAP_OPEN",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_PATCH_RESULT_VALIDATOR_RUN_GAP_NEXT_TASK_RESTORE.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "optimizer_status_before": "POST_REPAIR_RECONCILIATION_REQUIRED_OPEN",
            "optimizer_status_after": "REPAIR_HASH_MISMATCH_RECHECK_NEXT_LIVE_BLOCKED",
            ...
            "convergence_state_before": "POST_REPAIR_RECONCILIATION_REQUIRED_OPEN",
            "convergence_state_after": "REPAIR_HASH_MISMATCH_RECHECK_NEXT_LIVE_BLOCKED",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_POST_REPAIR_RECONCILIATION_RECHECK_REMAINS_BLOCKED",
            **summary,
    ...
    write_text(
        ...
        f"""# MVP4 Post-Repair Reconciliation Required Recheck Audit
