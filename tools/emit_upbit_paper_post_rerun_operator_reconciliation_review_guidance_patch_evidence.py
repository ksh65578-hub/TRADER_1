...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_GUIDANCE"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-OPERATOR-RECONCILIATION-REVIEW-GUIDANCE"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "contracts/schema/upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.schema.json",
    "trader1/runtime/paper/upbit_paper_post_rerun_operator_reconciliation_review_guidance.py",
    ...
    "tools/emit_upbit_paper_post_rerun_operator_reconciliation_review_guidance_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_GUIDANCE.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    source_path = runtime_path("upbit_paper_post_rerun_reconciliation_blocker_rollup_report.json")
    blocker_rollup = load_json(source_path)
    ...
    if source_result.status != "PASS":
        raise RuntimeError(
            "source post-rerun reconciliation blocker rollup validation failed: "
    ...
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun operator reconciliation review guidance validation failed: "
            f"{result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_post_rerun_operator_reconciliation_review_guidance_report(root=ROOT, report=report)
    return report


def referenced_runtime_paths(report: dict[str, Any]) -> list[str]:
    paths = [str(report.get("source_blocker_rollup_path") or "")]
    for item in report.get("guidance_items", []):
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_GUIDANCE.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_GUIDANCE
...
runtime_summary:
- review_guidance_status: {report["review_guidance_status"]}
- primary_blocker_code: {report["primary_blocker_code"]}
- guidance_item_count: {report["guidance_item_count"]}
- review_step_count: {report["review_step_count"]}
- forbidden_output_count: {report["forbidden_output_count"]}
- source_rollup_item_count: {report["source_rollup_item_count"]}
- source_unique_blocker_count: {report["source_unique_blocker_count"]}
...
Upbit PAPER post-rerun reconciliation blockers now have a review-only operator guidance artifact. It gives review steps and forbidden outputs, but keeps current evidence writes, live readiness, live orders, and scale-up blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun operator reconciliation review guidance",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun reconciliation blockers must produce review-only operator guidance "
                "with explicit forbidden outputs and no current evidence writes"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun operator reconciliation review guidance",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_post_rerun_operator_reconciliation_review_guidance.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"post rerun reconciliation blockers produce review only operator guidance with forbidden outputs and no current evidence writes"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REVIEW_ONLY_OPERATOR_GUIDANCE_CURRENT_EVIDENCE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_post_rerun_operator_reconciliation_review_guidance.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_post_rerun_operator_reconciliation_review_guidance.py"],
            "evidence_artifacts": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REVIEW_ONLY_OPERATOR_GUIDANCE_CURRENT_EVIDENCE_BLOCKED",
        }
    )
    matrix.update(
        {
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_post_rerun_operator_reconciliation_review_guidance_validator"],
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_REVIEW_GUIDANCE_CURRENT_EVIDENCE_BLOCKED",
            "optimizer_guardrail_result": "PASS_REVIEW_GUIDANCE_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "true",
            "convergence_layer_changed": True,
            "convergence_state_after": "POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_GUIDANCE_CURRENT_EVIDENCE_BLOCKED",
            ...
            "post_rerun_reconciliation_blocker_rollup_status": report["source_blocker_rollup_status"],
            "post_rerun_reconciliation_blocker_rollup_item_count": report["source_rollup_item_count"],
            "post_rerun_reconciliation_unique_blocker_count": report["source_unique_blocker_count"],
            "post_rerun_reconciliation_primary_blocker_item_count": report["source_primary_blocker_item_count"],
            "post_rerun_operator_reconciliation_review_guidance_status": report["review_guidance_status"],
            "post_rerun_operator_reconciliation_review_guidance_item_count": report["guidance_item_count"],
            "post_rerun_operator_reconciliation_review_step_count": report["review_step_count"],
            "post_rerun_operator_reconciliation_forbidden_output_count": report["forbidden_output_count"],
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REVIEW_ONLY_OPERATOR_GUIDANCE_CURRENT_EVIDENCE_BLOCKED",
            "review_guidance_status": report["review_guidance_status"],
            "primary_blocker_code": report["primary_blocker_code"],
            "guidance_item_count": report["guidance_item_count"],
            "review_step_count": report["review_step_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Operator Reconciliation Review Guidance
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_post_rerun_operator_reconciliation_review_guidance_validator")
