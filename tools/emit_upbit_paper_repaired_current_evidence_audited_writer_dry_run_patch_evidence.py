...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-REPAIRED-CURRENT-EVIDENCE-AUDITED-WRITER-DRY-RUN"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_DASHBOARD_BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "trader1/runtime/paper/upbit_paper_repaired_current_evidence_audited_writer_dry_run.py",
    "trader1/validation/mvp0_validators.py",
    "tests/runtime/test_upbit_paper_repaired_current_evidence_audited_writer_dry_run.py",
    "tools/emit_upbit_paper_repaired_current_evidence_audited_writer_dry_run_patch_evidence.py",
...
def write_runtime_dry_run_report() -> dict[str, Any]:
    report = build_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(
        ...
        audited_writer_dry_run_id="upbit-paper-repaired-current-evidence-audited-writer-dry-run-20260502",
    )
    result = validate_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            f"audited writer dry-run validation failed: {result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(root=ROOT, report=report)
    return report


def validate_dry_run_projection(report: dict[str, Any]) -> None:
    current_preview = report.get("current_evidence_snapshot_preview", {})
    portfolio_preview = report.get("portfolio_snapshot_preview", {})
    if (
        ...
        or current_preview.get("configured_initial_cash_krw") != 1000000
        or current_preview.get("cash_status") != "UNVERIFIED"
        or portfolio_preview.get("verified_cash_krw") is not None
        or portfolio_preview.get("positions_verified") is not False
        ...
        raise RuntimeError("audited writer dry-run did not preserve dry-run-only blocked boundary")
...
task_class: MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN
...
included_schema_ids: ["trader1.upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The dry-run report previews current-evidence and portfolio truth outputs without writing them.
- The dry-run manifest hashes the source design, planned targets, pre/post checks, and preview payloads.
- Configured PAPER capital remains visible as config-only and unverified until an audited writer exists.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER repaired current-evidence audited writer dry-run",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: repaired current-evidence audited writer dry-run must preview "
                "current-evidence and portfolio truth outputs, keep configured 1,000,000 KRW PAPER capital "
                "unverified, and keep all writer, live, and scale-up permissions blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER repaired current-evidence audited writer dry-run",
            "requirement_kind": "RUNTIME_SAFETY_PATCH",
            "schema_ids": ["trader1.upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/runtime/test_upbit_paper_repaired_current_evidence_audited_writer_dry_run.py"],
            "mvp_stage": "MVP-4",
            ...
            "depends_on": [
                "REQ-MVP4-UPBIT-PAPER-REPAIRED-CURRENT-EVIDENCE-AUDITED-WRITER-DESIGN",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"upbit paper repaired current evidence audited writer dry run no writes no live no scale"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_DRY_RUN_WRITER_DISABLED_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.schema.json"],
            "validator_files": ["trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/runtime/test_upbit_paper_repaired_current_evidence_audited_writer_dry_run.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_repaired_current_evidence_audited_writer_dry_run.py"],
            "evidence_artifacts": [
                f"system/evidence/{PATCH_BASENAME}.evidence_manifest.json",
                f"system/evidence/validator_runs/{PATCH_BASENAME}.validator_run_log.json",
                f"system/evidence/stage_gates/{PATCH_BASENAME}.stage_gate_result.json",
                f"system/evidence/patch_results/{PATCH_BASENAME}.patch_result.json",
                base.rel(DRY_RUN_PATH),
            ],
            "dashboard_artifacts": [],
            "patch_result_fields": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_DRY_RUN_WRITER_DISABLED_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DESIGN.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "optimizer_status_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_BLOCKED",
            "optimizer_guardrail_result": "PASS_DRY_RUN_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_LIVE_BLOCKED",
    ...
    validate_dry_run_projection(report)
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_AUDITED_WRITER_DRY_RUN_CREATED_WRITER_DISABLED",
            "dry_run_status": report["dry_run_status"],
            "primary_blocker_code": report["primary_blocker_code"],
            "dry_run_check_pass_count": report["dry_run_check_pass_count"],
            "dry_run_check_blocked_count": report["dry_run_check_blocked_count"],
