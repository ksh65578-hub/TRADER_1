...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_CLOSURE_RECHECK_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-CURRENT-EVIDENCE-CLOSURE-RECHECK-DASHBOARD-BINDING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_current_evidence_closure_recheck_dashboard_binding_patch_evidence.py",
...
def load_recheck_report() -> dict[str, Any]:
    report = load_json(runtime_recheck_path())
    result = validate_upbit_paper_post_rerun_current_evidence_closure_recheck_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun current-evidence closure recheck validation failed: "
...
def validate_dashboard_projection(dashboard: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if dashboard.get("blocking_reason") != "POST_RERUN_RECONCILIATION_REQUIRED":
        raise RuntimeError("recheck dashboard binding did not keep POST_RERUN_RECONCILIATION_REQUIRED")
    if reconciliation.get("source") != "upbit_paper_post_rerun_current_evidence_closure_recheck_report.json":
        raise RuntimeError("dashboard did not prefer the post-rerun current-evidence closure recheck source")
    if reconciliation.get("post_rerun_current_evidence_closure_recheck_status") != "BLOCKED_POST_RERUN_CLOSURE_CONFIRMED":
        raise RuntimeError("dashboard did not surface the current-evidence closure recheck status")
    if reconciliation.get("post_rerun_current_evidence_closure_recheck_validation_status") != "PASS":
        raise RuntimeError("dashboard did not surface current-evidence closure recheck validation PASS")
    if reconciliation.get("post_rerun_current_evidence_bridge_status") != "BLOCKED_BY_POST_RERUN_CLOSURE":
        raise RuntimeError("dashboard did not surface the blocked current-evidence bridge")
    if (
        reconciliation.get("post_rerun_current_evidence_portfolio_recheck_status")
        != "LEDGER_PROVENANCE_PASS_BUT_OPERATOR_CURRENT_EVIDENCE_BLOCKED"
    ):
        raise RuntimeError("dashboard did not surface portfolio truth as blocked from operator-current evidence")
    if reconciliation.get("post_rerun_current_evidence_closure_recheck_current_evidence_write_allowed") is not False:
        raise RuntimeError("dashboard exposed recheck current-evidence write allowance")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    recheck_source = source_ids.get("POST_RERUN_CURRENT_EVIDENCE_CLOSURE_RECHECK")
    if not isinstance(recheck_source, dict) or recheck_source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish the recheck source artifact as PASS")
    if operator_action.get("status") != "BLOCKED" or operator_action.get("safe_to_continue_paper") is not False:
        raise RuntimeError("operator action did not fail closed for post-rerun closure recheck")
...
def write_context(now: str, trader_hash: str, agents_hash: str, dashboard: dict[str, Any], recheck: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / f"{PATCH_BASENAME}.md",
        f"""# {PATCH_BASENAME}

context_pack_id: {PATCH_BASENAME}
task_class: MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_CLOSURE_RECHECK_DASHBOARD_BINDING
...
included_schema_ids: ["trader1.read_only_dashboard_shell.v1", "trader1.upbit_paper_post_rerun_current_evidence_closure_recheck_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The Upbit PAPER dashboard loads the validated post-rerun current-evidence closure recheck report.
- The recheck is displayed as dashboard truth only, not current evidence, execution truth, or live readiness.
- POST_RERUN_RECONCILIATION_REQUIRED remains the dashboard blocking reason and operator primary blocker.
- Bridge status, portfolio recheck status, source bindings, ledger evidence statuses, and zero write allowance are visible.
...
- It does not resolve POST_RERUN_RECONCILIATION_REQUIRED; it makes the recheck guard operator-visible.
...
- source_closure_file_load_status: {recheck["source_closure_file_load_status"]}
- source_ledger_idempotency_file_load_status: {recheck["source_ledger_idempotency_file_load_status"]}
...
Upbit PAPER post-rerun current-evidence closure recheck is now visible in the read-only dashboard. It confirms ledger idempotency/provenance PASS is support-only and cannot override POST_RERUN_RECONCILIATION_REQUIRED.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun current-evidence closure recheck dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: current-evidence closure recheck must be visible in the operator dashboard "
                "without creating current evidence, live order, or scale-up permission"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun current-evidence closure recheck dashboard binding",
            ...
            "depends_on": [
                ...
                "REQ-MVP4-UPBIT-PAPER-POST-RERUN-RESOLUTION-CURRENT-EVIDENCE-CLOSURE-DASHBOARD-BINDING",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"post rerun current evidence closure recheck visible in operator dashboard without current evidence live order or scale up permission"
            ),
    ...
    rows.append(
        {
            ...
            "dashboard_artifacts": [
                "reconciliation_recovery_summary.post_rerun_current_evidence_closure_recheck_status",
                "reconciliation_recovery_summary.post_rerun_current_evidence_bridge_status",
                "reconciliation_recovery_summary.post_rerun_current_evidence_portfolio_recheck_status",
                "reconciliation_recovery_summary.post_rerun_current_evidence_closure_recheck_current_evidence_write_allowed",
                "source_artifacts.POST_RERUN_CURRENT_EVIDENCE_CLOSURE_RECHECK",
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_CLOSURE_RECHECK.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_CLOSURE_RECHECK_VISIBLE_IN_DASHBOARD",
            "optimizer_guardrail_result": "PASS_DASHBOARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "POST_RERUN_CLOSURE_RECHECK_DASHBOARD_VISIBLE_LIVE_BLOCKED",
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_VISIBLE_POST_RERUN_CLOSURE_RECHECK_LIVE_BLOCKED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            ...
            "portfolio_truth_recheck_status": reconciliation["post_rerun_current_evidence_portfolio_recheck_status"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Current-Evidence Closure Recheck Dashboard Binding Audit
...
def write_patch_artifacts(
    ...
    write_evidence(now, trader_hash, agents_hash, patch_result, dashboard, recheck, launcher_artifacts)
...
def main() -> int:
    ...
    recheck = load_recheck_report()
    dashboard, reconciliation, operator_action, launcher_artifacts = write_launcher_artifacts()
    write_context(now, trader_hash, agents_hash, dashboard, recheck)
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, dashboard, recheck)
    write_patch_artifacts(now, trader_hash, agents_hash, patch_result, dashboard, recheck, launcher_artifacts)
