...
PATCH_BASENAME = "MVP4_SUMMARY_PORTFOLIO_FRESHNESS_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-SUMMARY-PORTFOLIO-FRESHNESS-GUARD"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_summary_portfolio_freshness_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_SUMMARY_PORTFOLIO_FRESHNESS_GUARD.md",
]
VALIDATORS_REQUIRED = [
    "schema_validator",
...
def build_audit() -> dict[str, Any]:
    ...
    checks = {
        "summary_schema_carries_snapshot_freshness": '"source_snapshot_generated_at_utc"' in schema_text
        and '"source_snapshot_age_seconds"' in schema_text
        and '"source_snapshot_stale_after_seconds"' in schema_text
        and '"source_snapshot_freshness_message"' in schema_text,
        "summary_builder_downgrades_stale_snapshot": "PAPER portfolio snapshot is stale" in summary_text
        and "PAPER_PORTFOLIO_SNAPSHOT_STALE_AFTER_SECONDS" in summary_text,
        "summary_validator_blocks_stale_age_claim": "verified portfolio source snapshot is stale" in summary_text
        and "LATENCY_TTL_EXPIRED" in summary_text,
        "unit_tests_cover_stale_snapshot": "test_summary_downgrades_stale_paper_portfolio_snapshot" in test_text
        and "test_summary_blocks_verified_portfolio_stale_age_claim" in test_text
        and "test_summary_blocks_verified_portfolio_missing_snapshot_time" in test_text,
        "validator_registry_covers_stale_snapshot": "stale paper portfolio snapshot was not downgraded before dashboard display" in validator_text
        and "verified portfolio stale age claim was not blocked" in validator_text,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    return {
        "audit_schema_id": "trader1.summary_portfolio_freshness_guard_audit.v1",
        "status": "PASS" if not blockers else "BLOCKED",
        "blockers": blockers,
        "checks": checks,
        "hidden_defects": [
            {
                "classification": "dashboard_summary_stale_portfolio_false_fresh",
                "condition": "A hash-valid PAPER portfolio snapshot could be old while summary still copied it as PASS display truth",
                "impact": "dashboard could show stale cash, equity, positions, or PnL as if they were current",
                "fix": "summary now carries source snapshot generated_at, age, stale threshold, and downgrades stale snapshots before values are trusted",
            },
            {
                "classification": "dashboard_summary_missing_source_time_provenance",
                "condition": "verified summary portfolio rows did not require source timestamp provenance",
                "impact": "operator could not tell whether dashboard portfolio values were fresh or stale",
                "fix": "verified summary portfolio validation now requires timestamp and age fields and blocks stale age claims",
            },
        ],
...
def update_navigation(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_SUMMARY_PORTFOLIO_FRESHNESS_GUARD.md",
        f"""# MVP4_SUMMARY_PORTFOLIO_FRESHNESS_GUARD
...
""",
    ...
    ensure_requirement(
        req_index,
        {
            ...
            "source_heading": "dashboard summary portfolio freshness guard",
            "full_text_marker": f"{REQUIREMENT_ID}:dashboard summary portfolio source freshness must block stale PAPER values",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard portfolio freshness must be explicit before display",
            ...
            "depends_on": ["REQ-MVP4-SUMMARY-POSITION-ROLLUP-GUARD", "REQ-MVP4-PAPER-PORTFOLIO-POSITION-ROLLUP-GUARD"],
...
Dashboard summary portfolio values now carry source snapshot freshness metadata, and stale PAPER snapshots are downgraded before cash, equity, positions, or PnL can be displayed as trusted values.
    ...
    patch_result.update(
        {
            ...
            "target_mvp_level": "MVP-4",
            "patch_class": "RUNTIME_SAFETY_PATCH",
            "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-SUMMARY-POSITION-ROLLUP-GUARD", "REQ-MVP4-PAPER-PORTFOLIO-POSITION-ROLLUP-GUARD"],
            ...
            "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_SUMMARY_PORTFOLIO_FRESHNESS_GUARD",
