...
PATCH_BASENAME = "MVP4_DASHBOARD_RESIDUAL_OPERATOR_HANDOFF_CLARITY"
PATCH_ID = f"{PATCH_BASENAME}_20260505_001"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-RESIDUAL-OPERATOR-HANDOFF-CLARITY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_residual_operator_handoff_clarity_patch_evidence.py",
...
def load_handoff_report() -> dict[str, Any]:
    report = load_json(ROOT / HANDOFF_REPORT_PATH)
    if report.get("schema_id") != "trader1.residual_operator_handoff_packet_report.v1":
        raise RuntimeError("residual handoff packet report schema mismatch")
    if report.get("handoff_status") != "BLOCKED_HANDOFF_REQUIRED":
        raise RuntimeError("residual handoff packet report must remain blocked")
    for field in (
        "live_order_ready",
        "live_order_allowed",
        "can_live_trade",
    ...
    if report.get("open_gap_count") != len(BLOCKERS):
        raise RuntimeError("residual handoff report open gap count does not match residual blockers")
    if report.get("handoff_ready_count") != 0:
        raise RuntimeError("residual handoff report unexpectedly has ready handoff packets")
    if any(packet.get("handoff_status") != "BLOCKED_HANDOFF_REQUIRED" for packet in packets):
        raise RuntimeError("all residual handoff packets must remain blocked")
    return report
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP4-RESIDUAL-OPERATOR-HANDOFF-PACKET", "REQ-MVP4-DASHBOARD-RESIDUAL-ACTION-PLAN-SUMMARY", "REQ-MVP4-LIVE-FINAL-GUARD"]
included_schema_ids: ["trader1.read_only_dashboard_shell.v1", "trader1.residual_operator_handoff_packet_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS + DASHBOARD_ARTIFACTS + [HANDOFF_REPORT_PATH])}
...
- The dashboard summarizes {report["open_gap_count"]} open residual blockers as {report["handoff_packet_count"]} handoff packets without expanding the full technical list on the first screen.
- The dashboard shows blocked={report["blocked_handoff_packet_count"]}, ready={report["handoff_ready_count"]}, and the top operator actions from the handoff packet order.
...
The dashboard first screen now points operators to the residual handoff packets: {report["handoff_packet_count"]} packets remain blocked, {report["handoff_ready_count"]} are ready, and live execution remains unavailable.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard residual operator handoff clarity",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: dashboard must expose residual operator handoff packets as concise "
                "display-only live blocker guidance while preserving false live/scale permissions"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard residual operator handoff clarity",
            "requirement_kind": "DASHBOARD_UX_PATCH",
            "schema_ids": ["trader1.read_only_dashboard_shell.v1", "trader1.residual_operator_handoff_packet_report.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS + DASHBOARD_ARTIFACTS + [HANDOFF_REPORT_PATH],
            ...
            "depends_on": [
                ...
                "REQ-MVP4-DASHBOARD-RESIDUAL-ACTION-PLAN-SUMMARY",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(b"dashboard residual operator handoff clarity display only live blocked"),
    ...
    rows.append(
        {
            ...
            "fixture_files": [HANDOFF_REPORT_PATH],
    ...
    patch_result.update(
        {
            ...
            "optimizer_guardrail_result": "PASS_DASHBOARD_RESIDUAL_HANDOFF_CLARITY_LIVE_BLOCKED",
            "convergence_guardrail_result": "PASS_DASHBOARD_RESIDUAL_HANDOFF_CLARITY_LIVE_BLOCKED",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_DASHBOARD_RESIDUAL_HANDOFF_CLARITY_LIVE_BLOCKED",
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Residual Operator Handoff Clarity Audit
...
Patch:
- Bound the dashboard shell to {HANDOFF_REPORT_PATH} as display truth only.
- Added first-screen handoff counts: {report["handoff_packet_count"]} total, {report["blocked_handoff_packet_count"]} blocked, {report["handoff_ready_count"]} ready.
