...
PATCH_BASENAME = "MVP4_REVIEW_PLAN_DELETE_POLICY"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-REVIEW-PLAN-DELETE-POLICY"
...
from tools.review_plan_reflection_status import LEDGER_PATH, build_reflection_ledger, validate_reflection_ledger  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_review_plan_delete_policy_patch_evidence.py",
    "tests/contract/test_review_plan_reflection_status.py",
    "system/evidence/audit_reports/REVIEW_PLAN_REFLECTION_LEDGER.json",
    "contracts/generated/context_pack/MVP4_REVIEW_PLAN_DELETE_POLICY.md",
]
VALIDATORS_REQUIRED = [
...
def build_audit(now: str, trader_hash: str, agents_hash: str, ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    policy = ledger.get("deletion_policy", {})
    return {
        "audit_schema_id": "trader1.review_plan_delete_policy_audit.v1",
        "generated_at_utc": now,
        "authority": {"trader1_sha256": trader_hash, "agents_sha256": agents_hash},
        "status": "PASS"
        if validation["status"] == "PASS"
        and policy.get("original_review_file_preservation_required_after_reflection") is False
        else "BLOCKED",
        "review_files_checked": ledger.get("review_files_count"),
        "delete_ready_count": ledger.get("delete_ready_count"),
        "pending_reflection_count": ledger.get("pending_reflection_count"),
        "original_review_file_preservation_required_after_reflection": policy.get(
            "original_review_file_preservation_required_after_reflection"
        ),
        "delete_one_file_at_a_time": policy.get("delete_one_file_at_a_time"),
        "hidden_defects": [
            {
                "classification": "review_cleanup_policy_ambiguity",
                "condition": "operator intends reflected review-plan originals to be deleted, but the ledger wording could imply indefinite preservation.",
                "impact": "reflected review addenda could accumulate and make the backlog look larger than the remaining work.",
                "fix": "deletion policy now explicitly states original review file preservation is not required after reflection evidence; delete command defaults to one reflected file per run.",
                "reproducibility": "tests verify reflected files can be deleted without preserving the original and pending files remain blocked.",
                "live_safety_impact": "no live permission is created; deletion still requires reflected patch evidence and false live flags.",
                "ux_impact": "operator can refresh 검토안 with new files while reflected files are removed cleanly.",
            }
        ],
...
def update_navigation(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_heading": "Reflected 검토안 originals may be deleted",
            "full_text_marker": f"{REQUIREMENT_ID}: reflected review-plan files can be deleted without preserving the original",
            "authority_level": "USER_OPERATION_POLICY_WITH_ACTIVE_AUTHORITY_SAFETY",
            "requirement_title": "Delete reflected review plan originals after evidence",
            ...
            "depends_on": ["REQ-MVP4-REVIEW-PLAN-REFLECTION-LEDGER", "REQ-MVP4-LIVE-FINAL-GUARD"],
            "source_text_sha256": sha256_bytes(
                b"reflected review-plan files can be deleted without preserving the original"
            ),
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_REVIEW_PLAN_DELETE_POLICY.md",
        f"""# MVP4_REVIEW_PLAN_DELETE_POLICY
...
Reflected 검토안 originals may be deleted without preservation. Pending files remain blocked from deletion until reflection evidence exists.
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_REVIEW_PLAN_REFLECTION_LEDGER.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "retained_archive_semantic_mapping_status": "PRESERVED_NOT_READ_REVIEW_ADDENDUM_DELETE_POLICY",
            ...
            "active_read_surface_used": ["USER_REVIEW_PLAN_DELETE_POLICY", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "DOCUMENT_NORMALIZATION",
            "required_section_ids": ["USER_REVIEW_PLAN_DELETE_POLICY", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["USER_REVIEW_PLAN_DELETE_POLICY"],
            "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_REVIEW_PLAN_DELETE_POLICY",
