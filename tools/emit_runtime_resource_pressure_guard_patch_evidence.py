...
PATCH_BASENAME = "MVP4_LONG_RUN_STABILITY_RESOURCE_GUARD"
REQUIREMENT_ID = "REQ-MVP4-LONG-RUN-STABILITY-RESOURCE-GUARD"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/run_runtime_resource_pressure_validators.py",
    "tools/emit_runtime_resource_pressure_guard_patch_evidence.py",
    "contracts/generated/context_pack/LONG_RUN_STABILITY_RESOURCE_GUARD.md",
]
...
def update_validator_catalog(now: str) -> None:
    ...
    existing = {item["validator_id"] for item in implemented}
    if "runtime_resource_pressure_validator" not in existing:
        implemented.append(
            {
                "validator_id": "runtime_resource_pressure_validator",
                "module_path": "trader1.validation.mvp0_validators",
                "status": "IMPLEMENTED_FAIL_CLOSED",
                "live_enabling": False,
            }
        )
    registry["runtime_resource_pressure_module"] = "trader1.runtime.health.runtime_resource_pressure"
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    requirement = {
        ...
        "source_heading": "heartbeat runtime resource pressure and long-run stability guard",
        "full_text_marker": f"{REQUIREMENT_ID}:runtime resource pressure must be visible and fail closed",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "Long-run runtime resource pressure guard",
        "requirement_kind": "RUNTIME_SAFETY_PATCH",
        "schema_ids": ["trader1.heartbeat.v1", "trader1.read_only_dashboard_shell.v1"],
        "validator_ids": ["runtime_resource_pressure_validator", "heartbeat_validator", "read_only_dashboard_validator"],
        "artifact_ids": [
            "trader1/runtime/health/runtime_resource_pressure.py",
            "system/runtime/<exchange>/<market_type>/<mode>/<session_id>/heartbeat.json",
            "system/runtime/<exchange>/<market_type>/<mode>/<session_id>/dashboard_shell.json",
        ],
        "test_ids": [
            "tests/runtime/test_runtime_resource_pressure.py",
            "tests/runtime/test_safe_launcher.py",
        ...
        "depends_on": ["REQ-MVP1-HEARTBEAT", "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL", "REQ-MVP4-RUNTIME-STABILITY-HISTORY-VALIDATOR"],
        "source_text_sha256": sha256_bytes(b"runtime resource pressure must be visible and fail closed"),
    ...
    ensure_matrix_row(
        matrix,
        {
            ...
            "fixture_files": ["tests/runtime/test_runtime_resource_pressure.py"],
            "runtime_modules": [
                "trader1/runtime/health/runtime_resource_pressure.py",
                "trader1/runtime/boot/safe_launcher.py",
                "trader1/dashboard/read_only_dashboard.py",
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "LONG_RUN_STABILITY_RESOURCE_GUARD.md",
        f"""# LONG_RUN_STABILITY_RESOURCE_GUARD
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP1-HEARTBEAT", "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL"]
...
Runtime resource pressure is now inspected before launcher heartbeat/dashboard writes. The dashboard exposes Runtime artifact pressure as a separate stability metric, and stale write locks or hard artifact pressure fail closed through RESOURCE_LIMIT_BLOCK.
""",
    )


def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_RUNTIME_STABILITY_HISTORY_VALIDATOR.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "merged_requirements": [],
            "new_registry_items": [
                "contracts/registry.yaml::validators.runtime_resource_pressure_validator",
                "contracts/validators/validator_registry.json::runtime_resource_pressure_validator",
                *CHANGED_ARTIFACTS,
            ],
            ...
            "next_task_class": "MVP4_STALE_HEARTBEAT_RECOVERY_GUIDANCE",
            "next_required_section_ids": ["SECTION_MVP1_HEARTBEAT_SCOPE", "SECTION_DASHBOARD_SERVING_TRUTH", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "active_read_surface_used": ["SECTION_MVP1_HEARTBEAT_SCOPE", "SECTION_DASHBOARD_SERVING_TRUTH", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "MVP4_LONG_RUN_STABILITY_RESOURCE_GUARD",
            "required_section_ids": ["SECTION_MVP1_HEARTBEAT_SCOPE", "SECTION_DASHBOARD_SERVING_TRUTH", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SECTION_MVP1_HEARTBEAT_SCOPE", "SECTION_DASHBOARD_SERVING_TRUTH", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_RUNTIME_RESOURCE_PRESSURE_GUARD",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_LONG_RUN_RESOURCE_GUARD_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Long-Run Runtime Resource Guard Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_STALE_HEARTBEAT_RECOVERY_GUIDANCE"
    if REQUIREMENT_ID not in state["completed_requirement_ids"]:
        state["completed_requirement_ids"].append(REQUIREMENT_ID)
    if "runtime_resource_pressure_validator" not in state["implemented_validator_ids"]:
        state["implemented_validator_ids"].append("runtime_resource_pressure_validator")
    ...
    state["untested_validator_ids"] = [item for item in state.get("untested_validator_ids", []) if item != "runtime_resource_pressure_validator"]
...
def main() -> int:
    ...
    tests_run = [
        ...
        run_command([sys.executable, "-m", "unittest", "tests.runtime.test_runtime_resource_pressure", "tests.runtime.test_safe_launcher", "tests.dashboard.test_read_only_dashboard", "-v"]),
        run_command([sys.executable, "tools/run_runtime_resource_pressure_validators.py"]),
