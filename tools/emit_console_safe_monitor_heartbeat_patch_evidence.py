...
PATCH_BASENAME = "MVP4_CONSOLE_SAFE_MONITOR_HEARTBEAT"
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "CONSOLE_SAFE_MONITOR_HEARTBEAT.md",
        f"""# CONSOLE_SAFE_MONITOR_HEARTBEAT

context_pack_id: CONSOLE_SAFE_MONITOR_HEARTBEAT
task_class: MVP4_CONSOLE_SAFE_MONITOR_HEARTBEAT
...
- interactive root launcher console does not imply immediate shutdown after a short heartbeat burst
- interactive root launcher console enters SAFE_MONITOR with repeated heartbeat until Ctrl+C
- non-interactive automation remains bounded and cannot hang
...
Interactive root launchers now enter a console SAFE_MONITOR heartbeat loop until Ctrl+C. Non-interactive automation remains bounded. The console explicitly shows NO_TRADE, RUNNING_SAFE_MODE, and false live/order/scale-up flags.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_CONSOLE_HEARTBEAT_VISIBILITY.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "retained_archive_semantic_mapping_status": "UNCHANGED_NOT_REQUIRED_FOR_CONSOLE_SAFE_MONITOR_PATCH",
            ...
            "task_class": "MVP4_CONSOLE_SAFE_MONITOR_HEARTBEAT",
            "required_section_ids": ["SECTION_MVP1_ROOT_LAUNCHER_SCOPE", "SECTION_MVP1_HEARTBEAT"],
            ...
            "optimizer_output_type": "NO_CHANGE",
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_CONSOLE_SAFE_MONITOR_PATCH",
            ...
            "convergence_guardrail_result": "NO_CHANGE_LIVE_AND_SCALE_UP_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_CONSOLE_SAFE_MONITOR_HEARTBEAT_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "artifact_paths": [
                ...
                "tools/emit_console_safe_monitor_heartbeat_patch_evidence.py",
                "contracts/generated/context_pack/CONSOLE_SAFE_MONITOR_HEARTBEAT.md",
    ...
    write_text(
        ...
        f"""# MVP4 Console Safe Monitor Heartbeat Audit
