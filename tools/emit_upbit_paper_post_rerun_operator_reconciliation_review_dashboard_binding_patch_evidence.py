...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-OPERATOR-RECONCILIATION-REVIEW-DASHBOARD-BINDING"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RESOLUTION_AUDIT_REVIEW_ONLY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_operator_reconciliation_review_dashboard_binding_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING.md",
]
...
def runtime_guidance_path() -> Path:
    return (
        ROOT
        / "system"
        / "runtime"
...
def validate_dashboard_projection(dashboard: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if dashboard.get("blocking_reason") != "POST_RERUN_RECONCILIATION_REQUIRED":
        raise RuntimeError("post-rerun review guidance did not become the dashboard blocker")
    if reconciliation.get("source") != "upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.json":
        raise RuntimeError("dashboard did not prefer the post-rerun review guidance source")
    if reconciliation.get("post_rerun_review_guidance_status") != "BLOCKED_RECONCILIATION_REVIEW_REQUIRED":
        raise RuntimeError("dashboard did not surface the post-rerun review guidance as blocked")
    if reconciliation.get("post_rerun_review_step_count") < 4 or reconciliation.get("post_rerun_forbidden_output_count") < 5:
        raise RuntimeError("dashboard review guidance counts are incomplete")
    if reconciliation.get("post_rerun_guidance_current_evidence_write_allowed_count") != 0:
        raise RuntimeError("dashboard exposed post-rerun guidance current-evidence write allowance")
    if operator_action.get("status") != "BLOCKED" or operator_action.get("safe_to_continue_paper") is not False:
        raise RuntimeError("operator action did not fail closed for post-rerun review guidance")
...
def write_context(now: str, trader_hash: str, agents_hash: str, dashboard: dict[str, Any], guidance: dict[str, Any]) -> None:
    base.write_text(
        ROOT
        / "contracts"
        / "generated"
        / "context_pack"
        / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING
task_class: MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_REVIEW_DASHBOARD_BINDING
...
included_schema_ids: ["trader1.read_only_dashboard_shell.v1", "trader1.upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The Upbit PAPER dashboard loads validated post-rerun operator review guidance as display-only dashboard truth.
- POST_RERUN_RECONCILIATION_REQUIRED remains the dashboard blocking reason and operator primary blocker.
- Guidance item, review step, forbidden output, and zero current-evidence write counts are visible in the reconciliation panel.
...
- It does not resolve POST_RERUN_RECONCILIATION_REQUIRED; it makes the review-only guidance operator-visible.
...
- review_guidance_status: {guidance["review_guidance_status"]}
- guidance_item_count: {guidance["guidance_item_count"]}
- review_step_count: {guidance["review_step_count"]}
- forbidden_output_count: {guidance["forbidden_output_count"]}
- post_rerun_guidance_current_evidence_write_allowed_count: {guidance["current_evidence_write_allowed_count"]}
...
Upbit PAPER post-rerun operator reconciliation review guidance is now visible in the read-only dashboard. The dashboard and operator action both surface POST_RERUN_RECONCILIATION_REQUIRED and keep current evidence writes, live orders, and scale-up blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun operator reconciliation review dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun operator review guidance must be visible in the operator dashboard "
                "without creating current evidence, live order, or scale-up permission"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun operator reconciliation review dashboard binding",
            ...
            "depends_on": [
                ...
                "REQ-MVP4-UPBIT-PAPER-POST-RERUN-OPERATOR-BLOCKER-DASHBOARD-VISIBILITY",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"post rerun operator review guidance visible in operator dashboard without current evidence live order or scale up permission"
            ),
    ...
    rows.append(
        {
            ...
            "fixture_files": [
                "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_post_rerun_operator_reconciliation_review_guidance_report.json"
            ],
            ...
            "dashboard_artifacts": [
                "reconciliation_recovery_summary.post_rerun_review_guidance_status",
                "reconciliation_recovery_summary.post_rerun_review_step_count",
                "reconciliation_recovery_summary.post_rerun_forbidden_output_count",
                "operator_action_summary.primary_blocker_code",
                "blocking_reason",
            ],
            "patch_result_fields": [
                "post_rerun_operator_reconciliation_review_guidance_status",
                "post_rerun_operator_reconciliation_review_guidance_item_count",
                "post_rerun_operator_reconciliation_review_step_count",
                "post_rerun_operator_reconciliation_forbidden_output_count",
...
def build_patch_result(
    ...
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_BLOCKER_DASHBOARD_VISIBILITY.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "post-rerun operator reconciliation review guidance",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_POST_RERUN_REVIEW_GUIDANCE_VISIBLE_IN_DASHBOARD",
            ...
            "convergence_state_after": "POST_RERUN_REVIEW_GUIDANCE_DASHBOARD_VISIBLE_LIVE_BLOCKED",
            ...
            "post_rerun_operator_reconciliation_review_guidance_status": guidance["review_guidance_status"],
            "post_rerun_operator_reconciliation_review_guidance_item_count": guidance["guidance_item_count"],
            "post_rerun_operator_reconciliation_review_step_count": guidance["review_step_count"],
            "post_rerun_operator_reconciliation_forbidden_output_count": guidance["forbidden_output_count"],
            "post_rerun_current_evidence_write_authorized_count": guidance["current_evidence_write_authorized_count"],
            "post_rerun_current_evidence_write_allowed_count": guidance["current_evidence_write_allowed_count"],
            "candidate_current_evidence_usable_count": guidance["candidate_current_evidence_usable_count"],
        }
    )
    if dashboard.get("blocking_reason") != "POST_RERUN_RECONCILIATION_REQUIRED":
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_VISIBLE_POST_RERUN_REVIEW_GUIDANCE_LIVE_BLOCKED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            "operator_action_status": operator_action["status"],
            "operator_primary_blocker_code": operator_action["primary_blocker_code"],
            "reconciliation_status": reconciliation["status"],
            "post_rerun_review_guidance_status": reconciliation["post_rerun_review_guidance_status"],
            "post_rerun_review_guidance_item_count": reconciliation["post_rerun_review_guidance_item_count"],
            "post_rerun_review_step_count": reconciliation["post_rerun_review_step_count"],
            "post_rerun_forbidden_output_count": reconciliation["post_rerun_forbidden_output_count"],
            "post_rerun_guidance_current_evidence_write_allowed_count": reconciliation[
    ...
    base.write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "operator_action_status": operator_action["status"],
            "operator_primary_blocker_code": operator_action["primary_blocker_code"],
            "post_rerun_review_guidance_status": guidance["review_guidance_status"],
            "post_rerun_review_guidance_item_count": guidance["guidance_item_count"],
            "post_rerun_review_step_count": guidance["review_step_count"],
            "post_rerun_forbidden_output_count": guidance["forbidden_output_count"],
            "post_rerun_guidance_current_evidence_write_allowed_count": guidance["current_evidence_write_allowed_count"],
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Review Guidance Dashboard Binding Audit
...
def write_patch_artifacts(
    ...
    write_evidence(now, trader_hash, agents_hash, patch_result, dashboard, guidance, launcher_artifacts)
...
def main() -> int:
    ...
    write_context(now, trader_hash, agents_hash, dashboard, guidance)
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, dashboard, guidance)
    write_patch_artifacts(now, trader_hash, agents_hash, patch_result, dashboard, guidance, launcher_artifacts)
