...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-REVIEW-GUIDANCE-SOURCE-BINDING-GUARD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RESOLUTION_AUDIT_SOURCE_BINDING_GUARD"

if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

import tools.emit_persistent_runtime_resource_boundary_patch_evidence as base  # noqa: E402
import tools.emit_upbit_paper_post_rerun_operator_reconciliation_review_guidance_patch_evidence as previous  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_review_guidance_source_binding_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD.md",
]
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD
task_class: MVP4_UPBIT_PAPER_POST_RERUN_REVIEW_GUIDANCE_SOURCE_BINDING_GUARD
...
- The operator review guidance reloads the source blocker rollup artifact from disk.
- The source artifact's stored blocker_rollup_hash and recomputed hash must match the in-memory source blocker rollup hash.
- Missing, invalid, scope-mismatched, or hash-mismatched source blocker rollup files block the guidance.
...
- source_file_load_status: {report["source_blocker_rollup_file_load_status"]}
- source_file_hash_match: {str(report["source_blocker_rollup_file_hash_match"]).lower()}
...
Upbit PAPER post-rerun operator reconciliation review guidance is now bound to the persisted blocker rollup artifact hash. Missing or mismatched source files block guidance while all live and scale-up flags remain false.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun review guidance source binding guard",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun operator review guidance must bind source blocker rollup "
                "evidence to the persisted source artifact hash before operator guidance is accepted"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun review guidance source binding guard",
            ...
            "depends_on": [
                ...
                "REQ-MVP4-UPBIT-PAPER-POST-RERUN-BLOCKER-ROLLUP-SOURCE-BINDING-GUARD",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(b"post rerun review guidance source blocker rollup file hash binding"),
    ...
    rows.append(
        {
            ...
            "patch_result_fields": [
                ...
                "post_rerun_review_guidance_source_blocker_rollup_file_load_status",
                "post_rerun_review_guidance_source_blocker_rollup_file_hash_match",
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "post-rerun operator reconciliation review guidance report",
                "persisted blocker rollup artifact",
                "live final guard",
            ],
            "required_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LEDGER_RECONCILIATION", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LEDGER_RECONCILIATION", "SECTION_LIVE_FINAL_GUARD"],
            "post_rerun_review_guidance_source_blocker_rollup_file_load_status": report["source_blocker_rollup_file_load_status"],
            "post_rerun_review_guidance_source_blocker_rollup_file_hash_match": report["source_blocker_rollup_file_hash_match"],
            "optimizer_guardrail_result": "PASS_REVIEW_GUIDANCE_SOURCE_BLOCKER_ROLLUP_ARTIFACT_HASH_BOUND",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Review Guidance Source Binding Guard Audit
...
- review_guidance_status: {report["review_guidance_status"]}
- source_file_load_status: {report["source_blocker_rollup_file_load_status"]}
- source_file_hash_match: {report["source_blocker_rollup_file_hash_match"]}
- current_evidence_write_allowed_count: {report["current_evidence_write_allowed_count"]}
