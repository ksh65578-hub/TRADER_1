...
PATCH_BASENAME = "MVP4_PAPER_SHADOW_ACTIONABILITY_DEFICIT_SUMMARY"
PATCH_ID = f"{PATCH_BASENAME}_20260506_001"
REQUIREMENT_ID = "REQ-MVP4-PAPER-SHADOW-ACTIONABILITY-DEFICIT-SUMMARY"
...
FIXTURE_ARTIFACTS = [
    "tests/validators/fixtures/paper_shadow_evidence_accumulation_false_long_run_claim_fail.json",
    "tests/validators/fixtures/paper_shadow_evidence_accumulation_identity_mismatch_fail.json",
    "tests/validators/fixtures/paper_shadow_evidence_accumulation_identity_stale_fail.json",
...
CHANGED_ARTIFACTS = [
    ...
    "tests/research/test_shadow_observation_runtime.py",
    "tests/validators/test_paper_shadow_evidence_accumulation_validator.py",
    "tools/emit_paper_shadow_actionability_deficit_summary_patch_evidence.py",
    CONTEXT_PACK_PATH,
    *FIXTURE_ARTIFACTS,
    *ROUTE_GUARD_TEST_ARTIFACTS,
]
...
def build_report(now: str, trader_hash: str, agents_hash: str) -> dict[str, Any]:
    ...
    return {
        "schema_id": "trader1.paper_shadow_actionability_deficit_summary_report.v1",
        "patch_id": PATCH_ID,
        "generated_at_utc": now,
        "authority": {"trader1_sha256": trader_hash, "agents_sha256": agents_hash},
        "finding": (
            "PAPER/SHADOW evidence accumulation had hard PASS/BLOCKED gates, but it did not expose a deterministic "
            "deficit vector telling the operator which non-live evidence dimension is next: PAPER samples, SHADOW "
            "samples, reason/cost evidence, paired windows, span, or validated runtime source binding."
        ),
        ...
        "acceptance_conditions": [
            "paper_shadow_evidence_accumulation_report carries actionability version, status, primary deficit, next action, and numeric deficits",
            "validator recomputes actionability fields and blocks drift even when the evidence hash is recomputed",
...
- PAPER/SHADOW evidence actionability priority is {", ".join(report["closed_priority_order"])}.
...
PAPER/SHADOW evidence accumulation now exposes deterministic actionability deficits. It can say which non-live evidence dimension is next, but it still cannot close gaps, write LIVE_READY, or permit live orders.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "PAPER/SHADOW evidence actionability deficit summary",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: paper/shadow evidence accumulation must expose a deterministic deficit vector and "
                "next collection action without creating live readiness or closing gaps"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "PAPER/SHADOW evidence actionability deficit summary",
            "requirement_kind": "EVIDENCE_VALIDATOR_PATCH",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_PROFIT_CONVERGENCE", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": [
            ...
            "source_text_sha256": base.sha256_bytes(
                b"paper shadow evidence actionability deficit summary deterministic non-live"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_ACTIONABILITY_DEFICITS_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "runtime_modules": ["trader1/research/shadow/shadow_runner.py", "trader1/research/shadow/evidence_accumulator.py"],
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_ACTIONABILITY_DEFICITS_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
    ...
    patch_result.update(
        {
            ...
            "next_required_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_PROFIT_CONVERGENCE", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "task_class": "PROFIT_CONVERGENCE_MVP3",
            "required_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_PROFIT_CONVERGENCE", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_PROFIT_CONVERGENCE", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "current_implementation_state_status": "UPDATED_PAPER_SHADOW_ACTIONABILITY_DEFICITS_LIVE_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_PAPER_SHADOW_ACTIONABILITY_DEFICITS_LIVE_BLOCKED",
    ...
    base.write_text(
        ...
        f"""# MVP4 PAPER/SHADOW Actionability Deficit Summary Audit
...
def write_session_artifacts(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    areas = [
        ...
        ("paper / shadow / replay / micro-live / live", "Critical", "Patched PAPER/SHADOW evidence report only.", "No micro-live/live path touched."),
        ...
        ("order lifecycle / execution quality / partial fill", "High", "No order-capable path touched.", "Actionability is report-only."),
        ...
        ("dashboard / USER_STATUS_SUMMARY / user simplicity", "Medium", "Report now gives user-level next action.", "Dashboard wiring can consume this later."),
        ("validator / schema / registry / acceptance artifacts", "Medium", "Schema, validator, fixtures, and evidence updated.", "Validators pass."),
...
""",
    ...
    base.write_json(
        SESSION_DIR / "PAPER_RUN_SUMMARY.json",
        {
            ...
            "paper_shadow_actionability_added": True,
    ...
    base.write_json(
        SESSION_DIR / "DASHBOARD_READINESS_SUMMARY.json",
        {
            "patch_id": PATCH_ID,
            "generated_at_utc": now,
            "status": "PASS_EVIDENCE_ACTIONABILITY_ADDED_LIVE_BLOCKED",
            "actionability_version": report["actionability_version"],
...
Current state: PAPER/SHADOW evidence reports now say what evidence is missing next, but live trading is still blocked.

What changed:
- The evidence report now gives one primary next action.
...
def main() -> int:
    ...
    tests_run: list[dict[str, Any]] = [
        ...
        run_command([sys.executable, "-B", "-m", "pytest", "-p", "no:cacheprovider", "tests/research/test_paper_shadow_evidence_accumulator.py", "tests/research/test_shadow_observation_runtime.py", "-q"]),
