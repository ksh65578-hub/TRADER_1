...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-LEDGER-ROLLUP-CONSISTENCY-GUARD"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_ledger_rollup_consistency_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD.md",
]
VALIDATORS_REQUIRED = [
    "registry_validator",
    "schema_validator",
...
def run_runtime_artifacts() -> list[str]:
    loop = run_upbit_paper_persistent_loop(
        root=ROOT,
        loop_id="mvp4-upbit-paper-ledger-rollup-consistency-guard",
    ...
    paths.append(
        ...
        "paper_runtime/mvp4-upbit-paper-ledger-rollup-consistency-guard.persistent_loop_report.json"
    )
...
def build_audit() -> dict[str, Any]:
    ...
    checks = {
        "portfolio_scope_guard": "paper ledger rollup portfolio scope mismatch" in module_text,
        "filled_count_position_guard": "filled PAPER rollup requires at least one portfolio position" in module_text,
        "latest_head_hash_guard": "requires latest ledger head hash" in module_text,
        "artifact_path_namespace_guard": "artifact path escaped PAPER ledger namespace" in module_text,
        "validator_cross_scope_negative": "cross-scope portfolio snapshot was not blocked" in validator_text,
        "validator_count_mismatch_negative": "filled count mismatch was not failed closed" in validator_text,
        "validator_path_escape_negative": "artifact path escape was not blocked" in validator_text,
        "test_cross_scope_portfolio": "test_rollup_blocks_cross_scope_portfolio_snapshot" in test_text,
        "test_count_mismatch": "test_rollup_blocks_filled_count_portfolio_mismatch" in test_text,
        "test_path_escape": "test_rollup_blocks_artifact_path_escape" in test_text,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    return {
        "audit_schema_id": "trader1.upbit_paper_ledger_rollup_consistency_guard_audit.v1",
        "status": "PASS" if not blockers else "BLOCKED",
        "blockers": blockers,
        "checks": checks,
        "hidden_defects": [
            {
                "classification": "cross_scope_portfolio_truth_mixing",
                "condition": "paper ledger rollup could carry a portfolio snapshot from a different supported paper scope after hash recomputation",
                "impact": "dashboard portfolio truth could show a mismatched exchange/market/session balance",
                "fix": "rollup validator now requires portfolio scope to match the rollup scope exactly",
            },
            {
                "classification": "rollup_count_false_safe",
                "condition": "filled_order_count could be changed independently from the portfolio positions after hash recomputation",
                "impact": "operator could see inconsistent positions/PnL vs execution count",
                "fix": "rollup validator now checks filled count, position count, and latest ledger head requirements",
            },
            {
                "classification": "cross_namespace_rollup_artifact_path",
                "condition": "rollup artifact paths could be mutated outside the UPBIT/KRW_SPOT/PAPER ledger namespace",
                "impact": "dashboard/evidence could reference live or cross-mode artifacts",
                "fix": "rollup validator now blocks artifact path namespace escape",
            },
        ],
...
def update_context(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD
...
Upbit PAPER ledger rollup validation now blocks cross-scope portfolio snapshots, count/position drift, missing ledger head truth, and cross-namespace artifact paths. This is PAPER-only hardening and does not create long-run evidence eligibility or live readiness.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER ledger rollup consistency guard",
            "full_text_marker": f"{REQUIREMENT_ID}:paper ledger rollup portfolio and artifact truth must remain scope-bound and count-consistent",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "PAPER ledger rollup must not false-pass cross-scope or count-drifted portfolio truth",
            "requirement_kind": "VALIDATOR_PATCH",
            "schema_ids": ["trader1.paper_ledger_rollup_report.v1", "trader1.paper_portfolio_snapshot.v1", "trader1.patch_result.v1"],
            ...
            "live_affecting": True,
            "read_when": ["SECTION_LEDGER_RECONCILIATION", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_NAMESPACE_SEPARATION", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": ["REQ-MVP4-UPBIT-PAPER-LEDGER-ROLLUP", "REQ-MVP4-UPBIT-PAPER-RUNTIME-E2E-CONSISTENCY-GUARD"],
            "source_text_sha256": sha256_bytes(b"paper ledger rollup portfolio and artifact truth must remain scope-bound and count-consistent"),
...
def build_patch_result(
    ...
    patch_result: dict[str, Any] = {
        ...
        "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD",
        ...
        "failure_analysis_status": "NOT_REQUIRED_LEDGER_ROLLUP_CONSISTENCY_GUARD",
...
def write_evidence(
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_UPBIT_PAPER_LEDGER_ROLLUP_CONSISTENCY_GUARD_NO_LIVE_ORDERS",
            "ledger_rollup_consistency_guard_audit": audit,
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Upbit PAPER Ledger Rollup Consistency Guard
