...
PATCH_BASENAME = "MVP4_DASHBOARD_ACTUAL_LONG_RUN_FLOOR_UX"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-ACTUAL-LONG-RUN-FLOOR-UX"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_actual_long_run_floor_ux_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_DASHBOARD_ACTUAL_LONG_RUN_FLOOR_UX.md",
]
...
def regenerate_paper_dashboards() -> list[dict[str, Any]]:
    regenerated: list[dict[str, Any]] = []
    for launcher_name in ("UPBIT_PAPER", "BINANCE_PAPER"):
        report = build_launcher_report(launcher_name)
        report_path, paths = write_launcher_runtime_bundle(report)
        regenerated.append(
            {
                "launcher_name": launcher_name,
                "report_path": rel(report_path),
                "dashboard_shell_path": rel(paths["dashboard_shell"]),
                "dashboard_html_path": rel(paths["dashboard_html"]),
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_DASHBOARD_ACTUAL_LONG_RUN_FLOOR_UX.md",
        f"""# MVP4_DASHBOARD_ACTUAL_LONG_RUN_FLOOR_UX
...
The dashboard now keeps actual long-run evidence in COLLECTING state until the 86400s floor is met. Current stable operation can still be shown as healthy, but it no longer makes the Runtime Evidence Boundary look validated before real PAPER/SHADOW duration exists.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard actual long-run evidence floor",
            "full_text_marker": f"{REQUIREMENT_ID}: dashboard must not show actual long-run evidence validated before 86400s runtime evidence",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard actual long-run floor UX",
            ...
            "source_text_sha256": sha256_bytes(
                b"dashboard must not show actual long-run evidence validated before 86400s runtime evidence"
            ),
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], regenerated: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_DASHBOARD_LONG_RUN_EVIDENCE_REQUIREMENTS_VISIBILITY.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "dashboard long-run operator summary",
                "runtime evidence boundary",
                "read-only dashboard validator",
            ],
            "task_class": NEXT_TASK_CLASS,
            ...
            "optimizer_stage": "NOT_CHANGED_DASHBOARD_BOUNDARY_ONLY",
            ...
            "optimizer_guardrail_result": "PASS_DASHBOARD_LONG_RUN_FLOOR_BLOCKED_BEFORE_86400S",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "convergence_state_before": "LONG_RUN_EVIDENCE_BLOCKED_UPBIT_SCOPE_BOUNDARY_ENFORCED",
            "convergence_state_after": "LONG_RUN_EVIDENCE_BLOCKED_DASHBOARD_FLOOR_UX_ENFORCED",
            "convergence_guardrail_result": "PASS_DASHBOARD_LONG_RUN_FLOOR_BLOCKED_BEFORE_86400S",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_DASHBOARD_ACTUAL_LONG_RUN_FLOOR_UX_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Actual Long-Run Floor UX Audit
...
def main() -> int:
    ...
    regenerated = regenerate_paper_dashboards()
