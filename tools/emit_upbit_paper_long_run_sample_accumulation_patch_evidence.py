...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_LONG_RUN_SAMPLE_ACCUMULATION"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-LONG-RUN-SAMPLE-ACCUMULATION-SPAN-PROVENANCE"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_LONG_RUN_RUNTIME_SAMPLE_HISTORY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_long_run_sample_accumulation_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_LONG_RUN_SAMPLE_ACCUMULATION.md",
]
...
def write_context(now: str, trader_hash: str, agents_hash: str) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_LONG_RUN_SAMPLE_ACCUMULATION.md",
        f"""# MVP4_UPBIT_PAPER_LONG_RUN_SAMPLE_ACCUMULATION
...
PAPER/SHADOW evidence now records span provenance and paired supporting-window count. Aggregated PAPER operation reports without an explicit evidence_span_hours input stay blocked from long-run evidence as NOT_PROVIDED/MISSING.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER long-run sample accumulation provenance",
            "full_text_marker": f"{REQUIREMENT_ID}: long-run PAPER/SHADOW sample accumulation requires explicit span provenance and paired supporting-window count",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER long-run sample accumulation span provenance",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": [
                "REQ-MVP4-UPBIT-PAPER-LONG-RUN-EVIDENCE-BOUNDARY",
                "REQ-MVP4-PAPER-SHADOW-RUNTIME-SOURCE-REQUIRED-FIELDS",
                "REQ-MVP4-PAPER-SHADOW-SUPPORTING-WINDOW-PAIRING",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(b"long-run sample accumulation span provenance and supporting window count"),
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PAPER_LONG_RUN_EVIDENCE_BOUNDARY.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "next_required_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "active_read_surface_used": [
                ...
                "paper/shadow evidence accumulator",
                "paper/shadow evidence validator",
                "검토안-derived long-run evidence gap plan",
            ],
            "task_class": "MVP4_UPBIT_PAPER_LONG_RUN_SAMPLE_ACCUMULATION",
            "required_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SECTION_PAPER_SHADOW_EVIDENCE", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_SPAN_PROVENANCE_REQUIRED",
            "optimizer_guardrail_result": "PASS_LONG_RUN_SAMPLE_ACCUMULATION_REMAINS_LIVE_BLOCKED",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "true",
            "convergence_layer_changed": True,
            "convergence_state_after": "LONG_RUN_EVIDENCE_BLOCKED_UNTIL_SPAN_PROVENANCE_AND_RUNTIME_SOURCE_PASS",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_SPAN_PROVENANCE_REQUIRED_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Long-Run Sample Accumulation Audit
...
def main() -> int:
    ...
    tests_run = [
        base.run_command([sys.executable, "-m", "pytest", "-p", "no:cacheprovider", "tests/validators/test_paper_shadow_evidence_accumulation_validator.py", "tests/research/test_paper_shadow_evidence_accumulator.py", "-q"]),
