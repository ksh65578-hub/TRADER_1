...
PATCH_BASENAME = "MVP4_SESSION2_REAUDIT_REFLECTION"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-SESSION2-REAUDIT-REFLECTION"
    ...
    patch_result.update(
        {
            ...
            "retained_archive_semantic_mapping_status": "SESSION2_REAUDIT_REFLECTED_STALE_FINDINGS_SPLIT_FROM_CURRENT_GAPS",
            "read_cache_update_required": False,
            "context_pack_update_required": False,
            "current_implementation_state_updated": True,
            "next_task_class": NEXT_TASK_CLASS,
            "next_required_section_ids": [
                "SESSION2_REAUDIT_ADDENDUM",
            ...
            "active_read_surface_used": ["SESSION2_REAUDIT_ADDENDUM", "AGENTS_0G", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "DOCUMENT_NORMALIZATION",
            "required_section_ids": ["SESSION2_REAUDIT_ADDENDUM", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": ["SESSION2_REAUDIT_ADDENDUM"],
            ...
            "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_SESSION2_REAUDIT_REFLECTION",
...
def build_audit(now: str, trader_hash: str, agents_hash: str, ledger: dict[str, Any], deleted_files: list[str]) -> dict[str, Any]:
    ...
    return {
        "audit_schema_id": "trader1.session2_reaudit_reflection_audit.v1",
        ...
        "session2_deleted_after_reflection": SESSION2_FILE in deleted_files or session2.get("reflection_status") == "DELETED_AFTER_REFLECTION",
        "stale_findings_resolved_or_regression_guarded": [
            ...
            "validator registry resolves to current trader1.validation.mvp0_validators implementation",
            "fixture catalog is present and tracked in current repo",
            "source bundle manifest is regenerated after reflection",
            "pytest is executable in current repo",
        ],
        "current_defects_or_open_gaps_from_session2": [
            ...
            "validator depth must continue to be checked against real negative fixtures",
            "MICRO_LIVE is not enabled and remains out of scope before external evidence",
        ],
        "external_blockers_from_session2": [
...
def update_navigation(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_section_id": "SESSION2_REAUDIT_ADDENDUM",
            "source_file": SESSION2_FILE,
            "source_heading": "TRADER_1.zip 전수 재검토 세션 2",
            "full_text_marker": "Session 2 review is reflected as stale/current/external categories before deletion.",
            "authority_level": "REVIEW_ADDENDUM_NON_AUTHORITY",
            "requirement_title": "Reflect Session 2 reaudit without overriding active authority",
            ...
            "source_text_sha256": audit.get("session2_sha256") or sha256_bytes(b"session2 missing"),
    ...
    ensure_matrix_row(
        ...
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SESSION2_REAUDIT_ADDENDUM",
            "schema_files": [],
...
included_section_ids: ["SESSION2_REAUDIT_ADDENDUM", "SECTION_LIVE_FINAL_GUARD"]
...
Session 2 stale zip findings are separated from current repo gaps. No review-plan files remain pending after this reflection.
""",
    )
...
def main() -> int:
    ...
    write_json(
        audit_path,
        {
            "audit_schema_id": "trader1.session2_reaudit_reflection_audit.v1",
            "generated_at_utc": now,
            "status": "BOOTSTRAP_FOR_SESSION2_REFLECTION",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    mark_current_files_reflected(
        ...
        review_files=[SESSION2_FILE],
    )
    deleted_files = delete_reflected_files(ledger, max_delete_count=1)
    if not deleted_files and any(
        entry.get("review_file") == SESSION2_FILE and entry.get("reflection_status") == "DELETED_AFTER_REFLECTION"
        for entry in ledger.get("review_files", [])
    ):
        deleted_files = [SESSION2_FILE]
