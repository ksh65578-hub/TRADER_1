...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_BLOCKED_REPAIR_PLAN"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-BLOCKED-REPAIR-PLAN"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_LEDGER_ROLLUP_REBUILD_READY_REPAIR"

if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

import tools.emit_persistent_runtime_resource_boundary_patch_evidence as base  # noqa: E402
from trader1.runtime.paper.upbit_paper_blocked_repair_plan import (  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_blocked_repair_plan_report.json",
    "contracts/security/source_bundle_manifest.json",
    "tools/emit_upbit_paper_blocked_repair_plan_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_BLOCKED_REPAIR_PLAN.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    ...
    report = build_upbit_paper_blocked_repair_plan_report(
        root=ROOT,
        post_reconciliation_report=post_report,
        source_report_path=base.rel(post_path),
    )
    result = validate_upbit_paper_blocked_repair_plan_report(report)
    if result.status != "PASS":
        raise RuntimeError(f"blocked repair plan failed validation: {result.status} {result.message}")
    write_upbit_paper_blocked_repair_plan_report(root=ROOT, report=report)
    return report


def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_BLOCKED_REPAIR_PLAN.md",
        f"""# MVP4_UPBIT_PAPER_BLOCKED_REPAIR_PLAN
...
runtime_summary:
- repair_plan_status: {report["repair_plan_status"]}
- repair_item_count: {report["repair_item_count"]}
- repair_lane_counts: {json.dumps(report["repair_lane_counts"])}
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER blocked repair plan",
            "full_text_marker": f"{REQUIREMENT_ID}: blocked regenerated PAPER replacements must be repair-lane classified before any repair or evidence use",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER blocked repair plan",
            "requirement_kind": "SCHEMA_VALIDATOR_RUNTIME_ARTIFACT_PATCH",
            "schema_ids": ["trader1.upbit_paper_blocked_repair_plan_report.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/runtime/test_upbit_paper_blocked_repair_plan.py"],
            "mvp_stage": "MVP-4",
            ...
            "depends_on": [
                "REQ-MVP4-UPBIT-PAPER-BLOCKED-REPAIR-REASON-ROLLUP",
                "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-POST-REGENERATION-RECONCILIATION",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(b"blocked regenerated PAPER replacements must be repair lane classified before evidence use"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REPAIR_PLAN_VISIBLE_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/upbit_paper_blocked_repair_plan_report.schema.json"],
            "validator_files": [
                "trader1/runtime/paper/upbit_paper_blocked_repair_plan.py",
                "trader1/validation/mvp0_validators.py",
            ],
            "test_files": ["tests/runtime/test_upbit_paper_blocked_repair_plan.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_blocked_repair_plan.py"],
            "evidence_artifacts": [
                "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_blocked_repair_plan_report.json",
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REPAIR_PLAN_VISIBLE_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PAPER_BLOCKED_REPAIR_REASON_ROLLUP.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_blocked_repair_plan_validator"],
            "new_or_changed_schema_ids": ["trader1.upbit_paper_blocked_repair_plan_report.v1"],
            ...
            "active_read_surface_used": [
                ...
                "blocked repair reason rollup context pack",
                "PAPER ledger rollup runtime artifacts",
            ],
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_BLOCKED_REPAIR_PLAN_VISIBLE_NOT_LIVE_READY",
            "optimizer_guardrail_result": "PASS_REPAIR_PLAN_EXCLUDES_BLOCKED_REPLACEMENTS_FROM_EVIDENCE",
            "convergence_state_after": "STALE_RUNTIME_LOOP_BLOCKED_REPAIR_PLAN_VISIBLE_LIVE_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REPAIR_PLAN_VISIBLE_LIVE_BLOCKED",
            "repair_plan_status": report["repair_plan_status"],
            "repair_lane_counts": report["repair_lane_counts"],
            "ledger_rollup_rebuild_ready_count": report["ledger_rollup_rebuild_ready_count"],
            "runtime_cycle_rerun_required_count": report["runtime_cycle_rerun_required_count"],
            "recovery_guard_rerun_required_count": report["recovery_guard_rerun_required_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Blocked Repair Plan Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    gaps.add("BLOCKED_REPAIR_PLAN_REQUIRES_OPERATOR_RECONCILIATION")
    ...
    state["implemented_schema_ids"] = sorted(set(state.get("implemented_schema_ids", [])) | {"trader1.upbit_paper_blocked_repair_plan_report.v1"})
    state["implemented_validator_ids"] = sorted(set(state.get("implemented_validator_ids", [])) | {"upbit_paper_blocked_repair_plan_validator"})
