...
PATCH_BASENAME = "MVP4_CONSOLE_HEARTBEAT_VISIBILITY"
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "CONSOLE_HEARTBEAT_VISIBILITY.md",
        f"""# CONSOLE_HEARTBEAT_VISIBILITY
...
Root launchers now print bounded console heartbeat lines after the dashboard path. The heartbeat states RUNNING_SAFE_MODE and keeps live_order_ready=false, live_order_allowed=false, can_live_trade=false, and scale_up_allowed=false.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_FULL_SYSTEM_UI_SAFETY_AUDIT.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "retained_archive_semantic_mapping_status": "UNCHANGED_NOT_REQUIRED_FOR_CONSOLE_HEARTBEAT_PATCH",
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_CONSOLE_HEARTBEAT_PATCH",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_CONSOLE_HEARTBEAT_VISIBILITY_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "artifact_paths": [
                ...
                "tools/emit_console_heartbeat_visibility_patch_evidence.py",
                "contracts/generated/context_pack/CONSOLE_HEARTBEAT_VISIBILITY.md",
    ...
    write_text(
        ...
        f"""# MVP4 Console Heartbeat Visibility Audit
...
def main() -> int:
    ...
    artifacts = sorted(set(write_launcher_artifacts() + ["contracts/generated/context_pack/CONSOLE_HEARTBEAT_VISIBILITY.md"]))
    tests_run = [
