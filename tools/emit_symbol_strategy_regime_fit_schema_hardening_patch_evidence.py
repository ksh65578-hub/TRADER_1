...
PATCH_BASENAME = "MVP4_SYMBOL_REGIME_FIT_SCHEMA_HARDENING"
REQUIREMENT_ID = "REQ-MVP4-SYMBOL-REGIME-FIT-SCHEMA-HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tests/validators/test_symbol_strategy_regime_fit_validator.py",
    "tests/validators/fixtures/symbol_strategy_regime_fit_pass.json",
    "tests/validators/fixtures/symbol_strategy_regime_fit_low_liquidity_fail.json",
    "tests/validators/fixtures/symbol_strategy_regime_fit_live_flag_fail.json",
    "tests/validators/fixtures/symbol_strategy_regime_fit_missing_family_fail.json",
    "tools/run_symbol_strategy_regime_fit_validators.py",
    "tools/emit_symbol_strategy_regime_fit_schema_hardening_patch_evidence.py",
    ...
    "contracts/generated/context_pack/SYMBOL_REGIME_FIT_SCHEMA_HARDENING.md",
]
...
def update_gap_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    for gap in audit.get("gaps", []):
        if gap.get("component") == "symbol_selection_regime":
            gap["patch_status"] = "PARTIAL_PATCHED"
            gap["fix"] = (
                "symbol_strategy_regime_fit_report schema and validator now require liquidity, volatility, "
                "spread, depth, regime, strategy family coverage, evidence ids, false live flags, and negative fixtures; "
                "runtime selection performance evidence remains required."
            )
    ...
    actions.append(
        {
            "action_id": PATCH_ID,
            "status": "APPLIED",
            "summary": "Hardened symbol-regime fit schema and validator for liquidity, volatility, spread, depth, and regime evidence.",
    ...
    gap["notes"] = (
        "Candidate scorecards require cost-adjusted net EV, strategy_condition_matrix requires entry/exit/no-trade/regime "
        "traceability, and symbol_strategy_regime_fit_report now requires liquidity/volatility/spread/depth/regime "
        "evidence with fail-closed fixtures. The broader profitability maturity gap remains open until OOS/walk-forward/"
        "bootstrap robustness, execution feedback, convergence memory, and paper/shadow evidence are validated. No live "
        "readiness or scale-up permission is created."
    )
...
def update_registry(now: str) -> None:
    ...
    registry.setdefault("schemas", {})["symbol_strategy_regime_fit_report"] = {
        "schema_id": "trader1.symbol_strategy_regime_fit_report.v1",
        "path": "contracts/schema/symbol_strategy_regime_fit_report.schema.json",
    }
    validators = registry.setdefault("validators", {})
    optimizer_core = validators.setdefault("VALIDATOR_GROUP:OPTIMIZER_CORE", [])
    if "symbol_strategy_regime_fit_validator" not in optimizer_core:
        optimizer_core.append("symbol_strategy_regime_fit_validator")
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") != "symbol_strategy_regime_fit_validator"]
    implemented.append(
        {
            "validator_id": "symbol_strategy_regime_fit_validator",
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_section_id": "SECTION_SYMBOL_SELECTION_REGIME",
            "source_file": "TRADER_1.md",
            "source_heading": "symbol selection must be liquidity, volatility, spread, depth, and regime scoped",
            "full_text_marker": f"{REQUIREMENT_ID}:symbol strategy regime fit requires liquidity volatility spread depth and regime evidence",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Symbol regime fit schema hardening",
            "requirement_kind": "SCHEMA_VALIDATOR_PATCH",
            "schema_ids": ["trader1.symbol_strategy_regime_fit_report.v1"],
            "validator_ids": ["symbol_strategy_regime_fit_validator"],
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/validators/test_symbol_strategy_regime_fit_validator.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": sha256_bytes(
                b"symbol strategy regime fit requires liquidity volatility spread depth and regime evidence"
            ),
    ...
    ensure_matrix_row(
        ...
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_SYMBOL_SELECTION_REGIME",
            "schema_files": ["contracts/schema/symbol_strategy_regime_fit_report.schema.json"],
            "validator_files": ["trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/validators/test_symbol_strategy_regime_fit_validator.py"],
            "fixture_files": [
                "tests/validators/fixtures/symbol_strategy_regime_fit_pass.json",
                "tests/validators/fixtures/symbol_strategy_regime_fit_low_liquidity_fail.json",
                "tests/validators/fixtures/symbol_strategy_regime_fit_live_flag_fail.json",
            ...
            "runtime_modules": ["tools/run_symbol_strategy_regime_fit_validators.py"],
            "evidence_artifacts": [
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "SYMBOL_REGIME_FIT_SCHEMA_HARDENING.md",
        f"""# SYMBOL_REGIME_FIT_SCHEMA_HARDENING
...
Symbol-regime fit evidence now requires liquidity, volatility, spread, depth, regime, and strategy-family coverage with negative fixture coverage. This improves symbol selection and operator review clarity without creating live readiness, live order permission, or scale-up permission.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_STRATEGY_CONDITION_MATRIX_SCHEMA_HARDENING.patch_result.json"
    )
    optimizer_results = [
        result
    ...
    patch_result.update(
        {
            ...
            "merged_requirements": [],
            "new_registry_items": [
                "contracts/registry.yaml::schemas.symbol_strategy_regime_fit_report",
                "contracts/registry.yaml::validators.symbol_strategy_regime_fit_validator",
                "contracts/validators/validator_registry.json::symbol_strategy_regime_fit_validator",
                *CHANGED_ARTIFACTS,
            ],
            "new_or_changed_schema_ids": ["trader1.symbol_strategy_regime_fit_report.v1"],
            ...
            "next_task_class": "MVP4_OOS_ROBUSTNESS_SCHEMA_HARDENING",
            ...
            "expanded_section_ids": [
                "TRADER_1:symbol-selection-regime-active-surface",
            ...
            "optimizer_patch": "SYMBOL_REGIME_FIT_SCHEMA_VALIDATOR_PATCH",
            "optimizer_stage": "MVP4_REVIEW_PREP",
            "optimizer_status_before": "STRATEGY_CONDITION_MATRIX_GUARDED_LIVE_BLOCKED",
            "optimizer_status_after": "SYMBOL_REGIME_FIT_GUARDED_LIVE_BLOCKED",
            "optimizer_maturity_level_before": "MVP4_ENTRY_EXIT_NO_TRADE_REGIME_GUARDED",
            "optimizer_maturity_level_after": "MVP4_SYMBOL_REGIME_FIT_GUARDED",
            "optimizer_output_type": "ANALYSIS_ONLY_SYMBOL_REGIME_FIT_SCHEMA",
            ...
            "profit_convergence_patch": "SYMBOL_REGIME_FIT_GUARD_NO_CONVERGENCE_RUNTIME_CHANGE",
            "convergence_layer_changed": False,
            "convergence_state_before": "STRATEGY_CONDITION_MATRIX_GUARDED_LIVE_BLOCKED",
            "convergence_state_after": "SYMBOL_REGIME_FIT_GUARDED_LIVE_BLOCKED",
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": True,
            "failure_analysis_status": "CONTRACT_GAP_REMAINS_OPEN_FOR_OOS_EXECUTION_AND_MEMORY_EVIDENCE",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_SYMBOL_REGIME_FIT_SCHEMA_HARDENING_LIVE_REVIEW_STILL_BLOCKED",
            "open_contract_gap_id": CONTRACT_GAP_ID,
            "next_allowed_task_class": "MVP4_OOS_ROBUSTNESS_SCHEMA_HARDENING",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Symbol Regime Fit Schema Hardening
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_OOS_ROBUSTNESS_SCHEMA_HARDENING"
    state["completed_requirement_ids"] = sorted(set(state.get("completed_requirement_ids", []) + [REQUIREMENT_ID]))
    state["implemented_schema_ids"] = sorted(
        set(state.get("implemented_schema_ids", []) + ["trader1.symbol_strategy_regime_fit_report.v1"])
    )
    state["implemented_validator_ids"] = sorted(
        set(state.get("implemented_validator_ids", []) + ["symbol_strategy_regime_fit_validator"])
    )
    state["untested_validator_ids"] = [
        item for item in state.get("untested_validator_ids", []) if item != "symbol_strategy_regime_fit_validator"
    ]
