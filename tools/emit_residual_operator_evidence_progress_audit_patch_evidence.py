...
SOURCE_EVIDENCE_ARTIFACTS = [EXECUTION_GUIDE_REPORT_PATH]
EVIDENCE_ARTIFACTS = [
...
def load_execution_guide_report() -> dict[str, Any]:
    report = base.load_json(ROOT / EXECUTION_GUIDE_REPORT_PATH)
    if report.get("schema_id") != "trader1.residual_operator_execution_guide_report.v1":
        raise RuntimeError("residual operator execution guide report schema mismatch")
    if report.get("guide_status") != "BLOCKED_GUIDE_ONLY":
        raise RuntimeError("residual operator execution guide must remain blocked")
    for field in (
        "live_order_ready",
        "live_order_allowed",
        "can_live_trade",
        ...
        if report.get(field) is not False:
            raise RuntimeError(f"residual execution guide has non-false {field}")
    return report
...
def build_report(
    ...
    execution_guide_report = load_execution_guide_report()
    report = build_residual_operator_evidence_progress_report(
    ...
    errors = validate_residual_operator_evidence_progress_report(report, execution_guide_report, state)
    if errors:
        raise RuntimeError("residual operator evidence progress audit failed: " + "; ".join(errors))
...
def write_evidence(
    ...
    base.assert_false_flags("evidence progress report", report, "")
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_RESIDUAL_OPERATOR_EVIDENCE_PROGRESS_AUDIT_LIVE_BLOCKED",
            "open_gap_count": report["open_gap_count"],
            "evidence_item_count": report["evidence_item_count"],
            "local_runtime_command_count": report["local_runtime_command_count"],
            "local_runtime_completed_count": report["local_runtime_completed_count"],
