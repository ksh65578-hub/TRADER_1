...
from trader1.runtime.paper.upbit_paper_repaired_current_evidence_audited_writer_dry_run import (  # noqa: E402
...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-REPAIRED-CURRENT-EVIDENCE-AUDITED-WRITER-DRY-RUN-DASHBOARD-BINDING"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT_SCAFFOLD"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_repaired_current_evidence_audited_writer_dry_run_dashboard_binding_patch_evidence.py",
    f"contracts/generated/context_pack/{PATCH_BASENAME}.md",
    "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.json",
...
def write_runtime_dry_run_report() -> dict[str, Any]:
    report = build_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(
        root=ROOT,
        source_audited_writer_design_report=load_json(DESIGN_PATH),
        audited_writer_dry_run_id="upbit-paper-repaired-current-evidence-audited-writer-dry-run-dashboard-binding-20260502",
    )
    result = validate_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(report)
    if result.status != "PASS":
        raise RuntimeError(f"audited writer dry-run validation failed: {result.status} {result.blocker_code} {result.message}")
    write_upbit_paper_repaired_current_evidence_audited_writer_dry_run_report(root=ROOT, report=report)
    return report
...
def validate_dashboard_projection(
    ...
    if not all(isinstance(item, dict) for item in (reconciliation, portfolio, operator_action, workflow)):
        raise RuntimeError("dashboard missing audited writer dry-run summaries")
    if reconciliation.get("source") != "upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.json":
        raise RuntimeError("dashboard did not prefer audited writer dry-run source")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_dry_run_status")
        != AUDITED_WRITER_DRY_RUN_STATUS
        ...
        raise RuntimeError("dashboard did not expose audited writer dry-run blocker")
    expected_counts = {
        "check_count": report["dry_run_check_count"],
        "check_pass_count": report["dry_run_check_pass_count"],
        "check_blocked_count": report["dry_run_check_blocked_count"],
        "configured_initial_cash_krw": report["current_evidence_snapshot_preview"]["configured_initial_cash_krw"],
    }
    actual_counts = {
        "check_count": reconciliation["upbit_paper_repaired_current_evidence_audited_writer_dry_run_check_count"],
        ...
        "configured_initial_cash_krw": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_dry_run_configured_initial_cash_krw"
        ],
    }
    if actual_counts != expected_counts:
        raise RuntimeError(f"dashboard audited writer dry-run counts drifted: {actual_counts}")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_dry_run_cash_status")
        != "UNVERIFIED"
        or reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_dry_run_portfolio_source_status")
        ...
        raise RuntimeError("dashboard attempted to promote dry-run preview into portfolio truth")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    source = source_ids.get("UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN")
    if not isinstance(source, dict) or source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish audited writer dry-run source artifact as PASS")
    if portfolio.get("status") != "UNVERIFIED" or portfolio.get("source_snapshot_status") != "BLOCKED":
        raise RuntimeError("portfolio was incorrectly promoted by audited writer dry-run display")
    if portfolio.get("blocking_reason") != AUDITED_CURRENT_EVIDENCE_WRITER_NOT_IMPLEMENTED_BLOCKER_CODE:
        raise RuntimeError("portfolio did not expose audited writer dry-run blocker")
    if operator_action.get("primary_action_label") != "Inspect audited writer dry-run":
        raise RuntimeError("operator action did not point to audited writer dry-run")
    if "audited current-evidence writer dry-run is review-only" not in str(operator_action.get("one_line_blocker")):
        raise RuntimeError("operator action did not explain audited writer dry-run blocker")
    if "Audited current-evidence writer dry-run is review-only" not in str(workflow.get("summary")):
        raise RuntimeError("workflow did not explain audited writer dry-run blocker")
...
- dry_run_check_pass_count: {report["dry_run_check_pass_count"]}
- dry_run_check_blocked_count: {report["dry_run_check_blocked_count"]}
- configured_initial_cash_krw: {report["current_evidence_snapshot_preview"]["configured_initial_cash_krw"]}
- cash_status: {report["current_evidence_snapshot_preview"]["cash_status"]}
...
The read-only dashboard now surfaces the Upbit PAPER repaired current-evidence audited writer dry-run. It shows the configured 1,000,000 KRW PAPER capital as config-only and unverified while the audited writer, current-evidence writes, portfolio truth writes, live orders, and scale-up remain blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER audited writer dry-run dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: dashboard must surface the audited writer dry-run preview while current evidence, "
                "portfolio truth, live orders, and scale-up remain blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER audited writer dry-run dashboard binding",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"upbit paper repaired current evidence audited writer dry-run dashboard binding no writes no live no scale"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_DASHBOARD_BOUND_DRY_RUN_ONLY_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": [
                "contracts/schema/read_only_dashboard_shell.schema.json",
                "contracts/schema/upbit_paper_repaired_current_evidence_audited_writer_dry_run_report.schema.json",
            ],
            ...
            "fixture_files": [base.rel(DRY_RUN_PATH)],
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_DASHBOARD_BOUND_DRY_RUN_ONLY_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(
    ...
    patch_result.update(
        {
            ...
            "next_optional_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_OPERATOR_REVIEW"],
            ...
            "optimizer_status_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_DASHBOARD_BOUND",
            "optimizer_guardrail_result": "PASS_DASHBOARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_DRY_RUN_DASHBOARD_BOUND_LIVE_BLOCKED",
    ...
    if dashboard.get("live_order_allowed") is not False or dashboard.get("scale_up_allowed") is not False:
        raise RuntimeError("patch result refused dashboard live or scale permission")
    if not dashboard_paths:
        raise RuntimeError("dashboard binding patch requires dashboard artifacts")
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_AUDITED_WRITER_DRY_RUN_DASHBOARD_BOUND_WRITER_DISABLED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            "dry_run_status": report["dry_run_status"],
            "primary_blocker_code": report["primary_blocker_code"],
            "dashboard_artifact_paths": dashboard_paths,
...
def main() -> int:
    ...
    report = write_runtime_dry_run_report()
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, dashboard, dashboard_paths)
