...
PATCH_BASENAME = "MVP4_SECRET_PATTERN_HYGIENE_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-SECRET-PATTERN-HYGIENE-RECHECK"
NEXT_TASK_CLASS = "MVP4_LIVE_BLOCKED_NEGATIVE_RECHECK"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_secret_pattern_hygiene_recheck_patch_evidence.py",
    "contracts/generated/context_pack/SECRET_PATTERN_HYGIENE_RECHECK.md",
]
BLOCKERS = [
    "LIVE_READY_MISSING",
    "API_UNVERIFIED",
...
def secret_pattern_audit(manifest: dict[str, Any]) -> dict[str, Any]:
    status = "PASS"
    blockers: list[str] = []
    ...
    return {
        "audit_schema_id": "trader1.secret_pattern_hygiene_recheck_audit.v1",
        "status": status,
        "blockers": blockers,
        ...
        "covered_secret_shapes": [
            "AWS_STYLE_SECRET_ACCESS_KEY_ASSIGNMENT",
        ...
        "next_action": (
            "Continue secret hygiene monitoring."
            if status == "PASS"
            else "Block source bundle release and remove credential-like material."
        ),
    }
...
def update_context(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "SECRET_PATTERN_HYGIENE_RECHECK.md",
        f"""# SECRET_PATTERN_HYGIENE_RECHECK
...
- covered_secret_shapes: {json.dumps(audit["covered_secret_shapes"])}
...
Secret hygiene scan now covers AWS-style environment keys, Authorization Bearer headers, JWT-like literals, AWS AKIA keys, and private key blocks. Current bundle contains no credential-like findings.
...
def update_requirement_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    requirements.append(
        {
            ...
            "source_section_id": "SECTION_SECRET_HYGIENE",
            "source_file": "TRADER_1.md",
            "source_heading": "Secret pattern hygiene recheck",
            "full_text_marker": f"{REQUIREMENT_ID}:secret scanner must detect common runtime credential shapes",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Secret scanner detects common runtime credential shapes",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_SECRET_HYGIENE", "SECTION_SOURCE_BUNDLE_HYGIENE", "SECTION_LIVE_FINAL_GUARD"],
            "depends_on": ["REQ-MVP4-SOURCE-BUNDLE-HYGIENE-RECHECK", "REQ-MVP0-BUNDLE-SECURITY"],
            "source_text_sha256": sha256_bytes(b"secret scanner must detect common runtime credential shapes"),
    ...
    rows.append(
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_SECRET_HYGIENE",
            "schema_files": [],
...
def build_patch_result(
    ...
    patch_result: dict[str, Any] = {
        ...
        "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-SOURCE-BUNDLE-HYGIENE-RECHECK", "REQ-MVP0-BUNDLE-SECURITY", "REQ-MVP4-LIVE-FINAL-GUARD"],
        ...
        "next_required_section_ids": ["SECTION_LIVE_BLOCKED_TEST", "SECTION_LIVE_FINAL_GUARD", "SECTION_ORDER_PATH_GUARD"],
        ...
        "active_read_surface_used": ["SECTION_SECRET_HYGIENE", "SECTION_SOURCE_BUNDLE_HYGIENE", "SECTION_LIVE_FINAL_GUARD"],
        "task_class": "MVP4_SECRET_PATTERN_HYGIENE_RECHECK",
        "required_section_ids": ["SECTION_SECRET_HYGIENE", "SECTION_SOURCE_BUNDLE_HYGIENE", "SECTION_LIVE_FINAL_GUARD"],
        "expanded_section_ids": ["TRADER_1:secret-hygiene-active-surface", "AGENTS:bundle-security-implementation-guide"],
        ...
        "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_SECRET_PATTERN_HYGIENE",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], audit: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_SECRET_PATTERN_HYGIENE_RECHECK_NO_LIVE_ORDERS",
            "secret_pattern_audit": audit,
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Secret Pattern Hygiene Recheck
...
- excluded_count: {audit['excluded_count']}
- secret_findings_count: {audit['secret_findings_count']}
- covered_secret_shapes: {', '.join(audit['covered_secret_shapes'])}
...
def main() -> int:
    ...
    audit = secret_pattern_audit(manifest)
    ...
    if audit["status"] != "PASS":
        failed.append({"status": "FAIL", "reason": "secret pattern audit failed"})
    print(
        json.dumps(
            {
                "patch_id": PATCH_ID,
                ...
                "excluded_count": audit["excluded_count"],
                "secret_findings_count": audit["secret_findings_count"],
                "covered_secret_shapes": audit["covered_secret_shapes"],
