...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT_DASHBOARD_BINDING"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-REPAIRED-CURRENT-EVIDENCE-AUDITED-WRITER-LOCKED-OUTPUT-DASHBOARD-BINDING"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_IMPLEMENTATION_PREP"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_repaired_current_evidence_audited_writer_locked_output_dashboard_binding_patch_evidence.py",
...
def write_runtime_locked_output_report() -> dict[str, Any]:
    report = build_upbit_paper_repaired_current_evidence_audited_writer_locked_output_report(
        ...
        audited_writer_locked_output_id="upbit-paper-repaired-current-evidence-audited-writer-locked-output-dashboard-binding-20260502",
    )
...
def validate_dashboard_projection(dashboard: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    if not all(isinstance(item, dict) for item in (reconciliation, portfolio, operator_action, workflow)):
        raise RuntimeError("dashboard missing audited writer locked-output summaries")
    if reconciliation.get("source") != "upbit_paper_repaired_current_evidence_audited_writer_locked_output_report.json":
        raise RuntimeError("dashboard did not prefer audited writer locked-output source")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_locked_output_status")
        != AUDITED_WRITER_LOCKED_OUTPUT_STATUS
        or reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_locked_output_validation_status")
        != "PASS"
        or reconciliation.get("primary_blocker_code") != AUDITED_CURRENT_EVIDENCE_WRITER_NOT_IMPLEMENTED_BLOCKER_CODE
    ):
        raise RuntimeError("dashboard did not expose audited writer locked-output blocker")
    expected = {
        "control_count": report["locked_output_control_count"],
        "control_pass_count": report["locked_output_control_pass_count"],
        "control_blocked_count": report["locked_output_control_blocked_count"],
        "configured_initial_cash_krw": report["current_evidence_locked_payload"]["source_preview"][
            "configured_initial_cash_krw"
        ],
    }
    actual = {
        "control_count": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_locked_output_control_count"
        ],
        "control_pass_count": reconciliation[
            "upbit_paper_repaired_current_evidence_audited_writer_locked_output_control_pass_count"
        ],
        "control_blocked_count": reconciliation[
    ...
    if actual != expected:
        raise RuntimeError(f"dashboard audited writer locked-output counts drifted: {actual}")
    if (
        reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_locked_output_cash_status")
        != "UNVERIFIED"
        ...
        is not False
        or reconciliation.get("upbit_paper_repaired_current_evidence_audited_writer_locked_output_lock_file_written")
        is not False
        ...
        raise RuntimeError("dashboard attempted to promote locked-output preview into portfolio truth")
    source_ids = {source.get("artifact_id"): source for source in dashboard.get("source_artifacts", [])}
    source = source_ids.get("UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT")
    if not isinstance(source, dict) or source.get("freshness_status") != "PASS":
        raise RuntimeError("dashboard did not publish audited writer locked-output source artifact as PASS")
    if portfolio.get("status") != "UNVERIFIED" or portfolio.get("source_snapshot_status") != "BLOCKED":
        raise RuntimeError("portfolio was incorrectly promoted by audited writer locked-output display")
    if operator_action.get("primary_action_label") != "Inspect audited writer locked output":
        raise RuntimeError("operator action did not point to audited writer locked output")
    if "Audited current-evidence writer locked output is review-only" not in str(workflow.get("summary")):
        raise RuntimeError("workflow did not explain audited writer locked-output blocker")
...
task_class: MVP4_UPBIT_PAPER_REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT_DASHBOARD_BINDING
...
included_schema_ids: ["trader1.read_only_dashboard_shell.v1", "trader1.upbit_paper_repaired_current_evidence_audited_writer_locked_output_report.v1"]
included_validator_ids: {json.dumps(VALIDATORS_REQUIRED)}
included_artifact_ids: {json.dumps(CHANGED_ARTIFACTS)}

acceptance_checklist:
- The read-only dashboard loads the audited writer locked-output source.
- The first-screen operator action names the locked-output review-only blocker.
- The ledger panel shows locked control counts, configured 1,000,000 KRW as unverified, false lock flags, and false writer/write flags.
...
The read-only dashboard now surfaces the Upbit PAPER audited writer locked-output scaffold. It shows fixed future targets and configured 1,000,000 KRW as unverified review-only evidence while current-evidence writes, portfolio truth writes, lock writes, live orders, and scale-up remain blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER audited writer locked-output dashboard binding",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: dashboard must surface the audited writer locked-output scaffold while "
                "current evidence, portfolio truth, locks, live orders, and scale-up remain blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER audited writer locked-output dashboard binding",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"upbit paper repaired current evidence audited writer locked-output dashboard binding no writes no locks no live no scale"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_DASHBOARD_BOUND_LOCKED_OUTPUT_ONLY_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_DASHBOARD_BOUND_LOCKED_OUTPUT_ONLY_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
    ...
    patch_result.update(
        {
            ...
            "optimizer_status_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT_DASHBOARD_BOUND",
            "optimizer_guardrail_result": "PASS_DASHBOARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "REPAIRED_CURRENT_EVIDENCE_AUDITED_WRITER_LOCKED_OUTPUT_DASHBOARD_BOUND_LIVE_BLOCKED",
...
def write_evidence(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_AUDITED_WRITER_LOCKED_OUTPUT_DASHBOARD_BOUND_WRITER_DISABLED",
            "dashboard_blocking_reason": dashboard["blocking_reason"],
            "locked_output_status": report["locked_output_status"],
            "primary_blocker_code": report["primary_blocker_code"],
