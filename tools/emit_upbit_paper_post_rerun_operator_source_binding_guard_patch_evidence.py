...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-OPERATOR-SOURCE-BINDING-GUARD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_RECONCILIATION_DECISION_AUDIT_EVIDENCE"

if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

import tools.emit_persistent_runtime_resource_boundary_patch_evidence as base  # noqa: E402
import tools.emit_upbit_paper_post_rerun_operator_reconciliation_queue_patch_evidence as previous  # noqa: E402
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_upbit_paper_post_rerun_operator_source_binding_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD.md",
]

BLOCKERS = [
    "POST_RERUN_RECONCILIATION_REQUIRED",
    "POST_RERUN_OPERATOR_SOURCE_PROMOTION_GUARD_BINDING_REQUIRED",
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD

context_pack_id: MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD
task_class: MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_SOURCE_BINDING_GUARD
...
- The operator reconciliation queue reloads the source promotion guard artifact from disk.
- The source artifact's stored promotion_guard_hash and recomputed hash must match the in-memory source hash.
- Missing, invalid, or hash-mismatched source promotion guard files block the queue.
- Current evidence writes, live permission, promotion, long-run evidence, and scale-up stay false.

known_omissions_by_design:
- This patch is not a reconciliation decision or current-evidence writer.
...
- source_file_load_status: {report["source_promotion_guard_file_load_status"]}
- source_file_hash_match: {str(report["source_promotion_guard_file_hash_match"]).lower()}
...
Upbit PAPER post-rerun operator reconciliation queue evidence is now bound to the persisted promotion guard artifact hash. Missing or mismatched source files block the queue while all live and scale-up flags remain false.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun operator source binding guard",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun operator reconciliation queue must bind source promotion guard "
                "evidence to the persisted source artifact hash before review items are accepted"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun operator source binding guard",
            ...
            "depends_on": [
                ...
                "REQ-MVP4-UPBIT-PAPER-POST-RERUN-SOURCE-RECONCILIATION-BINDING-GUARD",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(b"post rerun operator queue source promotion guard file hash binding"),
    ...
    rows.append(
        {
            ...
            "patch_result_fields": [
                ...
                "post_rerun_operator_source_promotion_guard_file_load_status",
                "post_rerun_operator_source_promotion_guard_file_hash_match",
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "persisted promotion guard artifact",
                "live final guard",
            ],
            ...
            "post_rerun_operator_source_promotion_guard_file_load_status": report["source_promotion_guard_file_load_status"],
            "post_rerun_operator_source_promotion_guard_file_hash_match": report["source_promotion_guard_file_hash_match"],
            "optimizer_guardrail_result": "PASS_OPERATOR_QUEUE_SOURCE_PROMOTION_GUARD_ARTIFACT_HASH_BOUND",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Operator Source Binding Guard Audit
...
- queue_status: {report["queue_status"]}
- source_file_load_status: {report["source_promotion_guard_file_load_status"]}
- source_file_hash_match: {report["source_promotion_guard_file_hash_match"]}
- current_evidence_write_allowed_count: {report["current_evidence_write_allowed_count"]}
    ...
    tests_run = [
        base.run_command(
            [
                ...
                "-m",
                "unittest",
                "tests.runtime.test_upbit_paper_post_rerun_operator_reconciliation_queue",
