...
PATCH_BASENAME = "MVP4_UPBIT_PUBLIC_REST_CONTINUITY_HISTORY"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PUBLIC-REST-CONTINUITY-HISTORY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/update_upbit_public_rest_continuity_history.py",
    "tools/emit_upbit_public_rest_continuity_history_patch.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PUBLIC_REST_CONTINUITY_HISTORY.md",
]
...
def update_validator_registry(now: str) -> None:
    ...
    registry["implemented_logic_status"] = "MVP4_UPBIT_PUBLIC_REST_CONTINUITY_HISTORY_VALIDATOR_ADDED"
    entry = {
        "validator_id": "upbit_public_rest_continuity_history_validator",
        "module_path": "trader1.validation.mvp0_validators",
        "status": "IMPLEMENTED_FAIL_CLOSED",
        "live_enabling": False,
    }
    registry["implemented_validators"] = [
        item for item in registry.get("implemented_validators", []) if item.get("validator_id") != entry["validator_id"]
    ]
    registry["implemented_validators"].append(entry)
    registry["implemented_validators"] = sorted(registry["implemented_validators"], key=lambda item: item["validator_id"])
...
def upsert_requirement_index(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    entry = {
        ...
        "source_section_id": "SECTION_LONG_RUN_STABILITY",
        "source_file": "TRADER_1.md",
        "source_heading": "Upbit PAPER public REST continuity history boundary",
        "full_text_marker": f"{REQUIREMENT_ID}:SECTION_LONG_RUN_STABILITY:public-rest-continuity-history-paper-only-not-live-ready",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "Public REST continuity attempts must accumulate as PAPER-only history before long-run evidence can be reviewed",
        "requirement_kind": "RUNTIME_DATA_CONTINUITY_HISTORY_PATCH",
        "schema_ids": ["trader1.upbit_public_rest_continuity_history.v1"],
        "validator_ids": ["upbit_public_rest_continuity_history_validator"],
        "artifact_ids": CHANGED_ARTIFACTS,
        "test_ids": ["tests/integration/test_upbit_public_rest_continuity_history.py"],
        "mvp_stage": "MVP-4",
        "implementation_depth_min": "DEPTH_4_RUNTIME_INTEGRATION",
        "blocking_level": "PAPER_RUNTIME_BLOCKING",
        "live_affecting": True,
        "read_when": ["SECTION_MARKET_DATA", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LONG_RUN_STABILITY", "SECTION_LIVE_FINAL_GUARD"],
        "depends_on": [
            "REQ-MVP4-UPBIT-PUBLIC-REST-CONTINUITY-EVIDENCE",
            "REQ-MVP4-UPBIT-PUBLIC-REST-SAMPLE-EVIDENCE",
        ],
        "source_text_sha256": sha256_json({"requirement_id": REQUIREMENT_ID, "title": "public rest continuity history boundary"}),
...
def upsert_requirement_artifact_matrix(now: str) -> None:
    ...
    row = {
        "requirement_id": REQUIREMENT_ID,
        "section_id": "SECTION_LONG_RUN_STABILITY",
        "schema_files": ["contracts/schema/upbit_public_rest_continuity_history.schema.json"],
        "validator_files": ["trader1/validation/mvp0_validators.py"],
        "test_files": ["tests/integration/test_upbit_public_rest_continuity_history.py"],
        "fixture_files": [],
        "runtime_modules": [
            "trader1/runtime/paper/upbit_public_rest_continuity_history.py",
        ...
        "dashboard_artifacts": ["system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/market_data/public/rest_continuity_history.json"],
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PUBLIC_REST_CONTINUITY_HISTORY.md",
        f"""# MVP4_UPBIT_PUBLIC_REST_CONTINUITY_HISTORY
...
Upbit public REST continuity now has a PAPER-only accumulated history. It preserves attempt hashes, blocks stale latest attempts, quarantines corrupt previous history, and cannot create live readiness or scale-up permission.
""",
    )
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    for key, value in {
        ...
        "implemented_schema_ids": "trader1.upbit_public_rest_continuity_history.v1",
        "implemented_validator_ids": "upbit_public_rest_continuity_history_validator",
    }.items():
        values = state.setdefault(key, [])
        if value not in values:
            values.append(value)
            values.sort()
    state["untested_validator_ids"] = [item for item in state.get("untested_validator_ids", []) if item != "upbit_public_rest_continuity_history_validator"]
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_FOR_PUBLIC_REST_CONTINUITY_HISTORY_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    artifact_paths = [
        *CHANGED_ARTIFACTS,
        "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/market_data/public/rest_continuity_history.json",
    ...
    write_text(
        ...
        f"""# MVP4 Upbit Public REST Continuity History Audit
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PUBLIC_REST_CONTINUITY_EVIDENCE.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-UPBIT-PUBLIC-REST-CONTINUITY-EVIDENCE"],
            ...
            "new_registry_items": [REQUIREMENT_ID, "upbit_public_rest_continuity_history_validator"],
            "new_or_changed_schema_ids": ["trader1.upbit_public_rest_continuity_history.v1"],
            ...
            "active_read_surface_used": ["SECTION_MARKET_DATA", "SECTION_UPBIT_PAPER_RUNTIME", "SECTION_LONG_RUN_STABILITY", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": PATCH_BASENAME,
            "required_section_ids": ["SECTION_MARKET_DATA", "SECTION_LONG_RUN_STABILITY", "SECTION_LIVE_FINAL_GUARD"],
            ...
            "profit_convergence_patch": "PUBLIC_REST_CONTINUITY_HISTORY_BOUNDARY_NOT_LIVE_READY",
            "convergence_layer_changed": False,
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": False,
            "failure_analysis_status": "NOT_REQUIRED_FOR_PUBLIC_REST_CONTINUITY_HISTORY_PATCH",
...
def main() -> int:
    ...
    tests_run = [
        run_command([sys.executable, "-m", "unittest", "tests.integration.test_upbit_public_rest_continuity_history", "-v"], timeout_seconds=120),
        run_command([sys.executable, "tools/update_upbit_public_rest_continuity_history.py", "--no-network"], timeout_seconds=120),
        run_command([sys.executable, "tools/update_upbit_public_rest_continuity_history.py", "--timeout-seconds", "2.5"], timeout_seconds=120),
    ]
