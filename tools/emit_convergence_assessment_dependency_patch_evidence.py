...
PATCH_ID = "MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES_20260429_001"
PATCH_RESULT_REL = "system/evidence/patch_results/MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES.patch_result.json"
DEPENDENCY_IDS = [
    "overfit_diagnostic_validator",
    "execution_feedback_loop_validator",
    "model_drift_validator",
]
SUPPORT_IDS = ["coverage_index_validator"]
GUARDRAIL_IDS = [
...
def update_validator_registry(now: str) -> None:
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") not in DEPENDENCY_IDS]
    for validator_id in DEPENDENCY_IDS:
        implemented.append(
            {
    ...
    fixture_paths = {
        "convergence_assessment_dependencies_schema_only_pass": "tests/validators/fixtures/convergence_assessment_dependencies_pass.json",
        "convergence_assessment_dependencies_live_permission_fail": "tests/validators/fixtures/convergence_assessment_dependencies_fail.json",
        "convergence_assessment_dependencies_blocked_before_evidence": "tests/validators/fixtures/convergence_assessment_dependencies_blocked.json",
    }
    catalog["fixtures"] = [
...
def write_validator_log(
    ...
    path = ROOT / "system" / "evidence" / "validator_runs" / "MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES.validator_run_log.json"
    write_json(
        path,
        {
            ...
            "created_at_utc": now,
            "patch_id": PATCH_ID,
            "validators_run": dependency_results + support_results,
            "optimizer_convergence_guardrail_results": guardrail_results,
            "fixture_results": fixture_results,
            "validators_blocked_by_missing_dependencies": RISK_UNTESTED_IDS,
...
def write_stage_gate(now: str, validator_log_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "stage_gates" / "MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES.stage_gate_result.json"
    write_json(
        path,
        {
            ...
            "stage_gate_status": "CONVERGENCE_ASSESSMENT_PASS_LIVE_AND_SCALE_UP_REMAIN_BLOCKED",
...
def write_evidence_manifest(now: str, authority: dict[str, str], validator_log_path: Path, stage_gate_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES.evidence_manifest.json"
    write_json(
        path,
        {
            ...
            "project_id": "TRADER_1",
            "authority": authority,
            "evidence_manifest_id": "MVP4_CONVERGENCE_ASSESSMENT_DEPENDENCIES_EVIDENCE",
            "artifact_paths": [
                "trader1/validation/mvp0_validators.py",
                "tools/run_convergence_assessment_dependency_validators.py",
                "tools/run_optimizer_convergence_guardrail_validators.py",
                "tests/validators/test_convergence_assessment_dependency_validators.py",
                "tests/validators/fixtures/convergence_assessment_dependencies_pass.json",
                "tests/validators/fixtures/convergence_assessment_dependencies_fail.json",
                "tests/validators/fixtures/convergence_assessment_dependencies_blocked.json",
...
def build_patch_result(
    ...
    convergence_results = dependency_results + support_results + [
        result
    ...
    patch_result = {
        ...
        "affected_contract_ids": ["REQ-CONV-MVP0-SCAFFOLD", "REQ-CONV-ASSESSMENT-DEPENDENCIES"],
        ...
        "new_registry_items": ["convergence assessment dependency validators marked IMPLEMENTED_FAIL_CLOSED"],
        "new_or_changed_schema_ids": [],
        "validators_required": DEPENDENCY_IDS + SUPPORT_IDS + GUARDRAIL_IDS,
        "validators_run": dependency_results + support_results + guardrail_results,
        "tests_run": test_results,
        "coverage_unmapped_count": 0,
        "registry_yaml_parse_status": "PASS",
        "registry_placeholders_remaining": [],
        "retained_archive_semantic_mapping_status": "NOT_READ_NOT_REQUIRED_FOR_CONVERGENCE_ASSESSMENT_DEPENDENCY_PATCH",
        ...
        "expanded_section_ids": ["TRADER_1:1952-1986", "AGENTS:730-773"],
        ...
        "profit_convergence_patch": "FAIL_CLOSED_ASSESSMENT_DEPENDENCY_VALIDATOR_PATCH",
        "convergence_layer_changed": False,
        "convergence_state_before": "BLOCKED_FAIL_CLOSED_REMAINING_DEPENDENCY_UNTESTED",
        "convergence_state_after": "ASSESSMENT_GUARDRAIL_PASS_LIVE_STILL_BLOCKED",
        "objective_profile_changed": False,
        "memory_schema_changed": False,
        "failure_analysis_required": False,
        "failure_analysis_status": "NOT_REQUIRED_FOR_CONVERGENCE_ASSESSMENT_DEPENDENCY_PATCH",
        "exploration_exploitation_policy_changed": False,
        "regime_adaptation_changed": False,
        "risk_scaling_policy_changed": False,
        "survival_layer_changed": False,
        "convergence_validators_required": DEPENDENCY_IDS
        + SUPPORT_IDS
...
def write_patch_and_state(now: str, patch_result: dict[str, Any]) -> None:
    ...
    for validator_id in DEPENDENCY_IDS + SUPPORT_IDS + GUARDRAIL_IDS + ["live_final_guard_validator"]:
        if validator_id not in state["implemented_validator_ids"]:
            state["implemented_validator_ids"].append(validator_id)
    state["untested_validator_ids"] = RISK_UNTESTED_IDS
...
def main() -> int:
    ...
    fixture_paths = [
        ROOT / "tests" / "validators" / "fixtures" / "convergence_assessment_dependencies_pass.json",
        ROOT / "tests" / "validators" / "fixtures" / "convergence_assessment_dependencies_fail.json",
        ROOT / "tests" / "validators" / "fixtures" / "convergence_assessment_dependencies_blocked.json",
    ]
    dependency_results = run_validators(DEPENDENCY_IDS)
    support_results = run_validators(SUPPORT_IDS)
    guardrail_results = run_validators(GUARDRAIL_IDS)
    fixture_results = [run_fixture_file(path) for path in fixture_paths]
    validator_log_path = write_validator_log(now, dependency_results, support_results, guardrail_results, fixture_results)
    ...
    preliminary = build_patch_result(
        now, dependency_results, support_results, guardrail_results, [], evidence_path, validator_log_path, stage_gate_path
    )
    ...
    test_results = [
        ...
        run_command([sys.executable, "tools/run_convergence_assessment_dependency_validators.py"], "python tools/run_convergence_assessment_dependency_validators.py"),
