...
PATCH_BASENAME = "MVP4_OPTIMIZER_MEMORY_STATE_SCHEMA_HARDENING"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-OPTIMIZER-MEMORY-STATE-SCHEMA-HARDENING"
...
FIXTURE_FILES = [
    "tests/validators/fixtures/optimizer_memory_state_pass.json",
    "tests/validators/fixtures/optimizer_memory_state_live_flag_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_reset_without_audit_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_forget_failed_candidate_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_cross_scope_reuse_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_failed_candidate_unblocked_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_append_without_hash_fail.json",
    "tests/validators/fixtures/optimizer_memory_state_live_source_mode_fail.json",
]
...
CHANGED_ARTIFACTS = [
    ...
    "tests/validators/test_optimizer_memory_state_validator.py",
    *FIXTURE_FILES,
    "tools/run_optimizer_memory_state_validators.py",
    "tools/emit_optimizer_memory_state_schema_hardening_patch_evidence.py",
    "contracts/generated/context_pack/OPTIMIZER_MEMORY_STATE_SCHEMA.md",
    "system/evidence/audit_reports/MVP4_OPTIMIZER_MEMORY_STATE_SCHEMA_HARDENING_20260429.md",
]
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "OPTIMIZER_MEMORY_STATE_SCHEMA.md",
        f"""# OPTIMIZER_MEMORY_STATE_SCHEMA
...
Optimizer memory now requires append-audit-only write policy, hash-linked sequencing, explicit namespace scope, failed/blocked/retired candidate retention, and false fields for memory reset without audit, failed-candidate forgetting, cross-scope reuse, live config mutation, exchange account calls, order submission, and scale-up.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "Optimizer memory state schema",
            "full_text_marker": f"{REQUIREMENT_ID}:SECTION_CONVERGENCE_MEMORY:Optimizer memory must be append-auditable and cannot forget failures",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Harden optimizer memory against unsafe reset, forgetting, and cross-scope reuse",
            "requirement_kind": "OPTIMIZER_CONVERGENCE_GUARDRAIL",
            "schema_ids": ["trader1.optimizer_memory_state.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/validators/test_optimizer_memory_state_validator.py"],
            "mvp_stage": "MVP-4",
            ...
            "depends_on": [
                "REQ-MVP4-OPTIMIZATION-STATE-SCHEMA-HARDENING",
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/optimizer_memory_state.schema.json"],
            "validator_files": ["trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/validators/test_optimizer_memory_state_validator.py"],
    ...
    patch_result.update(
        {
            ...
            "new_or_changed_schema_ids": ["trader1.optimizer_memory_state.v1"],
            ...
            "next_task_class": "MVP4_STRATEGY_PERFORMANCE_MEMORY_SCHEMA_RECHECK",
            "next_required_section_ids": ["SECTION_CONVERGENCE_MEMORY", "SECTION_CONVERGENCE_GUARDRAIL", "SECTION_LIVE_FINAL_GUARD"],
            "next_optional_section_ids": ["SECTION_FAILURE_ANALYSIS", "SECTION_DASHBOARD_SHELL"],
            ...
            "active_read_surface_used": ["SECTION_CONVERGENCE_MEMORY", "SECTION_OPTIMIZER_GUARDRAIL", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "MVP4_OPTIMIZER_MEMORY_STATE_SCHEMA_HARDENING",
            "required_section_ids": ["SECTION_CONVERGENCE_MEMORY", "SECTION_OPTIMIZER_GUARDRAIL", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": [
                "TRADER_1:convergence-memory-active-surface",
                "TRADER_1:optimizer-memory-live-mutation-ban",
                "AGENTS:optimizer-memory-negative-fixtures",
            ],
            ...
            "optimizer_patch": "OPTIMIZER_MEMORY_STATE_SCHEMA_HARDENING",
            "optimizer_stage": "MVP4_ANALYSIS_ONLY",
            "optimizer_status_before": "MEMORY_SCHEMA_SCAFFOLD_ONLY",
            "optimizer_status_after": "STRICT_APPEND_AUDIT_MEMORY_VALIDATED",
            "optimizer_maturity_level_before": "MVP4_SCAFFOLD",
            "optimizer_maturity_level_after": "MVP4_NEGATIVE_FIXTURE_VALIDATED",
            "optimizer_output_type": "APPEND_AUDIT_MEMORY_ONLY",
            "optimizer_guardrail_result": "PASS_NO_LIVE_PERMISSION",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "OPTIMIZER_MEMORY_APPEND_AUDIT_HARDENING",
            "convergence_layer_changed": True,
            "convergence_state_before": "MEMORY_SCHEMA_SCAFFOLD_ONLY",
            "convergence_state_after": "MEMORY_APPEND_AUDIT_VALIDATED_NO_LIVE_PERMISSION",
            "objective_profile_changed": False,
            "memory_schema_changed": True,
            "failure_analysis_required": True,
            "failure_analysis_status": "REQUIRED_FOR_FAILED_CANDIDATES_AND_PRESERVED_IN_MEMORY",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_OPTIMIZER_MEMORY_STATE_SCHEMA_HARDENING_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Optimizer Memory State Schema Hardening Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["implemented_schema_ids"] = sorted(set(state.get("implemented_schema_ids", []) + ["trader1.optimizer_memory_state.v1"]))
    state["implemented_validator_ids"] = sorted(set(state.get("implemented_validator_ids", []) + ["optimizer_memory_state_validator"]))
    state["untested_validator_ids"] = [item for item in state.get("untested_validator_ids", []) if item != "optimizer_memory_state_validator"]
    state["last_patch_id"] = PATCH_ID
    state["last_patch_result_hash"] = patch_result["result_hash"]
    state["next_allowed_task_class"] = "MVP4_STRATEGY_PERFORMANCE_MEMORY_SCHEMA_RECHECK"
...
def main() -> int:
    ...
    tests_run = [
        ...
        run_command([sys.executable, "-m", "unittest", "tests.validators.test_optimization_state_validator", "-v"]),
        ...
        run_command([sys.executable, "tools/run_optimization_state_validators.py"]),
