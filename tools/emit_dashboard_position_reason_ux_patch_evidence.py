...
PATCH_BASENAME = "MVP4_DASHBOARD_POSITION_REASON_UX"
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "DASHBOARD_POSITION_REASON_UX.md",
        f"""# DASHBOARD_POSITION_REASON_UX

context_pack_id: DASHBOARD_POSITION_REASON_UX
task_class: MVP4_DASHBOARD_POSITION_REASON_UX
...
- dashboard exposes final_action, no_trade_reason, entry status, and exit status from summary display truth
- open PAPER positions are shown as a display-only table with an explicit empty state
- decision_trace and position_snapshot cannot create live permission or claim execution truth
- no-trade reason must match the primary blocker when a blocker exists
...
Dashboard now shows a first-screen trading decision trace and open PAPER positions. The view explains NO_TRADE, entry, and exit status while remaining display-only and live-blocked.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_DASHBOARD_OPERATION_STATUS_UX.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "next_task_class": "MVP4_DASHBOARD_RECENT_EVENTS_UX",
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_DASHBOARD_POSITION_REASON_UX",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_DASHBOARD_POSITION_REASON_UX_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_json(
        ROOT / patch_result["evidence_manifest_path"],
        {
            ...
            "artifact_paths": [
                ...
                "tools/emit_dashboard_position_reason_ux_patch_evidence.py",
                "contracts/generated/context_pack/DASHBOARD_POSITION_REASON_UX.md",
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Position Reason UX Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_DASHBOARD_RECENT_EVENTS_UX"
