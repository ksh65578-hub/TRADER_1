...
PATCH_BASENAME = "MVP4_ORDER_PATH_ADAPTER_SUBMIT_RECHECK"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-ORDER-PATH-ADAPTER-SUBMIT-RECHECK"
NEXT_TASK_CLASS = "MVP4_IDEMPOTENCY_RECONCILIATION_EDGE_RECHECK"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_order_path_adapter_submit_recheck_patch_evidence.py",
    "contracts/generated/context_pack/ORDER_PATH_ADAPTER_SUBMIT_RECHECK.md",
]
BLOCKERS = [
    "LIVE_READY_MISSING",
    "API_UNVERIFIED",
...
class ExplodingOrderAdapter:
    def __getattribute__(self, name: str) -> Any:
        if name == "submit_order":
            raise AssertionError("submit_order must not be touched")
...
def order_path_adapter_audit() -> dict[str, Any]:
    prior_attempt = evaluate_live_order_path(
        {
            "source_kind": "FinalDecision",
            "final_decision": "ENTER_LONG",
            "client_order_id": "audit-existing-submit-attempt",
    ...
    all_green = evaluate_live_order_path(
        {
            "source_kind": "FinalDecision",
            "final_decision": "ENTER_LONG",
            "client_order_id": "audit-exploding-adapter",
            "single_writer_available": True,
            "budget_reserved": True,
            "local_reservation_committed": True,
            "ledger_reconciled": True,
    ...
    blockers: list[str] = []
    if not prior_attempt.external_submit_attempted or "LIVE_FINAL_GUARD_FAILED" not in prior_attempt.blockers:
        blockers.append("PRIOR_SUBMIT_ATTEMPT_FALSE_SAFE")
    if prior_attempt.order_adapter_called:
        blockers.append("PRIOR_SUBMIT_ATTEMPT_CALLED_ADAPTER")
    if all_green.order_adapter_called or all_green.external_submit_attempted:
        blockers.append("ALL_GREEN_PAYLOAD_TOUCHED_ADAPTER")
    if all_green.primary_blocker_code != "LIVE_ENABLING_EVIDENCE_MISSING":
        blockers.append("ALL_GREEN_PAYLOAD_NOT_LIVE_ENABLING_BLOCKED")
    status = "PASS" if not blockers else "BLOCKED"
    return {
        "audit_schema_id": "trader1.order_path_adapter_submit_recheck_audit.v1",
        "status": status,
        "blockers": blockers,
        "prior_attempt_external_submit_attempted": prior_attempt.external_submit_attempted,
        "prior_attempt_order_adapter_called": prior_attempt.order_adapter_called,
        "prior_attempt_blockers": list(prior_attempt.blockers),
        "all_green_order_adapter_called": all_green.order_adapter_called,
        "all_green_external_submit_attempted": all_green.external_submit_attempted,
        "all_green_primary_blocker_code": all_green.primary_blocker_code,
        "live_order_ready": False,
        "live_order_allowed": False,
        "can_live_trade": False,
        "scale_up_allowed": False,
        "next_action": (
            "Continue adapter-submit blocked regression monitoring."
            if status == "PASS"
            else "Block live review and repair adapter-submit false-safe handling."
        ),
    }
...
def update_context(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "ORDER_PATH_ADAPTER_SUBMIT_RECHECK.md",
        f"""# ORDER_PATH_ADAPTER_SUBMIT_RECHECK
...
coverage_snapshot:
- prior_attempt_external_submit_attempted: {str(audit["prior_attempt_external_submit_attempted"]).lower()}
- prior_attempt_order_adapter_called: {str(audit["prior_attempt_order_adapter_called"]).lower()}
- all_green_order_adapter_called: {str(audit["all_green_order_adapter_called"]).lower()}
- all_green_primary_blocker_code: {audit["all_green_primary_blocker_code"]}
...
Order path submit evidence is fail-closed: prior submit-attempt evidence is surfaced as LIVE_FINAL_GUARD_FAILED, and all-green spoofed live gate payloads still do not touch adapter.submit_order.
...
def update_requirement_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    requirements.append(
        {
            ...
            "source_section_id": "SECTION_ORDER_PATH_GUARD",
            "source_file": "TRADER_1.md",
            "source_heading": "Order path adapter submit recheck",
            "full_text_marker": f"{REQUIREMENT_ID}:adapter submit attempt evidence must be surfaced fail-closed",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Adapter submit attempt evidence is not reported safe",
            ...
            "live_affecting": True,
            "read_when": ["SECTION_ORDER_PATH_GUARD", "SECTION_LIVE_FINAL_GUARD", "SECTION_LEDGER_IDEMPOTENCY"],
            "depends_on": ["REQ-MVP4-LIVE-BLOCKED-NEGATIVE-RECHECK", "REQ-MVP0-ORDER-PATH-GUARD"],
            "source_text_sha256": sha256_bytes(b"adapter submit attempt evidence must be surfaced fail-closed"),
    ...
    rows.append(
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_ORDER_PATH_GUARD",
            "schema_files": [],
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], validators_required: list[str]) -> dict[str, Any]:
    patch_result: dict[str, Any] = {
        ...
        "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP0-ORDER-PATH-GUARD", "REQ-MVP4-LIVE-FINAL-GUARD"],
        ...
        "next_required_section_ids": ["SECTION_LEDGER_IDEMPOTENCY", "SECTION_RECONCILIATION", "SECTION_LIVE_FINAL_GUARD"],
        ...
        "active_read_surface_used": ["SECTION_ORDER_PATH_GUARD", "SECTION_LIVE_FINAL_GUARD", "SECTION_LEDGER_IDEMPOTENCY"],
        "task_class": "MVP4_ORDER_PATH_ADAPTER_SUBMIT_RECHECK",
        "required_section_ids": ["SECTION_ORDER_PATH_GUARD", "SECTION_LIVE_FINAL_GUARD", "SECTION_LEDGER_IDEMPOTENCY"],
        "expanded_section_ids": ["TRADER_1:order-path-guard-active-surface", "AGENTS:live-blocked-test-implementation-guide"],
        ...
        "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_ORDER_PATH_ADAPTER_SUBMIT_RECHECK",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], audit: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_ORDER_PATH_ADAPTER_SUBMIT_RECHECK_NO_LIVE_ORDERS",
            "order_path_adapter_audit": audit,
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Order Path Adapter Submit Recheck
...
Audit:
- prior_attempt_external_submit_attempted: {audit['prior_attempt_external_submit_attempted']}
- prior_attempt_blockers: {audit['prior_attempt_blockers']}
- all_green_order_adapter_called: {audit['all_green_order_adapter_called']}
- all_green_primary_blocker_code: {audit['all_green_primary_blocker_code']}
...
def main() -> int:
    ...
    audit = order_path_adapter_audit()
    ...
    if audit["status"] != "PASS":
        failed.append({"status": "FAIL", "reason": "order path adapter audit failed"})
    print(
        json.dumps(
            {
                "patch_id": PATCH_ID,
