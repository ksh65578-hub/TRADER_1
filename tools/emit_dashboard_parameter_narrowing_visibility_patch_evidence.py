...
PATCH_BASENAME = "MVP4_DASHBOARD_PARAMETER_NARROWING_VISIBILITY"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-PARAMETER-NARROWING-VISIBILITY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_parameter_narrowing_visibility_patch_evidence.py",
    ...
    "system/evidence/audit_reports/MVP4_DASHBOARD_PARAMETER_NARROWING_VISIBILITY_20260429.md",
    "contracts/generated/context_pack/DASHBOARD_PARAMETER_NARROWING_VISIBILITY.md",
]
...
def update_registry(now: str) -> None:
    ...
    for group in (
        ...
        for validator_id in ("read_only_dashboard_validator", "parameter_narrowing_validator"):
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_section_id": "SECTION_DASHBOARD_PARAMETER_NARROWING",
            "source_file": "TRADER_1.md",
            "source_heading": "dashboard parameter narrowing proposal visibility without live permission",
            "full_text_marker": (
                f"{REQUIREMENT_ID}:dashboard shows parameter narrowing dependency closure sample coverage "
                "proposal only paper parameter review active config mutation blocked live blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard parameter narrowing proposal visibility",
            "requirement_kind": "DASHBOARD_UX_PATCH",
            "schema_ids": ["trader1.read_only_dashboard_shell.v1"],
            "validator_ids": ["read_only_dashboard_validator", "parameter_narrowing_validator"],
            ...
            "depends_on": [
                "REQ-MVP4-PARAMETER-NARROWING-SCHEMA-HARDENING",
                "REQ-MVP4-DASHBOARD-EXPLORATION-POLICY-VISIBILITY",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": sha256_bytes(
                b"dashboard shows parameter narrowing proposal only paper review active config mutation blocked live blocked"
            ),
    ...
    ensure_matrix_row(
        ...
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_DASHBOARD_PARAMETER_NARROWING",
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "DASHBOARD_PARAMETER_NARROWING_VISIBILITY.md",
        f"""# DASHBOARD_PARAMETER_NARROWING_VISIBILITY
...
Read-only dashboards show system status, operator next action, workflow, portfolio, positions, strategy evidence maturity, execution feedback, risk exposure, convergence assessment, exploration/exploitation policy, and parameter narrowing. Parameter narrowing is explicitly proposal-only and PAPER review only; it cannot mutate active config, live config, live readiness, order submission, or scale-up permission.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(
        ROOT / "system" / "evidence" / "patch_results" / "MVP4_PARAMETER_NARROWING_SCHEMA_HARDENING.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "next_task_class": "MVP4_PROFITABILITY_EVIDENCE_MATURITY_GAP_RECHECK",
            "next_required_section_ids": [
                "SECTION_PROFITABILITY_EVIDENCE_MATURITY",
                "SECTION_OPTIMIZER_CONVERGENCE_GUARDRAIL",
                "SECTION_LIVE_FINAL_GUARD",
            ],
            "next_optional_section_ids": ["SECTION_DASHBOARD_OPERATOR_UX", "SECTION_PARAMETER_NARROWING"],
            ...
            "optimizer_patch": "DASHBOARD_PARAMETER_NARROWING_VISIBILITY_ANALYSIS_ONLY",
            "optimizer_stage": "MVP4_REVIEW_PREP",
            "optimizer_status_before": "PARAMETER_NARROWING_VALIDATED_BUT_NOT_OPERATOR_VISIBLE",
            "optimizer_status_after": "PARAMETER_NARROWING_OPERATOR_VISIBLE_LIVE_BLOCKED",
            "optimizer_maturity_level_before": "MVP4_PARAMETER_NARROWING_SCHEMA_HARDENED",
            "optimizer_maturity_level_after": "MVP4_PARAMETER_NARROWING_OPERATOR_VISIBLE",
            "optimizer_output_type": "DASHBOARD_DISPLAY_ONLY_PARAMETER_NARROWING_PROPOSAL_STATUS",
            ...
            "optimizer_validators_run": [
                ...
                in {"read_only_dashboard_validator", "parameter_narrowing_validator", "optimizer_guardrail_validator"}
            ],
            ...
            "profit_convergence_patch": "DASHBOARD_PARAMETER_NARROWING_VISIBILITY_NO_LIVE_MUTATION",
            "convergence_layer_changed": False,
            "convergence_state_before": "PARAMETER_NARROWING_SCHEMA_HARDENED_LIVE_BLOCKED",
            "convergence_state_after": "PARAMETER_NARROWING_DASHBOARD_VISIBLE_LIVE_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_DASHBOARD_PARAMETER_NARROWING_VISIBILITY_LIVE_REVIEW_STILL_BLOCKED",
            "next_allowed_task_class": "MVP4_PROFITABILITY_EVIDENCE_MATURITY_GAP_RECHECK",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Dashboard Parameter Narrowing Visibility
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_PROFITABILITY_EVIDENCE_MATURITY_GAP_RECHECK"
    ...
    state["implemented_validator_ids"] = sorted(
        set(state.get("implemented_validator_ids", []) + ["read_only_dashboard_validator", "parameter_narrowing_validator"])
    )
