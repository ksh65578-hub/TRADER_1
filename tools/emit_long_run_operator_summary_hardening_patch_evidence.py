...
PATCH_BASENAME = "MVP4_LONG_RUN_OPERATOR_SUMMARY_HARDENING"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-LONG-RUN-OPERATOR-SUMMARY-HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_long_run_operator_summary_hardening_patch_evidence.py",
    "contracts/generated/context_pack/LONG_RUN_OPERATOR_SUMMARY.md",
    "system/evidence/audit_reports/MVP4_LONG_RUN_OPERATOR_SUMMARY_HARDENING_20260429.md",
]
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "LONG_RUN_OPERATOR_SUMMARY.md",
        f"""# LONG_RUN_OPERATOR_SUMMARY
...
Dashboard now includes a Long-Run Operation panel on the first screen. It separates "running normally now" from "validated stable history" so the operator can see current health without mistaking it for live readiness. Blue/green indicate normal display status, yellow indicates stale or attention, and red is reserved for error severity.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "Long-run operator summary and status colors",
            "full_text_marker": f"{REQUIREMENT_ID}:SECTION_DASHBOARD_SHELL:Long-run operation visible without live permission",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Show current normal operation and validated stability history without live permission",
            ...
            "live_affecting": True,
            "read_when": ["DASHBOARD_UX", "LONG_RUN_STABILITY", "LIVE_BLOCKED_TEST"],
            "depends_on": [
                "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL",
                "REQ-MVP4-RUNTIME-STABILITY-HISTORY-SCAFFOLD",
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_EXECUTION_FEEDBACK_DASHBOARD_OPERATOR_VISIBILITY.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "next_task_class": "MVP4_OPERATOR_ACTION_SUMMARY_HARDENING",
            "next_required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_OPERATOR_VISIBILITY", "SECTION_LIVE_FINAL_GUARD"],
            "next_optional_section_ids": ["SECTION_RUNTIME_STABILITY", "SECTION_CONVERGENCE_DASHBOARD"],
            ...
            "active_read_surface_used": ["SECTION_DASHBOARD_SHELL", "SECTION_RUNTIME_STABILITY", "SECTION_OPERATOR_VISIBILITY", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "MVP4_LONG_RUN_OPERATOR_SUMMARY_HARDENING",
            "required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_RUNTIME_STABILITY", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": [
                "TRADER_1:dashboard-shell-active-surface",
                "TRADER_1:runtime-stability-active-surface",
            ...
            "optimizer_status_before": "UNCHANGED",
            "optimizer_status_after": "UNCHANGED",
            "optimizer_output_type": "NO_OPTIMIZER_OUTPUT",
            "optimizer_validators_required": ["optimizer_no_live_mutation_validator"],
            "optimizer_validators_run": run_validators(["optimizer_no_live_mutation_validator"]),
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_LONG_RUN_OPERATOR_SUMMARY",
            "convergence_validators_required": ["convergence_assessment_validator"],
            "convergence_validators_run": run_validators(["convergence_assessment_validator"]),
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_LONG_RUN_OPERATOR_SUMMARY_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Long-Run Operator Summary Hardening Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["implemented_validator_ids"] = sorted(set(state.get("implemented_validator_ids", []) + ["read_only_dashboard_validator", "runtime_stability_history_validator"]))
    state["last_patch_id"] = PATCH_ID
    state["last_patch_result_hash"] = patch_result["result_hash"]
    state["next_allowed_task_class"] = "MVP4_OPERATOR_ACTION_SUMMARY_HARDENING"
