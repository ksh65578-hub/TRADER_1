...
PATCH_BASENAME = "MVP4_PAPER_SHADOW_LONG_RUN_STATE_CONSISTENCY"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-PAPER-SHADOW-LONG-RUN-STATE-CONSISTENCY"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_paper_shadow_long_run_state_consistency_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_PAPER_SHADOW_LONG_RUN_STATE_CONSISTENCY.md",
]
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_PAPER_SHADOW_LONG_RUN_STATE_CONSISTENCY.md",
        f"""# MVP4_PAPER_SHADOW_LONG_RUN_STATE_CONSISTENCY
...
PAPER/SHADOW long-run evidence state is now internally consistent. A report cannot claim scorecard eligibility while hiding a long-run evidence flag mismatch.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Paper/shadow long-run evidence state consistency",
            "full_text_marker": f"{REQUIREMENT_ID}: long-run eligibility state must match validated source evidence and cannot be hidden by scorecard eligibility",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Paper/shadow long-run state consistency",
            ...
            "source_text_sha256": base.sha256_bytes(b"long-run eligibility state must match validated source evidence"),
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "paper/shadow evidence accumulation validator",
                "long-run evidence state mismatch probe",
            ],
            "task_class": NEXT_TASK_CLASS,
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_LONG_RUN_STATE_CONSISTENT",
            "optimizer_guardrail_result": "PASS_SCORECARD_CANNOT_HIDE_LONG_RUN_STATE_DRIFT",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "convergence_state_after": "LONG_RUN_EVIDENCE_BLOCKED_STATE_CONSISTENCY_GUARDED",
            "convergence_guardrail_result": "PASS_SCORECARD_CANNOT_HIDE_LONG_RUN_STATE_DRIFT",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_LONG_RUN_STATE_CONSISTENCY_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Paper/Shadow Long-Run State Consistency Audit
