...
PATCH_BASENAME = "MVP4_OOS_ROBUSTNESS_SCHEMA_HARDENING"
REQUIREMENT_ID = "REQ-MVP4-OOS-ROBUSTNESS-SCHEMA-HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tests/validators/test_overfit_diagnostic_validator.py",
    "tests/validators/fixtures/overfit_diagnostic_pass.json",
    "tests/validators/fixtures/overfit_diagnostic_short_window_fail.json",
    "tests/validators/fixtures/overfit_diagnostic_bootstrap_unstable_fail.json",
    "tests/validators/fixtures/overfit_diagnostic_live_flag_fail.json",
    "tools/run_overfit_diagnostic_validators.py",
    "tools/emit_oos_robustness_schema_hardening_patch_evidence.py",
    ...
    "contracts/generated/context_pack/OOS_ROBUSTNESS_SCHEMA_HARDENING.md",
]
...
def update_gap_artifacts(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    for gap in audit.get("gaps", []):
        if gap.get("component") == "overfit_oos_walk_forward":
            gap["patch_status"] = "PARTIAL_PATCHED"
            gap["fix"] = (
                "overfit_diagnostic_report schema and validator now require OOS, walk-forward, bootstrap, ranking "
                "stability, survivorship-bias, data-snooping, concentration, evidence ids, false live flags, and "
                "negative fixtures; runtime evidence accumulation and longer paper/shadow samples remain required."
            )
    ...
    actions.append(
        {
            "action_id": PATCH_ID,
            "status": "APPLIED",
            "summary": "Hardened overfit diagnostic schema and validator for OOS, walk-forward, bootstrap, ranking stability, and bias checks.",
    ...
    gap["notes"] = (
        "Candidate scorecards require cost-adjusted net EV, strategy_condition_matrix requires entry/exit/no-trade/"
        "regime traceability, symbol_strategy_regime_fit_report requires liquidity/volatility/spread/depth/regime "
        "evidence, and overfit_diagnostic_report now requires OOS/walk-forward/bootstrap/ranking-stability/bias "
        "evidence with fail-closed fixtures. The broader profitability maturity gap remains open until execution "
        "feedback, convergence memory, paper/shadow evidence accumulation, and dashboard profitability maturity "
        "visibility are validated. No live readiness or scale-up permission is created."
    )
...
def update_registry(now: str) -> None:
    ...
    registry.setdefault("schemas", {})["overfit_diagnostic_report"] = {
        "schema_id": "trader1.overfit_diagnostic_report.v1",
        "path": "contracts/schema/overfit_diagnostic_report.schema.json",
    }
    validators = registry.setdefault("validators", {})
    robustness = validators.setdefault("VALIDATOR_GROUP:OPTIMIZER_ROBUSTNESS", [])
    if "overfit_diagnostic_validator" not in robustness:
        robustness.append("overfit_diagnostic_validator")
    core = validators.setdefault("VALIDATOR_GROUP:OPTIMIZER_CORE", [])
    if "overfit_diagnostic_validator" not in core:
        core.append("overfit_diagnostic_validator")
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") != "overfit_diagnostic_validator"]
    implemented.append(
        {
            "validator_id": "overfit_diagnostic_validator",
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_requirement(
        index,
        {
            ...
            "source_section_id": "SECTION_OVERFIT_ROBUSTNESS",
            "source_file": "TRADER_1.md",
            "source_heading": "OOS, walk-forward, bootstrap, and overfit diagnostics block fragile candidates",
            "full_text_marker": f"{REQUIREMENT_ID}:overfit diagnostic requires OOS walk-forward bootstrap ranking stability and bias evidence",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "OOS robustness schema hardening",
            "requirement_kind": "SCHEMA_VALIDATOR_PATCH",
            "schema_ids": ["trader1.overfit_diagnostic_report.v1"],
            "validator_ids": ["overfit_diagnostic_validator"],
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/validators/test_overfit_diagnostic_validator.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": sha256_bytes(
                b"overfit diagnostic requires OOS walk-forward bootstrap ranking stability and bias evidence"
            ),
    ...
    ensure_matrix_row(
        ...
        {
            "requirement_id": REQUIREMENT_ID,
            "section_id": "SECTION_OVERFIT_ROBUSTNESS",
            "schema_files": ["contracts/schema/overfit_diagnostic_report.schema.json"],
            "validator_files": ["trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/validators/test_overfit_diagnostic_validator.py"],
            "fixture_files": [
                "tests/validators/fixtures/overfit_diagnostic_pass.json",
                "tests/validators/fixtures/overfit_diagnostic_short_window_fail.json",
                "tests/validators/fixtures/overfit_diagnostic_bootstrap_unstable_fail.json",
            ...
            "runtime_modules": ["tools/run_overfit_diagnostic_validators.py"],
            "evidence_artifacts": [
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "OOS_ROBUSTNESS_SCHEMA_HARDENING.md",
        f"""# OOS_ROBUSTNESS_SCHEMA_HARDENING

context_pack_id: OOS_ROBUSTNESS_SCHEMA_HARDENING
task_class: MVP4_OOS_ROBUSTNESS_SCHEMA_HARDENING
source_trader1_sha256: {trader_hash}
source_agents_sha256: {agents_hash}
included_section_ids: ["SECTION_OVERFIT_ROBUSTNESS", "SECTION_STRATEGY_PROFITABILITY", "SECTION_OPTIMIZER_OBJECTIVE", "SECTION_LIVE_FINAL_GUARD"]
...
- overfit diagnostic requires OOS, walk-forward, bootstrap, ranking stability, survivorship-bias, and data-snooping checks
- robustness eligibility requires positive OOS net EV after cost and threshold checks
- short-window, unstable bootstrap, and live-flag fixtures fail closed
- diagnostic remains dashboard/display truth only and cannot create live readiness, live order permission, promotion, or scale-up
...
- no risk scale-up
- execution feedback, convergence memory, and paper/shadow evidence accumulation remain open contract_gap work
...
OOS robustness evidence now requires OOS, walk-forward, bootstrap, ranking stability, survivorship-bias, data-snooping, and concentration checks with negative fixture coverage. This reduces short-window overfit risk without creating live readiness, live order permission, or scale-up permission.
""",
    )
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_SYMBOL_REGIME_FIT_SCHEMA_HARDENING.patch_result.json"
    )
    optimizer_results = [
        result
    ...
    patch_result.update(
        {
            ...
            "merged_requirements": [],
            "new_registry_items": [
                "contracts/registry.yaml::schemas.overfit_diagnostic_report",
                "contracts/registry.yaml::validators.overfit_diagnostic_validator",
                "contracts/validators/validator_registry.json::overfit_diagnostic_validator",
                *CHANGED_ARTIFACTS,
            ],
            "new_or_changed_schema_ids": ["trader1.overfit_diagnostic_report.v1"],
            ...
            "next_task_class": "MVP4_EXECUTION_FEEDBACK_COST_MODEL_HARDENING",
            "next_required_section_ids": [
                "SECTION_EXECUTION_FEEDBACK",
                "SECTION_COST_MODEL",
                "SECTION_STRATEGY_PROFITABILITY",
            ],
            "next_optional_section_ids": ["SECTION_CONVERGENCE_MEMORY", "SECTION_DASHBOARD_OPERATOR_UX"],
            ...
            "expanded_section_ids": [
                "TRADER_1:overfit-robustness-active-surface",
            ...
            "optimizer_patch": "OOS_ROBUSTNESS_SCHEMA_VALIDATOR_PATCH",
            "optimizer_stage": "MVP4_REVIEW_PREP",
            "optimizer_status_before": "SYMBOL_REGIME_FIT_GUARDED_LIVE_BLOCKED",
            "optimizer_status_after": "OOS_ROBUSTNESS_GUARDED_LIVE_BLOCKED",
            "optimizer_maturity_level_before": "MVP4_SYMBOL_REGIME_FIT_GUARDED",
            "optimizer_maturity_level_after": "MVP4_OOS_WALK_FORWARD_BOOTSTRAP_GUARDED",
            "optimizer_output_type": "ANALYSIS_ONLY_OVERFIT_DIAGNOSTIC_SCHEMA",
            ...
            "profit_convergence_patch": "OOS_ROBUSTNESS_GUARD_NO_CONVERGENCE_RUNTIME_CHANGE",
            "convergence_layer_changed": False,
            "convergence_state_before": "SYMBOL_REGIME_FIT_GUARDED_LIVE_BLOCKED",
            "convergence_state_after": "OOS_ROBUSTNESS_GUARDED_LIVE_BLOCKED",
            "objective_profile_changed": False,
            "memory_schema_changed": False,
            "failure_analysis_required": True,
            "failure_analysis_status": "CONTRACT_GAP_REMAINS_OPEN_FOR_EXECUTION_FEEDBACK_MEMORY_AND_PAPER_SHADOW_EVIDENCE",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_OOS_ROBUSTNESS_SCHEMA_HARDENING_LIVE_REVIEW_STILL_BLOCKED",
            "open_contract_gap_id": CONTRACT_GAP_ID,
            "next_allowed_task_class": "MVP4_EXECUTION_FEEDBACK_COST_MODEL_HARDENING",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 OOS Robustness Schema Hardening
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_EXECUTION_FEEDBACK_COST_MODEL_HARDENING"
    state["completed_requirement_ids"] = sorted(set(state.get("completed_requirement_ids", []) + [REQUIREMENT_ID]))
    state["implemented_schema_ids"] = sorted(
        set(state.get("implemented_schema_ids", []) + ["trader1.overfit_diagnostic_report.v1"])
    )
    state["implemented_validator_ids"] = sorted(
        set(state.get("implemented_validator_ids", []) + ["overfit_diagnostic_validator"])
    )
    state["untested_validator_ids"] = [
        item for item in state.get("untested_validator_ids", []) if item != "overfit_diagnostic_validator"
    ]
