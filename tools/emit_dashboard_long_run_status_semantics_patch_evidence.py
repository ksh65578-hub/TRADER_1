...
PATCH_BASENAME = "MVP4_DASHBOARD_LONG_RUN_STATUS_SEMANTICS"
PATCH_ID = f"{PATCH_BASENAME}_20260430_001"
REQUIREMENT_ID = "REQ-MVP4-DASHBOARD-LONG-RUN-STATUS-SEMANTICS"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_dashboard_long_run_status_semantics_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_DASHBOARD_LONG_RUN_STATUS_SEMANTICS.md",
]
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_DASHBOARD_LONG_RUN_STATUS_SEMANTICS.md",
        f"""# MVP4_DASHBOARD_LONG_RUN_STATUS_SEMANTICS
...
The dashboard shell now separates display-history stability from actual long-run validation. A clean display history can show normal operation, but the machine-readable blocker remains actual persistent runtime evidence missing.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard long-run status semantics",
            "full_text_marker": f"{REQUIREMENT_ID}: dashboard shell status must separate display-history stability from actual long-run validation",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard long-run status semantics",
            ...
            "depends_on": [
                "REQ-MVP4-DASHBOARD-LONG-RUN-DISPLAY-HISTORY-UX",
                "REQ-MVP4-DASHBOARD-ACTUAL-LONG-RUN-CYCLE-FLOOR",
                "REQ-MVP4-DASHBOARD-ACTUAL-LONG-RUN-FLOOR-UX",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"dashboard shell status must separate display-history stability from actual long-run validation"
            ),
    ...
    patch_result.update(
        {
            ...
            "active_read_surface_used": [
                ...
                "dashboard long-run status semantics",
                "read-only dashboard shell schema",
            ...
            "optimizer_guardrail_result": "PASS_DISPLAY_HISTORY_STATUS_NOT_OPTIMIZER_OR_LIVE_EVIDENCE",
            "convergence_state_after": "LONG_RUN_EVIDENCE_BLOCKED_DISPLAY_HISTORY_STATUS_SEPARATED_FROM_ACTUAL_RUNTIME",
            "convergence_guardrail_result": "PASS_DISPLAY_HISTORY_STATUS_NOT_CONVERGENCE_OR_LIVE_EVIDENCE",
        }
    )
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_DASHBOARD_LONG_RUN_STATUS_SEMANTICS_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Dashboard Long-Run Status Semantics Audit
...
def main() -> int:
    ...
    patch_result = finalize_patch_result(base.build_patch_result(now, tests_run, validators_run, regenerated), now, tests_run, validators_run)
