...
PATCH_ID = "MVP0_CONFIG_VALIDATION_20260428_001"
...
def update_registry(now: str) -> None:
    ...
    schemas["runtime_config"] = {
        "schema_id": "trader1.runtime_config.v1",
        "path": "contracts/schema/runtime_config.schema.json",
    }
    ...
    for group_name in ("VALIDATOR_GROUP:MVP0_CORE", "VALIDATOR_GROUP:LIVE_SAFETY_CORE", "supplemental_mvp0"):
        ...
        if "runtime_config_validator" not in group:
            group.append("runtime_config_validator")
...
def update_validator_registry(now: str) -> None:
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") != "runtime_config_validator"]
    implemented.append(
        {
            "validator_id": "runtime_config_validator",
            "module_path": "trader1.validation.mvp0_validators",
            "status": "IMPLEMENTED",
            "live_enabling": False,
        }
    )
    registry["runtime_config_module"] = "trader1.config.config_schema"
    write_json(path, registry)
...
def update_navigation(now: str, trader_hash: str, agents_hash: str) -> None:
    ...
    ensure_section(
        section_map,
        {
            "section_id": "SECTION_CONFIG_VALIDATION",
            "source_file": "TRADER_1.md",
            "source_sha256": trader_hash,
            "source_heading": "implementation order config validation",
            "line_start": 16009,
            "line_end": 16024,
            "source_section_sha256": source_hash(trader_lines, 16009, 16024),
    ...
    ensure_section(
        section_map,
        {
            "section_id": "SECTION_HARD_TRUTH_CONFIG_FIELDS",
            "source_file": "TRADER_1.md",
            "source_sha256": trader_hash,
            "source_heading": "hard truth and soft truth",
            "line_start": 9577,
            "line_end": 9610,
            "source_section_sha256": source_hash(trader_lines, 9577, 9610),
            "authority_level": "ACTIVE_AUTHORITY",
            "read_default": False,
            "generated_artifact_is_authority": False,
        },
    ...
    ensure_section(
        section_map,
        {
            "section_id": "SECTION_TEST_LANES_CONFIG_SCHEMA",
            "source_file": "TRADER_1.md",
            "source_sha256": trader_hash,
            "source_heading": "testing lanes",
            "line_start": 10293,
            "line_end": 10312,
            "source_section_sha256": source_hash(trader_lines, 10293, 10312),
            "authority_level": "ACTIVE_AUTHORITY",
            "read_default": False,
            "generated_artifact_is_authority": False,
        },
    ...
    ensure_section(
        section_map,
        {
            "section_id": "SECTION_AGENTS_CONFIG_VALIDATION",
            "source_file": "AGENTS.md",
            "source_sha256": agents_hash,
            "source_heading": "MVP-1 required files and config validation",
            "line_start": 10136,
            "line_end": 10150,
            "source_section_sha256": source_hash(agents_lines, 10136, 10150),
    ...
    source_text_sha = source_hash(trader_lines, 9577, 9610)
    requirement = {
        "requirement_id": "REQ-MVP0-CONFIG-VALIDATION",
        "source_section_id": "SECTION_CONFIG_VALIDATION",
        "source_file": "TRADER_1.md",
        "source_heading": "implementation order config validation",
        "full_text_marker": "config validation requires hard truth fields and safe config schema before startup probe",
        "authority_level": "ACTIVE_AUTHORITY",
        "requirement_title": "MVP-0 fail-closed runtime config validation scaffold",
        "requirement_kind": "MVP0_CONTRACT_BASELINE",
        "schema_ids": ["trader1.runtime_config.v1", "trader1.validator_result.v1"],
        "validator_ids": ["runtime_config_validator"],
        "artifact_ids": [
            "contracts/schema/runtime_config.schema.json",
            "trader1/config/config_schema.py",
            "tools/run_config_validators.py",
            "tests/config/test_config_validation.py",
        ],
        "test_ids": ["tests/config/test_config_validation.py"],
        "mvp_stage": "MVP-0",
        "implementation_depth_min": "DEPTH_3_NEGATIVE_FIXTURES",
        "blocking_level": "LIVE_BLOCKING",
        "live_affecting": True,
        "read_when": ["MVP0_CONTRACT_BASELINE", "LIVE_BLOCKED_TEST"],
        "depends_on": ["REQ-MVP0-LIVE-DEFAULT-FALSE", "REQ-MVP0-ROOT-LAUNCHER-GUARD"],
    ...
    ensure_matrix_row(
        matrix,
        {
            "requirement_id": "REQ-MVP0-CONFIG-VALIDATION",
            "section_id": "SECTION_CONFIG_VALIDATION",
            "schema_files": ["contracts/schema/runtime_config.schema.json"],
            "validator_files": [
                "trader1/config/config_schema.py",
                "trader1/validation/mvp0_validators.py",
            ],
            "test_files": ["tests/config/test_config_validation.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/config/config_schema.py"],
            "evidence_artifacts": [
                "system/evidence/validator_runs/MVP0_CONFIG_VALIDATION.validator_run_log.json",
                "system/evidence/patch_results/MVP0_CONFIG_VALIDATION.patch_result.json",
            ],
            "dashboard_artifacts": [],
            "patch_result_fields": [
    ...
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "CONFIG_VALIDATION.md",
        f"""# CONFIG_VALIDATION
...
def update_state_and_evidence(now: str, trader_hash: str, agents_hash: str) -> None:
    validator_results = run_validators(["runtime_config_validator"])
    validator_log = {
    ...
    write_json(ROOT / "system" / "evidence" / "validator_runs" / "MVP0_CONFIG_VALIDATION.validator_run_log.json", validator_log)
    ...
    stage_gate = {
        ...
        "runtime_config_validator": "PASS",
        "config_schema": "PASS",
        "live_order_allowed": False,
        "stage_gate_status": "PASS_FOR_MVP0_CONFIG_VALIDATION_ONLY",
    }
    write_json(ROOT / "system" / "evidence" / "stage_gates" / "MVP0_CONFIG_VALIDATION.stage_gate_result.json", stage_gate)
    ...
    evidence_manifest = {
        ...
        "evidence_manifest_id": "MVP0_CONFIG_VALIDATION_EVIDENCE",
        "artifact_paths": [
            "contracts/schema/runtime_config.schema.json",
            "trader1/config/config_schema.py",
            "trader1/validation/mvp0_validators.py",
            ...
            "tests/config/test_config_validation.py",
            "system/evidence/validator_runs/MVP0_CONFIG_VALIDATION.validator_run_log.json",
            "system/evidence/stage_gates/MVP0_CONFIG_VALIDATION.stage_gate_result.json",
        ],
        "known_blockers": [
            "LIVE_READY_MISSING",
    ...
    write_json(ROOT / "system" / "evidence" / "MVP0_CONFIG_VALIDATION.evidence_manifest.json", evidence_manifest)
    ...
    patch_result = {
        ...
        "affected_contract_ids": ["REQ-MVP0-CONFIG-VALIDATION"],
        ...
        "new_registry_items": ["runtime_config schema", "runtime_config_validator"],
        "new_or_changed_schema_ids": ["trader1.runtime_config.v1"],
        "validators_required": ["runtime_config_validator"],
        "validators_run": validator_results,
        "tests_run": [
            {"command": "python -m compileall trader1 tools tests -q", "status": "PASS"},
            {"command": "python tools/run_config_validators.py", "status": "PASS"},
            {"command": "python -m unittest tests.config.test_config_validation -v", "status": "PASS"},
        ...
        "retained_archive_semantic_mapping_status": "NOT_READ_NOT_REQUIRED_FOR_CONFIG_VALIDATION",
        ...
        "next_optional_section_ids": ["SECTION_STARTUP_PROBE", "SECTION_HEARTBEAT"],
        ...
        "evidence_manifest_path": "system/evidence/MVP0_CONFIG_VALIDATION.evidence_manifest.json",
        "validator_run_log_path": "system/evidence/validator_runs/MVP0_CONFIG_VALIDATION.validator_run_log.json",
        "stage_gate_result_path": "system/evidence/stage_gates/MVP0_CONFIG_VALIDATION.stage_gate_result.json",
        "token_navigation_patch": True,
        "active_read_surface_used": [
            "SECTION_CONFIG_VALIDATION",
            "SECTION_HARD_TRUTH_CONFIG_FIELDS",
            "SECTION_TEST_LANES_CONFIG_SCHEMA",
            "SECTION_AGENTS_CONFIG_VALIDATION",
        ],
        "task_class": "MVP0_CONTRACT_BASELINE",
        "required_section_ids": ["SECTION_CONFIG_VALIDATION", "SECTION_HARD_TRUTH_CONFIG_FIELDS"],
        ...
        "failure_analysis_status": "NOT_REQUIRED_FOR_CONFIG_VALIDATION",
    ...
    write_json(ROOT / "system" / "evidence" / "patch_results" / "MVP0_CONFIG_VALIDATION.patch_result.json", patch_result)

    state_path = ROOT / "contracts" / "generated" / "current_implementation_state.json"
    state = load_json(state_path)
    if "REQ-MVP0-CONFIG-VALIDATION" not in state["completed_requirement_ids"]:
        state["completed_requirement_ids"].append("REQ-MVP0-CONFIG-VALIDATION")
    if "trader1.runtime_config.v1" not in state["implemented_schema_ids"]:
        state["implemented_schema_ids"].append("trader1.runtime_config.v1")
    if "runtime_config_validator" not in state["implemented_validator_ids"]:
        state["implemented_validator_ids"].append("runtime_config_validator")
    ...
    ledger["patches"].append(
        {
            ...
            "patch_class": "SCHEMA_PATCH",
            "target_mvp_level": "MVP-0",
            "patch_result_path": "system/evidence/patch_results/MVP0_CONFIG_VALIDATION.patch_result.json",
