...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_STALE_LOOP_REGENERATION_EXECUTION_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-STALE-LOOP-REGENERATION-EXECUTION-GUARD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_STALE_LOOP_SAFE_REGENERATION_EXECUTOR"
...
CHANGED_ARTIFACTS = [
    "contracts/schema/upbit_paper_stale_loop_regeneration_execution_guard.schema.json",
    ...
    "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_stale_loop_regeneration_execution_guard.json",
    "contracts/security/source_bundle_manifest.json",
    "tools/emit_upbit_paper_stale_loop_execution_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_STALE_LOOP_REGENERATION_EXECUTION_GUARD.md",
]

BLOCKERS = [
    "LONG_RUN_PAPER_RUNTIME_EVIDENCE_INSUFFICIENT",
    "ACTUAL_LONG_RUN_RUNTIME_EVIDENCE_MISSING",
    "STALE_LOOP_SAFE_REGENERATION_EXECUTOR_REQUIRED",
    "LIVE_READY_MISSING",
    "API_UNVERIFIED",
...
def write_runtime_guard() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    return reconciliation, plan, guard


def write_context(now: str, trader_hash: str, agents_hash: str, guard: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_STALE_LOOP_REGENERATION_EXECUTION_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_STALE_LOOP_REGENERATION_EXECUTION_GUARD
...
- source_plan_status: {guard["source_plan_status"]}
- planned_regeneration_item_count: {guard["planned_regeneration_item_count"]}
- replacement_existing_count: {guard["replacement_existing_count"]}
- source_hash_mismatch_count: {guard["source_hash_mismatch_count"]}
...
- source plan status: {guard["source_plan_status"]}
- create-new replacement items: {guard["planned_regeneration_item_count"]}
- source hash mismatches: {guard["source_hash_mismatch_count"]}
- existing replacement paths: {guard["replacement_existing_count"]}
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER stale loop regeneration execution guard",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: stale loop regeneration requires source-preserving create-new execution "
                "precondition guard before any safe executor patch"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER stale loop regeneration execution guard",
            "requirement_kind": "SCHEMA_VALIDATOR_RUNTIME_ARTIFACT_PATCH",
            "schema_ids": ["trader1.upbit_paper_stale_loop_regeneration_execution_guard.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/runtime/test_upbit_paper_stale_loop_execution_guard.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(
                b"stale loop regeneration execution guard verifies create-new paper-only preconditions without execution"
            ),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_GUARD_ONLY_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "schema_files": ["contracts/schema/upbit_paper_stale_loop_regeneration_execution_guard.schema.json"],
            "validator_files": [
                "trader1/runtime/paper/upbit_paper_stale_loop_execution_guard.py",
                "trader1/validation/mvp0_validators.py",
            ],
            "test_files": ["tests/runtime/test_upbit_paper_stale_loop_execution_guard.py"],
            "fixture_files": [],
            "runtime_modules": [
                "trader1/runtime/paper/upbit_paper_stale_loop_execution_guard.py",
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_GUARD_ONLY_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_stale_loop_regeneration_execution_guard.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_stale_loop_execution_guard_validator")
    ...
    state["open_contract_gap_ids"] = sorted(
        ...
        | {"ACTUAL_LONG_RUN_RUNTIME_EVIDENCE_BOUNDARY", "STALE_LOOP_SAFE_REGENERATION_EXECUTOR_REQUIRED"}
    )
...
def build_patch_result(
    ...
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PAPER_STALE_LOOP_REGENERATION_POLICY.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_stale_loop_execution_guard_validator"],
            "new_or_changed_schema_ids": ["trader1.upbit_paper_stale_loop_regeneration_execution_guard.v1"],
            ...
            "active_read_surface_used": [
                ...
                "stale loop regeneration policy",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_STALE_LOOP_EXECUTION_GUARDED_BLOCKED",
            "optimizer_guardrail_result": "PASS_EXECUTION_GUARD_REMAINS_ANALYSIS_ONLY_AND_LIVE_BLOCKED",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "profit_convergence_patch": "true",
            "convergence_layer_changed": True,
            "convergence_state_after": "STALE_RUNTIME_LOOP_EXECUTION_GUARDED_EXECUTOR_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], guard: dict[str, Any]) -> None:
    base.write_json(
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_STALE_LOOP_EXECUTION_GUARDED_EXECUTOR_BLOCKED",
            "guard_status": guard["guard_status"],
            "source_plan_status": guard["source_plan_status"],
            "planned_regeneration_item_count": guard["planned_regeneration_item_count"],
            "replacement_existing_count": guard["replacement_existing_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Stale Loop Regeneration Execution Guard Audit
...
def write_patch_artifacts(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], guard: dict[str, Any]) -> None:
    patch_path = ROOT / "system" / "evidence" / "patch_results" / f"{PATCH_BASENAME}.patch_result.json"
    write_evidence(now, trader_hash, agents_hash, patch_result, guard)
...
def main() -> int:
    ...
    _, _, guard = write_runtime_guard()
    write_context(now, trader_hash, agents_hash, guard)
    ...
    patch_result = build_patch_result(now, tests_run, validators_run, guard)
    write_patch_artifacts(now, trader_hash, agents_hash, patch_result, guard)
