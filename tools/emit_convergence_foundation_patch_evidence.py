...
PATCH_ID = "MVP4_CONVERGENCE_FOUNDATION_20260429_001"
PATCH_RESULT_REL = "system/evidence/patch_results/MVP4_CONVERGENCE_FOUNDATION.patch_result.json"
FOUNDATION_IDS = [
...
def run_command(command: list[str], label: str) -> dict[str, str]:
    ...
    return {"command": label, "status": "PASS" if completed.returncode == 0 else "FAIL", "returncode": str(completed.returncode)}
...
def update_validator_registry(now: str) -> None:
    ...
    implemented[:] = [item for item in implemented if item.get("validator_id") not in FOUNDATION_IDS]
    for validator_id in FOUNDATION_IDS:
        implemented.append(
            {
    ...
    fixture_paths = {
        "convergence_foundation_schema_only_pass": "tests/validators/fixtures/convergence_foundation_pass.json",
        "convergence_foundation_live_permission_fail": "tests/validators/fixtures/convergence_foundation_fail.json",
        "convergence_foundation_dependency_blocked": "tests/validators/fixtures/convergence_foundation_blocked.json",
    }
    catalog["fixtures"] = [
...
def write_validator_log(now: str, foundation_results: list[dict[str, Any]], guardrail_results: list[dict[str, Any]], fixture_results: list[dict[str, Any]]) -> Path:
    path = ROOT / "system" / "evidence" / "validator_runs" / "MVP4_CONVERGENCE_FOUNDATION.validator_run_log.json"
    write_json(
        path,
        {
            ...
            "created_at_utc": now,
            "patch_id": PATCH_ID,
            "validators_run": foundation_results,
...
def write_stage_gate(now: str, validator_log_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "stage_gates" / "MVP4_CONVERGENCE_FOUNDATION.stage_gate_result.json"
    write_json(
        path,
        {
            ...
            "stage_gate_status": "CONVERGENCE_FOUNDATION_PASS_AS_SCHEMA_ONLY_LIVE_REMAINS_BLOCKED",
...
def write_evidence_manifest(now: str, authority: dict[str, str], validator_log_path: Path, stage_gate_path: Path) -> Path:
    path = ROOT / "system" / "evidence" / "MVP4_CONVERGENCE_FOUNDATION.evidence_manifest.json"
    write_json(
        path,
        {
            ...
            "project_id": "TRADER_1",
            "authority": authority,
            "evidence_manifest_id": "MVP4_CONVERGENCE_FOUNDATION_EVIDENCE",
            "artifact_paths": [
                "trader1/validation/mvp0_validators.py",
                "tools/run_convergence_foundation_validators.py",
                "tests/validators/test_convergence_foundation_validators.py",
                "tests/validators/fixtures/convergence_foundation_pass.json",
                "tests/validators/fixtures/convergence_foundation_fail.json",
                "tests/validators/fixtures/convergence_foundation_blocked.json",
...
def build_patch_result(
    ...
    convergence_results = foundation_results + [
        result
    ...
    patch_result = {
        ...
        "affected_contract_ids": ["REQ-CONV-MVP0-SCAFFOLD"],
        ...
        "new_registry_items": ["convergence foundation validators marked IMPLEMENTED_FAIL_CLOSED"],
        "new_or_changed_schema_ids": [],
        "validators_required": FOUNDATION_IDS + GUARDRAIL_IDS,
        "validators_run": foundation_results + guardrail_results,
        "tests_run": test_results,
        "coverage_unmapped_count": 0,
        "registry_yaml_parse_status": "PASS",
        "registry_placeholders_remaining": [],
        "retained_archive_semantic_mapping_status": "NOT_READ_NOT_REQUIRED_FOR_CONVERGENCE_FOUNDATION_PATCH",
        ...
        "profit_convergence_patch": "FAIL_CLOSED_FOUNDATION_VALIDATOR_PATCH",
        "convergence_layer_changed": False,
        "convergence_state_before": "BLOCKED_FAIL_CLOSED_DEPENDENCY_UNTESTED",
        "convergence_state_after": "BLOCKED_FAIL_CLOSED_REMAINING_DEPENDENCY_UNTESTED",
        "objective_profile_changed": False,
        "memory_schema_changed": False,
        "failure_analysis_required": False,
        "failure_analysis_status": "NOT_REQUIRED_FOR_CONVERGENCE_FOUNDATION_PATCH",
...
def write_patch_and_state(now: str, patch_result: dict[str, Any]) -> None:
    ...
    for validator_id in FOUNDATION_IDS + GUARDRAIL_IDS + ["live_final_guard_validator"]:
...
def main() -> int:
    ...
    foundation_results = run_validators(FOUNDATION_IDS)
    guardrail_results = run_validators(GUARDRAIL_IDS)
    fixture_paths = [
        ROOT / "tests" / "validators" / "fixtures" / "convergence_foundation_pass.json",
        ROOT / "tests" / "validators" / "fixtures" / "convergence_foundation_fail.json",
        ROOT / "tests" / "validators" / "fixtures" / "convergence_foundation_blocked.json",
    ]
    fixture_results = [run_fixture_file(path) for path in fixture_paths]
    validator_log_path = write_validator_log(now, foundation_results, guardrail_results, fixture_results)
    ...
    preliminary = build_patch_result(now, foundation_results, guardrail_results, [], evidence_path, validator_log_path, stage_gate_path)
    ...
    test_results = [
        ...
        run_command([sys.executable, "tools/run_convergence_foundation_validators.py"], "python tools/run_convergence_foundation_validators.py"),
    ...
    fixture_results = [run_fixture_file(path) for path in fixture_paths]
    validator_log_path = write_validator_log(now, foundation_results, guardrail_results, fixture_results)
    final_patch = build_patch_result(now, foundation_results, guardrail_results, test_results, evidence_path, validator_log_path, stage_gate_path)
