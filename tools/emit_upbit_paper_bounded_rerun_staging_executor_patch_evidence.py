...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_BOUNDED_RERUN_STAGING_EXECUTOR"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-BOUNDED-RERUN-STAGING-EXECUTOR"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_LEDGER_ROLLUP_RECONCILIATION"
...
CHANGED_ARTIFACTS = [
    ...
    "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_bounded_rerun_staging_executor_report.json",
    "contracts/security/source_bundle_manifest.json",
    "tools/emit_upbit_paper_bounded_rerun_staging_executor_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_BOUNDED_RERUN_STAGING_EXECUTOR.md",
]

BLOCKERS = [
    "POST_RERUN_LEDGER_ROLLUP_REQUIRED",
...
def write_runtime_report() -> dict[str, Any]:
    guard = load_json(runtime_path("upbit_paper_missing_cycle_rerun_guard_report.json"))
    guard_result = validate_upbit_paper_missing_cycle_rerun_guard_report(guard)
    ...
    report = build_upbit_paper_bounded_rerun_staging_executor_report(
        ...
        executor_id="mvp4-upbit-paper-bounded-rerun-staging-executor",
    )
    ...
    write_upbit_paper_bounded_rerun_staging_executor_report(root=ROOT, report=report)
    return report


def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_BOUNDED_RERUN_STAGING_EXECUTOR.md",
        f"""# MVP4_UPBIT_PAPER_BOUNDED_RERUN_STAGING_EXECUTOR
...
- ready_guard_item_count: {report["ready_guard_item_count"]}
- recovery_guard_blocked_item_count: {report["recovery_guard_blocked_item_count"]}
- eligible_missing_cycle_count: {report["eligible_missing_cycle_count"]}
- staged_cycle_count: {report["staged_cycle_count"]}
- staged_artifact_count: {report["staged_artifact_count"]}
- staging_written_artifact_count: {report["staging_written_artifact_count"]}
- staging_reused_existing_artifact_count: {report["staging_reused_existing_artifact_count"]}
- staged_current_evidence_usable_count: {report["staged_current_evidence_usable_count"]}
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER bounded rerun staging executor",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: bounded PAPER rerun staging must create isolated rerun candidates from "
                "validated missing-cycle guard items without mutating current evidence, live permission, or scale-up state"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER bounded rerun staging executor",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_bounded_rerun_staging_executor.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(b"bounded rerun staging executor remains isolated and live blocked"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_STAGED_CANDIDATES_LIVE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_bounded_rerun_staging_executor.py"],
            "fixture_files": [],
            "runtime_modules": [
                "trader1/runtime/paper/upbit_paper_bounded_rerun_staging_executor.py",
                "trader1/runtime/paper/upbit_paper_missing_cycle_rerun_guard.py",
                "trader1/runtime/paper/upbit_paper_runtime_cycle.py",
            ],
            "evidence_artifacts": [
                "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_bounded_rerun_staging_executor_report.json",
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_STAGED_CANDIDATES_LIVE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_UPBIT_PAPER_MISSING_CYCLE_RERUN_GUARD.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_bounded_rerun_staging_executor_validator"],
            ...
            "active_read_surface_used": [
                ...
                "missing cycle rerun guard",
                "PAPER runtime cycle reports",
                "PAPER ledger event JSONL candidates",
                "live final guard",
            ],
            "task_class": "MVP4_UPBIT_PAPER_BOUNDED_RERUN_STAGING_EXECUTOR",
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_RERUN_CANDIDATES_STAGED_NOT_CURRENT_EVIDENCE",
            "optimizer_guardrail_result": "PASS_STAGING_EXECUTOR_DOES_NOT_EXECUTE_RERUN_OR_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "MISSING_PAPER_CYCLE_RERUN_CANDIDATES_STAGED_ROLLUP_RECONCILIATION_REQUIRED",
            "convergence_guardrail_result": "PASS_NO_LIVE_PERMISSION_NO_CURRENT_EVIDENCE_MUTATION_NO_SCALE_UP",
            "guard_status": report["source_missing_cycle_rerun_guard_status"],
            "guard_item_count": report["source_guard_item_count"],
            "rerun_ready_item_count": report["ready_guard_item_count"],
            "recovery_guard_blocked_item_count": report["recovery_guard_blocked_item_count"],
            "missing_cycle_ledger_jsonl_total_count": report["eligible_missing_cycle_count"] + report["skipped_cycle_count"],
            "planned_staging_artifact_total_count": report["staged_artifact_count"],
            "actual_rerun_executed": False,
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_RERUN_CANDIDATES_STAGED_ROLLUP_RECONCILIATION_REQUIRED",
            "executor_status": report["executor_status"],
            "staging_status": report["staging_status"],
            "ready_guard_item_count": report["ready_guard_item_count"],
            "recovery_guard_blocked_item_count": report["recovery_guard_blocked_item_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Bounded Rerun Staging Executor Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_bounded_rerun_staging_executor_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_bounded_rerun_staging_executor_validator")
    completed = set(state.get("completed_requirement_ids", []))
    completed.add(REQUIREMENT_ID)
    gaps = set(state.get("open_contract_gap_ids", []))
    gaps.discard("BOUNDED_RERUN_STAGING_EXECUTOR_REQUIRED")
    gaps.update(
        {
            "POST_RERUN_LEDGER_ROLLUP_REQUIRED",
            "POST_RERUN_RECONCILIATION_REQUIRED",
