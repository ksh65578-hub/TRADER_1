...
CHANGED_ARTIFACTS = [
    ...
    "contracts/generated/context_pack/MVP4_DASHBOARD_VISIBILITY_LAYOUT_FIX.md",
]
...
def refresh_existing_runtime_dashboard_html() -> list[dict[str, Any]]:
    refreshed: list[dict[str, Any]] = []
    for shell_path in sorted((ROOT / "system" / "runtime").glob("**/dashboard_shell.json")):
        shell = load_json(shell_path)
        html = base.render_dashboard_html(shell) if hasattr(base, "render_dashboard_html") else None
        if html is None:
            ...
            from trader1.dashboard.read_only_dashboard import validate_dashboard_visual_layout_contract

            result = validate_dashboard_visual_layout_contract(html)
        if result.status != "PASS":
            raise RuntimeError(f"dashboard visual layout validation failed for {base.rel(shell_path)}: {result.message}")
        html_path = shell_path.parent / "dashboard" / "index.html"
        html_path.parent.mkdir(parents=True, exist_ok=True)
        base.write_text(html_path, html)
        refreshed.append(
            {
                "dashboard_shell_path": base.rel(shell_path),
                "dashboard_html_path": base.rel(html_path),
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_DASHBOARD_VISIBILITY_LAYOUT_FIX.md",
        f"""# MVP4_DASHBOARD_VISIBILITY_LAYOUT_FIX
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP4-DASHBOARD-VISUAL-LAYOUT-QA", "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL"]
...
The dashboard freshness header now uses bounded Total/PASS/Attention source chips so long source artifact lists cannot collapse the first-screen layout. Full source trace tokens remain in the Source Artifacts table for auditability.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard readability and operator visibility",
            "full_text_marker": f"{REQUIREMENT_ID}: dashboard first-screen source freshness must remain readable when many source artifacts are present",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Dashboard freshness source summary layout fix",
            ...
            "depends_on": [
                "REQ-MVP4-DASHBOARD-VISUAL-LAYOUT-QA",
                "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL",
                "REQ-MVP4-LIVE-FINAL-GUARD",
            ],
            "source_text_sha256": base.sha256_bytes(
                b"dashboard first-screen source freshness must remain readable when many source artifacts are present"
            ),
    ...
    patch_result.update(
        {
            ...
            "optimizer_guardrail_result": "PASS_DASHBOARD_VISIBILITY_LAYOUT_FIXED_LIVE_BLOCKED",
            "optimizer_live_mutation_detected": False,
            "optimizer_live_order_allowed_after": False,
            "convergence_state_before": "LIVE_BLOCKED_DASHBOARD_OPERATOR_VISIBILITY_REPAIR",
            "convergence_state_after": "LIVE_BLOCKED_DASHBOARD_OPERATOR_VISIBILITY_FIXED",
            "convergence_guardrail_result": "PASS_DASHBOARD_VISIBILITY_LAYOUT_FIXED_LIVE_BLOCKED",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_DASHBOARD_VISIBILITY_LAYOUT_FIXED_LIVE_BLOCKED",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    base.write_text(
        ...
        f"""# MVP4 Dashboard Visibility Layout Fix Audit
...
def main() -> int:
    ...
    regenerated.extend(refresh_existing_runtime_dashboard_html())
    ...
    tests_run.append(base.run_command([sys.executable, "tools/run_hygiene_safe_pytest.py", "--", "-q"]))
