...
PATCH_BASENAME = "MVP4_RISK_EXPOSURE_DRAWDOWN_UX_HARDENING"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-RISK-EXPOSURE-DRAWDOWN-UX-HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_risk_exposure_drawdown_ux_patch_evidence.py",
    "contracts/generated/context_pack/RISK_EXPOSURE_DRAWDOWN_UX.md",
    "system/evidence/audit_reports/MVP4_RISK_EXPOSURE_DRAWDOWN_UX_HARDENING_20260429.md",
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "RISK_EXPOSURE_DRAWDOWN_UX.md",
        f"""# RISK_EXPOSURE_DRAWDOWN_UX
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP1-READ-ONLY-DASHBOARD-SHELL"]
...
PAPER dashboards now include a first-screen Risk Exposure panel. The panel shows simulated PAPER equity, exposure, drawdown, open position count, and scale-up blocked state. It is dashboard display truth only and cannot approve live trading.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "Dashboard risk exposure and drawdown visibility",
            "full_text_marker": f"{REQUIREMENT_ID}:SECTION_DASHBOARD_SHELL:Risk exposure and drawdown UX hardening",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Show PAPER risk exposure and drawdown without live permission",
            ...
            "live_affecting": True,
            "read_when": ["DASHBOARD_UX", "RUNTIME_SAFETY_PATCH", "LIVE_BLOCKED_TEST"],
            "depends_on": ["REQ-MVP1-READ-ONLY-DASHBOARD-SHELL", "REQ-MVP4-SCALEUP-SAFETY-BLOCKED"],
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_MONITOR_STALE_SOURCE_WRITER_GUARD.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "affected_contract_ids": [
                ...
                "REQ-MVP4-SCALEUP-SAFETY-BLOCKED",
            ],
            ...
            "next_task_class": "MVP4_EXECUTION_FEEDBACK_RISK_REVIEW_GAP_HARDENING",
            "next_required_section_ids": ["SECTION_EXECUTION_FEEDBACK", "SECTION_RISK_EXPOSURE", "SECTION_DASHBOARD_SHELL"],
            "next_optional_section_ids": ["SECTION_STRATEGY_EVIDENCE", "SECTION_CONVERGENCE_MEMORY"],
            ...
            "active_read_surface_used": ["SECTION_DASHBOARD_SHELL", "SECTION_RISK_EXPOSURE", "SECTION_LIVE_FINAL_GUARD"],
            "task_class": "MVP4_RISK_EXPOSURE_DRAWDOWN_UX_HARDENING",
            "required_section_ids": ["SECTION_DASHBOARD_SHELL", "SECTION_RISK_EXPOSURE", "SECTION_LIVE_FINAL_GUARD"],
            "expanded_section_ids": [
                "TRADER_1:dashboard-operator-ux-active-surface",
                "TRADER_1:risk-exposure-active-surface",
                "AGENTS:dashboard-ux-implementation-guide",
            ],
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_RISK_EXPOSURE_DASHBOARD_UX",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_RISK_EXPOSURE_DRAWDOWN_UX_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Risk Exposure Drawdown UX Hardening Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["next_allowed_task_class"] = "MVP4_EXECUTION_FEEDBACK_RISK_REVIEW_GAP_HARDENING"
...
def main() -> int:
    ...
    artifacts = sorted(set(CHANGED_ARTIFACTS + write_launcher_artifacts()))
    tests_run = [
