...
PATCH_BASENAME = "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK"
...
NEXT_GAP_ID = "REPAIR_CANDIDATE_HASH_MISMATCH_RECONCILIATION_REQUIRED"
...
PREVIOUS_WRITE_BLOCKED_DEPTH_PATCH_RESULT = (
    "system/evidence/patch_results/"
    "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK.patch_result.json"
)
...
CHANGED_ARTIFACTS = [
    ...
    "tests/contract/test_post_rerun_current_evidence_write_blocked_implementation_depth_recheck.py",
    *ROUTE_TEST_ARTIFACTS,
    "tools/emit_post_repair_reconciliation_required_implementation_depth_recheck_patch_evidence.py",
...
def build_depth_report(now: str, trader_hash: str, agents_hash: str) -> dict[str, Any]:
    state = load_json(ROOT / "contracts" / "generated" / "current_implementation_state.json")
    previous_write_blocked_depth = load_json(ROOT / PREVIOUS_WRITE_BLOCKED_DEPTH_PATCH_RESULT)
    ...
    if REQUIREMENT_ID in state.get("completed_requirement_ids", []):
        if state.get("next_allowed_task_class") != NEXT_TASK_CLASS:
            errors.append("completed depth recheck no longer routes to hash-mismatch implementation depth recheck")
    elif state.get("next_allowed_task_class") != "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK":
        errors.append("current state is not routed to post-repair implementation depth recheck")
    if previous_write_blocked_depth.get("next_task_class") != "MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK":
        errors.append("previous write-blocked depth recheck no longer routes to this task")
    ...
    if post_repair.get("primary_blocker_code") != GAP_ID:
    ...
    if GAP_ID not in set(post_repair.get("blocker_codes") or []):
        errors.append("post-repair report lost post-repair blocker")
    if NEXT_GAP_ID not in set(post_repair.get("blocker_codes") or []):
    ...
    status = "PASS_DEPTH_5_POST_REPAIR_RECONCILIATION_REQUIRED_LIVE_BLOCKING" if not errors else "BLOCKED_DEPTH_RECHECK_ERROR"
    return {
        "schema_id": "trader1.post_repair_reconciliation_required_implementation_depth_recheck.v1",
        ...
        "previous_write_blocked_depth_patch_result_path": PREVIOUS_WRITE_BLOCKED_DEPTH_PATCH_RESULT,
        "previous_write_blocked_depth_patch_result_hash": previous_write_blocked_depth.get("result_hash"),
    ...
    write_json(
        CONTRACT_GAP_PATH,
        {
            ...
            "blockers": [
                {
                    "code": GAP_ID,
                    "message": "Post-repair reconciliation is depth-verified as blocked: a repair candidate exists, but the source expected rollup hash mismatch still requires explicit operator reconciliation.",
                }
            ],
...
task_class: MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK
...
included_requirement_ids: ["{REQUIREMENT_ID}", "REQ-MVP4-POST-REPAIR-RECONCILIATION-REQUIRED-RECHECK", "REQ-MVP4-UPBIT-PAPER-POST-REPAIR-RECONCILIATION"]
...
- Confirm post-repair reconciliation remains BLOCKED.
- Confirm repair candidate and operator queue remain review-only.
    ...
    requirements.append(
        {
            ...
            "source_heading": "post-repair reconciliation required implementation depth recheck",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: verify post-repair reconciliation required runtime evidence depth without "
                "current evidence promotion, live permission, or scale-up"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Post-repair reconciliation required implementation depth recheck",
            ...
            "live_affecting": True,
            "read_when": ["MVP4_POST_REPAIR_RECONCILIATION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK"],
            "depends_on": [
    ...
    rows.append(
        {
            ...
            "fixture_files": [],
            "runtime_modules": [
                "trader1/runtime/reconciliation/upbit_paper_post_repair_reconciliation.py",
                "trader1/runtime/reconciliation/upbit_paper_repair_operator_queue.py",
            ],
...
def build_patch_result(
    ...
    if report["status"] != "PASS_DEPTH_5_POST_REPAIR_RECONCILIATION_REQUIRED_LIVE_BLOCKING":
        raise RuntimeError("cannot emit patch_result while post-repair depth recheck is blocked")
    template = load_json(ROOT / PREVIOUS_POST_REPAIR_RECHECK_PATCH_RESULT)
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "stage_gate_status": "PASS_POST_REPAIR_RECONCILIATION_REQUIRED_DEPTH_RECHECK_LIVE_BLOCKING",
    ...
    write_text(
        ...
        f"""# MVP4 Post-Repair Reconciliation Required Implementation Depth Recheck
