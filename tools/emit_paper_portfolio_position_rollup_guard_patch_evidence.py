...
PATCH_BASENAME = "MVP4_PAPER_PORTFOLIO_POSITION_ROLLUP_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260501_001"
REQUIREMENT_ID = "REQ-MVP4-PAPER-PORTFOLIO-POSITION-ROLLUP-GUARD"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_paper_portfolio_position_rollup_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_PAPER_PORTFOLIO_POSITION_ROLLUP_GUARD.md",
]
VALIDATORS_REQUIRED = [
    "schema_validator",
...
def build_audit() -> dict[str, Any]:
    schema_text = (ROOT / "contracts" / "schema" / "paper_portfolio_snapshot.schema.json").read_text(encoding="utf-8")
    portfolio_text = (ROOT / "trader1" / "runtime" / "portfolio" / "paper_portfolio.py").read_text(encoding="utf-8")
    runtime_test_text = (ROOT / "tests" / "integration" / "test_upbit_paper_runtime_cycle.py").read_text(encoding="utf-8")
    portfolio_test_text = (ROOT / "tests" / "runtime" / "test_paper_portfolio.py").read_text(encoding="utf-8")
    validator_text = (ROOT / "trader1" / "validation" / "mvp0_validators.py").read_text(encoding="utf-8")
    checks = {
        "schema_stricts_position_items": '"position"' in schema_text
        and '"additionalProperties": false' in schema_text
        and '"average_entry_price"' in schema_text
        and '"market_value"' in schema_text,
        "validator_checks_position_market_value": "paper position market value arithmetic mismatch" in portfolio_text,
        "validator_checks_position_pnl_rollup": "paper portfolio unrealized PnL rollup mismatch" in portfolio_text,
        "validator_blocks_position_side_drift": "paper position must remain long spot only" in portfolio_text,
        "unit_tests_cover_position_tamper": "test_paper_portfolio_detects_position_market_value_tamper" in portfolio_test_text
        and "test_paper_portfolio_blocks_position_side_drift" in portfolio_test_text,
        "runtime_tests_cover_nested_position_tamper": "test_runtime_blocks_tampered_position_detail_rollup" in runtime_test_text,
        "validator_registry_covers_position_tamper": "paper portfolio position detail tamper was not detected" in validator_text,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    return {
        "audit_schema_id": "trader1.paper_portfolio_position_rollup_guard_audit.v1",
        "status": "PASS" if not blockers else "BLOCKED",
        "blockers": blockers,
        "checks": checks,
        "hidden_defects": [
            {
                "classification": "portfolio_position_false_consistency",
                "condition": "top-level cash/equity totals could be internally valid while nested position market value or PnL fields were stale or tampered",
                "impact": "dashboard could show detailed holding values that do not reconcile with PAPER portfolio totals",
                "fix": "paper portfolio validation now binds each position market value, unrealized PnL, side, source, and rollup totals",
            },
            {
                "classification": "schema_position_shape_too_loose",
                "condition": "paper_portfolio_snapshot schema accepted arbitrary position item shapes",
                "impact": "schema checks could pass while UI-critical position detail fields were missing",
                "fix": "position items now have required fields and additionalProperties=false",
            },
        ],
...
def update_navigation(now: str, trader_hash: str, agents_hash: str, audit: dict[str, Any]) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_PAPER_PORTFOLIO_POSITION_ROLLUP_GUARD.md",
        f"""# MVP4_PAPER_PORTFOLIO_POSITION_ROLLUP_GUARD
...
""",
    ...
    ensure_requirement(
        req_index,
        {
            ...
            "source_heading": "paper portfolio position rollup guard",
            "full_text_marker": f"{REQUIREMENT_ID}:paper portfolio position detail must reconcile to top-level PAPER totals",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "PAPER portfolio position detail must reconcile to rollup totals",
            "requirement_kind": "SCHEMA_VALIDATOR_TEST_PATCH",
            "schema_ids": ["trader1.paper_portfolio_snapshot.v1", "trader1.upbit_paper_runtime_cycle_report.v1"],
            "validator_ids": VALIDATORS_REQUIRED,
            "artifact_ids": CHANGED_ARTIFACTS,
            "test_ids": ["tests/runtime/test_paper_portfolio.py", "tests/integration/test_upbit_paper_runtime_cycle.py"],
            "mvp_stage": "MVP-4",
            "implementation_depth_min": "DEPTH_4_RUNTIME_INTEGRATION",
            "blocking_level": "HIGH",
            "live_affecting": True,
            "read_when": ["SCHEMA_GENERATION", "VALIDATOR_IMPLEMENTATION", "DASHBOARD_UX"],
            "depends_on": ["REQ-MVP4-PAPER-PORTFOLIO-PNL-DETAIL", "REQ-MVP4-UPBIT-PAPER-RUNTIME-E2E-CONSISTENCY-GUARD"],
    ...
    ensure_matrix_row(
        matrix,
        {
            ...
            "schema_files": ["contracts/schema/paper_portfolio_snapshot.schema.json"],
            "validator_files": ["trader1/runtime/portfolio/paper_portfolio.py", "trader1/validation/mvp0_validators.py"],
            "test_files": ["tests/runtime/test_paper_portfolio.py", "tests/integration/test_upbit_paper_runtime_cycle.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/portfolio/paper_portfolio.py", "trader1/runtime/paper/upbit_paper_runtime.py"],
            "evidence_artifacts": [
...
PAPER portfolio position rows now reconcile their market value and unrealized PnL to the top-level simulated portfolio totals before the runtime cycle or dashboard can treat them as valid display truth.
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    state["implemented_schema_ids"] = list(dict.fromkeys([*state.get("implemented_schema_ids", []), "trader1.paper_portfolio_snapshot.v1"]))
    ...
    patch_result.update(
        {
            ...
            "target_mvp_level": "MVP-4",
            "patch_class": "RUNTIME_SAFETY_PATCH",
            "affected_contract_ids": [REQUIREMENT_ID, "REQ-MVP4-PAPER-PORTFOLIO-PNL-DETAIL", "REQ-MVP4-UPBIT-PAPER-RUNTIME-E2E-CONSISTENCY-GUARD"],
            ...
            "new_registry_items": [],
            "new_or_changed_schema_ids": ["trader1.paper_portfolio_snapshot.v1"],
            ...
            "optimizer_patch": "NO_OPTIMIZER_RUNTIME_CHANGE_PAPER_PORTFOLIO_ROLLUP_GUARD",
...
def main() -> int:
    ...
    tests_run = [
        run_command([sys.executable, "-B", "-m", "unittest", "tests.runtime.test_paper_portfolio", "tests.integration.test_upbit_paper_runtime_cycle", "-q"], timeout_seconds=180),
        run_command([sys.executable, "-B", "-m", "unittest", "tests.contract.test_schema_instance_validation", "tests.contract.test_patch_result_runtime_schema_validation", "-q"], timeout_seconds=240),
