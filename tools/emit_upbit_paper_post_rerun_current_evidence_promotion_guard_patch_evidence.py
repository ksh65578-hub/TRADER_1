...
PATCH_BASENAME = "MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_PROMOTION_GUARD"
PATCH_ID = f"{PATCH_BASENAME}_20260502_001"
REQUIREMENT_ID = "REQ-MVP4-UPBIT-PAPER-POST-RERUN-CURRENT-EVIDENCE-PROMOTION-GUARD"
NEXT_TASK_CLASS = "MVP4_UPBIT_PAPER_POST_RERUN_OPERATOR_RECONCILIATION_QUEUE"
...
CHANGED_ARTIFACTS = [
    ...
    "system/runtime/upbit/krw_spot/paper/mvp1_upbit_paper_launcher/paper_runtime/upbit_paper_post_rerun_current_evidence_promotion_guard_report.json",
    "contracts/security/source_bundle_manifest.json",
    "tools/emit_upbit_paper_post_rerun_current_evidence_promotion_guard_patch_evidence.py",
    "contracts/generated/context_pack/MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_PROMOTION_GUARD.md",
]
...
def write_runtime_report() -> dict[str, Any]:
    source = load_json(runtime_path("upbit_paper_post_rerun_ledger_rollup_reconciliation_report.json"))
    source_result = validate_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(source)
    if source_result.status != "PASS":
        raise RuntimeError(
            "source post-rerun ledger rollup reconciliation validation failed: "
    ...
    report = build_upbit_paper_post_rerun_current_evidence_promotion_guard_report(
        root=ROOT,
        post_rerun_reconciliation_report=source,
        guard_id="mvp4-upbit-paper-post-rerun-current-evidence-promotion-guard",
    )
    result = validate_upbit_paper_post_rerun_current_evidence_promotion_guard_report(report)
    if result.status != "PASS":
        raise RuntimeError(
            "post-rerun current evidence promotion guard validation failed: "
            f"{result.status} {result.blocker_code} {result.message}"
        )
    write_upbit_paper_post_rerun_current_evidence_promotion_guard_report(root=ROOT, report=report)
    return report
...
def write_context(now: str, trader_hash: str, agents_hash: str, report: dict[str, Any]) -> None:
    base.write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_PROMOTION_GUARD.md",
        f"""# MVP4_UPBIT_PAPER_POST_RERUN_CURRENT_EVIDENCE_PROMOTION_GUARD
...
runtime_summary:
- promotion_guard_status: {report["promotion_guard_status"]}
- source_post_rerun_ledger_rollup_status: {report["source_post_rerun_ledger_rollup_status"]}
- source_post_rerun_reconciliation_status: {report["source_post_rerun_reconciliation_status"]}
- candidate_item_count: {report["candidate_item_count"]}
- candidate_rollup_verified_count: {report["candidate_rollup_verified_count"]}
...
Upbit PAPER post-rerun candidate rollups now have a review-only promotion guard. The guard verifies candidate hashes and scoped planned current-ledger paths, but it does not write current evidence and keeps live/scale blocked.
    ...
    requirements.append(
        {
            ...
            "source_heading": "Upbit PAPER post-rerun current evidence promotion guard",
            "full_text_marker": (
                f"{REQUIREMENT_ID}: post-rerun PAPER candidate rollups may become review-ready only after guard "
                "verification; current evidence writes, live permission, and scale-up remain blocked"
            ),
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Upbit PAPER post-rerun current evidence promotion guard",
            ...
            "test_ids": ["tests/runtime/test_upbit_paper_post_rerun_current_evidence_promotion_guard.py"],
            "mvp_stage": "MVP-4",
            ...
            "source_text_sha256": base.sha256_bytes(b"post rerun promotion guard verifies candidates but blocks current evidence writes"),
            "source_authority_sha256": trader_hash,
            "implementation_status": "IMPLEMENTED_REVIEW_ONLY_CURRENT_EVIDENCE_WRITE_BLOCKED",
            "test_status": "PASS",
        }
    )
    req_index.update(
    ...
    rows.append(
        {
            ...
            "test_files": ["tests/runtime/test_upbit_paper_post_rerun_current_evidence_promotion_guard.py"],
            "fixture_files": [],
            "runtime_modules": ["trader1/runtime/paper/upbit_paper_post_rerun_current_evidence_promotion_guard.py"],
            "evidence_artifacts": [
            ...
            "live_affecting": True,
            "status": "IMPLEMENTED_REVIEW_ONLY_CURRENT_EVIDENCE_WRITE_BLOCKED",
        }
    )
    matrix.update(
        {
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], report: dict[str, Any]) -> dict[str, Any]:
    template = load_json(
        ...
        / "evidence"
        / "patch_results"
        / "MVP4_UPBIT_PAPER_POST_RERUN_LEDGER_ROLLUP_RECONCILIATION.patch_result.json"
    )
    ...
    patch_result.update(
        {
            ...
            "affected_mode": "PAPER",
            "new_registry_items": [REQUIREMENT_ID, "upbit_paper_post_rerun_current_evidence_promotion_guard_validator"],
            ...
            "optimizer_status_after": "PAPER_SCORECARD_INPUT_ONLY_RERUN_CANDIDATE_PROMOTION_REVIEW_READY_NOT_CURRENT_EVIDENCE",
            "optimizer_guardrail_result": "PASS_PROMOTION_GUARD_DOES_NOT_MUTATE_CURRENT_EVIDENCE",
            "convergence_state_after": "POST_RERUN_PROMOTION_REVIEW_READY_RECONCILIATION_REQUIRED",
            ...
            "post_rerun_promotion_candidate_verified_count": report["candidate_rollup_verified_count"],
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    base.write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_REVIEW_ONLY_PROMOTION_GUARD_CURRENT_EVIDENCE_WRITE_BLOCKED",
            "promotion_guard_status": report["promotion_guard_status"],
            "candidate_item_count": report["candidate_item_count"],
            "candidate_rollup_verified_count": report["candidate_rollup_verified_count"],
            "promotion_review_ready_count": report["promotion_review_ready_count"],
    ...
    base.write_text(
        ...
        f"""# MVP4 Upbit PAPER Post-Rerun Current Evidence Promotion Guard Audit
...
def update_state_and_ledger(now: str, patch_result: dict[str, Any]) -> None:
    ...
    schema_ids.add("trader1.upbit_paper_post_rerun_current_evidence_promotion_guard_report.v1")
    validator_ids = set(state.get("implemented_validator_ids", []))
    validator_ids.add("upbit_paper_post_rerun_current_evidence_promotion_guard_validator")
    completed = set(state.get("completed_requirement_ids", []))
    completed.add(REQUIREMENT_ID)
    gaps = set(state.get("open_contract_gap_ids", []))
    gaps.discard("POST_RERUN_CURRENT_EVIDENCE_PROMOTION_GUARD_REQUIRED")
    gaps.update(
        {
            "POST_RERUN_RECONCILIATION_REQUIRED",
