...
PATCH_BASENAME = "MVP4_OPERATOR_ACTION_SUMMARY_HARDENING"
PATCH_ID = f"{PATCH_BASENAME}_20260429_001"
REQUIREMENT_ID = "REQ-MVP4-OPERATOR-ACTION-SUMMARY-HARDENING"
...
CHANGED_ARTIFACTS = [
    ...
    "tools/emit_operator_action_summary_hardening_patch_evidence.py",
    "contracts/generated/context_pack/OPERATOR_ACTION_SUMMARY.md",
    "system/evidence/audit_reports/MVP4_OPERATOR_ACTION_SUMMARY_HARDENING_20260429.md",
]
...
def update_context(now: str, trader_hash: str, agents_hash: str) -> None:
    write_text(
        ROOT / "contracts" / "generated" / "context_pack" / "OPERATOR_ACTION_SUMMARY.md",
        f"""# OPERATOR_ACTION_SUMMARY
...
Dashboard now includes an Operator Next Action summary on the first screen. It translates dashboard state into one safe user action while keeping PAPER review, LIVE blocking, no-order-controls, and emergency visibility explicit.
""",
    )
    ...
    requirements.append(
        {
            ...
            "source_heading": "Operator next action summary",
            "full_text_marker": f"{REQUIREMENT_ID}:SECTION_DASHBOARD_SHELL:Operator action visible without live permission",
            "authority_level": "ACTIVE_AUTHORITY",
            "requirement_title": "Show one safe next operator action while blocking live permission",
...
def build_patch_result(now: str, tests_run: list[dict[str, Any]], validators_run: list[dict[str, Any]], artifacts: list[str]) -> dict[str, Any]:
    template = load_json(ROOT / "system" / "evidence" / "patch_results" / "MVP4_LONG_RUN_OPERATOR_SUMMARY_HARDENING.patch_result.json")
    ...
    patch_result.update(
        {
            ...
            "failure_analysis_status": "NOT_REQUIRED_FOR_OPERATOR_ACTION_SUMMARY",
...
def write_evidence(now: str, trader_hash: str, agents_hash: str, patch_result: dict[str, Any]) -> None:
    ...
    write_json(
        ROOT / patch_result["stage_gate_result_path"],
        {
            ...
            "patch_id": PATCH_ID,
            "target_mvp_level": "MVP-4",
            "stage_gate_status": "PASS_FOR_OPERATOR_ACTION_SUMMARY_NO_LIVE_ORDERS",
            "live_order_ready": False,
            "live_order_allowed": False,
            "can_live_trade": False,
            "scale_up_allowed": False,
    ...
    write_text(
        ...
        f"""# MVP4 Operator Action Summary Hardening Audit
