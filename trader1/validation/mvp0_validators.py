...
from trader1.runtime.paper.upbit_paper_missing_cycle_rerun_guard import (
    MISSING_CYCLE_RERUN_GUARD_BLOCKER_CODE,
...
def upbit_paper_stale_loop_replacement_schema_normalization_preview_validator() -> ValidatorResult:
    validator_id = "upbit_paper_stale_loop_replacement_schema_normalization_preview_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_stale_loop_replacement_schema_normalization_preview_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_stale_loop_replacement_schema_normalization_preview.py"
    ledger_preview_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_stale_loop_ledger_recheck_preview.py"
    persistent_loop_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_persistent_loop.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_stale_loop_replacement_schema_normalization_preview.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_stale_loop_replacement_schema_normalization_preview_report.json"
        )
    )
    paths = [schema_path, module_path, ledger_preview_module_path, persistent_loop_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_stale_loop_replacement_schema_normalization_preview_report.v1":
        return fail_result(validator_id, "replacement schema normalization preview schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "replacement schema normalization preview schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(validator_id, f"replacement schema normalization preview schema missing required field: {field}", paths, "SCHEMA_IDENTITY_MISMATCH")
    ...
    if not preview_path.exists():
        return fail_result(validator_id, "replacement schema normalization source preview is missing", paths + [preview_path], "MEASUREMENT_MISSING")
    ledger_preview = load_json(preview_path)
    report = build_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(
        root=ROOT,
        ledger_recheck_preview_report=ledger_preview,
    )
    result = validate_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(report)
    if result.status != "PASS":
        return fail_result(validator_id, f"valid replacement schema normalization preview failed: {result.message}", paths, result.blocker_code or "UNKNOWN_BLOCKED")
    if (
        report.get("preview_status") != "BLOCKED"
        ...
        or report.get("normalization_write_allowed_count") != 0
        or report.get("current_evidence_write_allowed_count") != 0
    ):
        return fail_result(validator_id, "replacement schema normalization preview did not preserve expected blocked counts", paths, "SCHEMA_IDENTITY_MISMATCH")

    live_mutation = json.loads(json.dumps(report))
    live_mutation["live_order_allowed"] = True
    live_mutation["normalization_preview_hash"] = upbit_paper_stale_loop_replacement_schema_normalization_preview_hash(live_mutation)
    live_result = validate_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(live_mutation)
    if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(validator_id, "replacement schema normalization preview live mutation was not blocked", paths, live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    false_write = json.loads(json.dumps(report))
    false_write["normalization_write_allowed_count"] = 1
    false_write["normalization_preview_hash"] = upbit_paper_stale_loop_replacement_schema_normalization_preview_hash(false_write)
    false_write_result = validate_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(false_write)
    if false_write_result.status != "BLOCKED" or false_write_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(validator_id, "replacement schema normalization preview allowed write count drift", paths, false_write_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    false_ready = json.loads(json.dumps(report))
    false_ready["items"][0]["normalized_validation_blocker_code"] = "SCHEMA_IDENTITY_MISMATCH"
    false_ready["normalization_preview_hash"] = upbit_paper_stale_loop_replacement_schema_normalization_preview_hash(false_ready)
    false_ready_result = validate_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(false_ready)
    if false_ready_result.status != "FAIL":
        return fail_result(validator_id, "replacement schema normalization preview allowed READY item with schema mismatch", paths, false_ready_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")

    with TemporaryDirectory() as tmp:
        written_path = write_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(root=Path(tmp), report=report)
        if not written_path.exists():
            return fail_result(validator_id, "replacement schema normalization preview writer did not create report artifact", paths, "MEASUREMENT_MISSING")

    for runtime_path in runtime_report_paths:
        try:
            runtime_report = load_json(runtime_path)
        except Exception as exc:
            return fail_result(validator_id, f"runtime replacement schema normalization preview artifact is not valid json: {rel(runtime_path)}: {exc}", paths, "SCHEMA_IDENTITY_MISMATCH")
        runtime_result = validate_upbit_paper_stale_loop_replacement_schema_normalization_preview_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime replacement schema normalization preview artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        validator_id,
        "Upbit PAPER stale-loop replacement schema normalization preview removes schema mismatch in memory while keeping replacement writes and current evidence blocked",
        paths,
    )


def upbit_paper_stale_loop_normalized_reconciliation_preview_validator() -> ValidatorResult:
    validator_id = "upbit_paper_stale_loop_normalized_reconciliation_preview_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_stale_loop_normalized_reconciliation_preview_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_stale_loop_normalized_reconciliation_preview.py"
    normalization_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_stale_loop_replacement_schema_normalization_preview.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_stale_loop_normalized_reconciliation_preview.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_stale_loop_normalized_reconciliation_preview_report.json"
        )
    )
    paths = [schema_path, module_path, normalization_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_stale_loop_normalized_reconciliation_preview_report.v1":
        return fail_result(validator_id, "normalized reconciliation preview schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "normalized reconciliation preview schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
    ...
    if result.status != "PASS":
        return fail_result(validator_id, f"valid normalized reconciliation preview failed: {result.message}", paths, result.blocker_code or "UNKNOWN_BLOCKED")
    if (
        report.get("preview_status") != "BLOCKED"
        or report.get("primary_blocker_code") != NORMALIZED_RECONCILIATION_REQUIRED_BLOCKER_CODE
        or report.get("normalized_reconciliation_candidate_count") != 5
        or report.get("schema_normalization_resolved_count") != 5
        or report.get("reconciliation_required_count") != 5
        or report.get("schema_mismatch_after_normalization_count") != 0
        ...
        return fail_result(validator_id, "normalized reconciliation preview did not preserve expected blocked counts", paths, "SCHEMA_IDENTITY_MISMATCH")

    live_mutation = json.loads(json.dumps(report))
    live_mutation["live_order_allowed"] = True
    live_mutation["normalized_reconciliation_preview_hash"] = upbit_paper_stale_loop_normalized_reconciliation_preview_hash(live_mutation)
    live_result = validate_upbit_paper_stale_loop_normalized_reconciliation_preview_report(live_mutation)
    if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(validator_id, "normalized reconciliation preview live mutation was not blocked", paths, live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    false_write = json.loads(json.dumps(report))
    false_write["reconciliation_write_allowed_count"] = 1
    false_write["normalized_reconciliation_preview_hash"] = upbit_paper_stale_loop_normalized_reconciliation_preview_hash(false_write)
    false_write_result = validate_upbit_paper_stale_loop_normalized_reconciliation_preview_report(false_write)
    if false_write_result.status != "BLOCKED" or false_write_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(validator_id, "normalized reconciliation preview allowed write count drift", paths, false_write_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    false_required = json.loads(json.dumps(report))
    false_required["items"][0]["normalized_validation_blocker_code"] = "OTHER_BLOCKER"
    false_required["normalized_reconciliation_preview_hash"] = upbit_paper_stale_loop_normalized_reconciliation_preview_hash(false_required)
    false_required_result = validate_upbit_paper_stale_loop_normalized_reconciliation_preview_report(false_required)
    if false_required_result.status != "FAIL":
        return fail_result(validator_id, "normalized reconciliation preview allowed inconsistent reconciliation blocker", paths, false_required_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")

    with TemporaryDirectory() as tmp:
        written_path = write_upbit_paper_stale_loop_normalized_reconciliation_preview_report(root=Path(tmp), report=report)
        if not written_path.exists():
            return fail_result(validator_id, "normalized reconciliation preview writer did not create report artifact", paths, "MEASUREMENT_MISSING")

    for runtime_path in runtime_report_paths:
        try:
            runtime_report = load_json(runtime_path)
        except Exception as exc:
            return fail_result(validator_id, f"runtime normalized reconciliation preview artifact is not valid json: {rel(runtime_path)}: {exc}", paths, "SCHEMA_IDENTITY_MISMATCH")
        runtime_result = validate_upbit_paper_stale_loop_normalized_reconciliation_preview_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime normalized reconciliation preview artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
...
def upbit_paper_repair_operator_queue_validator() -> ValidatorResult:
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_repair_operator_queue_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_repair_operator_queue.py"
    plan_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_blocked_repair_plan.py"
    repair_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_ledger_rollup_repair.py"
    post_repair_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_repair_reconciliation.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_repair_operator_queue.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob("*/paper_runtime/upbit_paper_repair_operator_queue_report.json")
    )
    paths = [schema_path, module_path, plan_module_path, repair_module_path, post_repair_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_repair_operator_queue_report.v1":
        return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result("upbit_paper_repair_operator_queue_validator", f"repair operator queue schema missing required field: {field}", paths, "SCHEMA_IDENTITY_MISMATCH")

    with TemporaryDirectory() as tmp:
        root = Path(tmp)
        current = run_upbit_paper_persistent_loop(root=root, loop_id="validator-repair-queue-current", requested_cycle_count=1)
        legacy = json.loads(json.dumps(current))
        ...
        loop_path = (
            ...
            / "mvp1_upbit_paper_launcher"
            / "paper_runtime"
            / "validator-repair-queue-legacy.persistent_loop_report.json"
        )
        loop_path.write_text(json.dumps(legacy, indent=2), encoding="utf-8")
        reconciliation = build_upbit_paper_stale_loop_reconciliation_report(root=root)
        plan = build_upbit_paper_stale_loop_regeneration_plan(root=root, reconciliation_report=reconciliation)
        ...
        if result.status != "PASS":
            return fail_result("upbit_paper_repair_operator_queue_validator", f"valid repair operator queue failed: {result.message}", paths, result.blocker_code or "UNKNOWN_BLOCKED")
        if (
            report.get("queue_status") != "BLOCKED"
            or report.get("primary_blocker_code") != REPAIR_OPERATOR_QUEUE_BLOCKER_CODE
            or report.get("queue_item_count") != 1
            or report.get("ledger_candidate_review_ready_count") != 1
            or report.get("candidate_current_evidence_usable_count") != 0
            or report.get("current_evidence_mutation_allowed")
            or report.get("live_order_allowed")
            or report.get("scale_up_allowed")
            or not report["items"][0].get("ready_for_operator_ledger_candidate_review")
            or not report["items"][0].get("requires_hash_operator_reconciliation")
            or report["items"][0].get("candidate_current_evidence_usable")
        ):
            return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue did not prioritize blocked ledger candidate while staying live-blocked", paths, "MEASUREMENT_MISSING")
        written_path = write_upbit_paper_repair_operator_queue_report(root=root, report=report)
        if not written_path.exists():
            return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue writer did not create report", paths, "MEASUREMENT_MISSING")

        count_tamper = json.loads(json.dumps(report))
        count_tamper["ledger_candidate_review_ready_count"] = 0
        count_tamper["queue_hash"] = upbit_paper_repair_operator_queue_hash(count_tamper)
        count_result = validate_upbit_paper_repair_operator_queue_report(count_tamper)
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue count tamper was not rejected", paths, count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")

        live_mutation = json.loads(json.dumps(report))
        live_mutation["live_order_allowed"] = True
        live_mutation["queue_hash"] = upbit_paper_repair_operator_queue_hash(live_mutation)
        live_result = validate_upbit_paper_repair_operator_queue_report(live_mutation)
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result("upbit_paper_repair_operator_queue_validator", "repair operator queue live mutation was not blocked", paths, live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    for runtime_path in runtime_report_paths:
        try:
            runtime_report = load_json(runtime_path)
        except Exception as exc:
            return fail_result("upbit_paper_repair_operator_queue_validator", f"runtime repair operator queue artifact is not valid json: {rel(runtime_path)}: {exc}", paths, "SCHEMA_IDENTITY_MISMATCH")
        runtime_result = validate_upbit_paper_repair_operator_queue_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                "upbit_paper_repair_operator_queue_validator",
                f"runtime repair operator queue artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
...
def upbit_paper_missing_cycle_rerun_guard_validator() -> ValidatorResult:
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_missing_cycle_rerun_guard_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_missing_cycle_rerun_guard.py"
    queue_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_repair_operator_queue.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_missing_cycle_rerun_guard.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob("*/paper_runtime/upbit_paper_missing_cycle_rerun_guard_report.json")
    )
    paths = [schema_path, module_path, queue_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_missing_cycle_rerun_guard_report.v1":
        return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", f"missing cycle rerun guard schema missing required field: {field}", paths, "SCHEMA_IDENTITY_MISMATCH")

    with TemporaryDirectory() as tmp:
        root = Path(tmp)
        current = run_upbit_paper_persistent_loop(root=root, loop_id="validator-rerun-guard-current", requested_cycle_count=1)
        legacy = json.loads(json.dumps(current))
        legacy["loop_id"] = "validator-rerun-guard-legacy"
        for field in (
            "paper_ledger_rollup_status",
            "paper_ledger_rollup_hash",
            "paper_ledger_rollup_primary_blocker_code",
        ...
        loop_path = (
            ...
            / "mvp1_upbit_paper_launcher"
            / "paper_runtime"
            / "validator-rerun-guard-legacy.persistent_loop_report.json"
        )
        loop_path.write_text(json.dumps(legacy, indent=2), encoding="utf-8")
        reconciliation = build_upbit_paper_stale_loop_reconciliation_report(root=root)
        plan = build_upbit_paper_stale_loop_regeneration_plan(root=root, reconciliation_report=reconciliation)
        ...
        report = build_upbit_paper_missing_cycle_rerun_guard_report(root=root, repair_operator_queue_report=queue)
        result = validate_upbit_paper_missing_cycle_rerun_guard_report(report)
        if result.status != "PASS":
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", f"valid missing cycle rerun guard failed: {result.message}", paths, result.blocker_code or "UNKNOWN_BLOCKED")
        if (
            report.get("guard_status") != "BLOCKED"
            or report.get("primary_blocker_code") != MISSING_CYCLE_RERUN_GUARD_BLOCKER_CODE
            or report.get("guard_item_count") != 1
            or report.get("rerun_ready_item_count") != 1
            or report.get("missing_cycle_ledger_jsonl_total_count") != 1
            or report.get("planned_staging_artifact_total_count") != 3
            or report.get("actual_rerun_executed")
            or report.get("current_ledger_jsonl_write_allowed")
            or report.get("live_order_allowed")
            or report.get("scale_up_allowed")
            or not report["items"][0].get("next_patch_staging_rerun_candidate_eligible")
            or not report["items"][0].get("missing_cycle_identity_match")
            or report["items"][0].get("candidate_current_evidence_usable")
        ):
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard did not expose staging-eligible rerun while staying live-blocked", paths, "MEASUREMENT_MISSING")
        written_path = write_upbit_paper_missing_cycle_rerun_guard_report(root=root, report=report)
        if not written_path.exists():
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard writer did not create report", paths, "MEASUREMENT_MISSING")
        ...
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard count tamper was not rejected", paths, count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")
        ...
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", "missing cycle rerun guard live mutation was not blocked", paths, live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    for runtime_path in runtime_report_paths:
        try:
            runtime_report = load_json(runtime_path)
        except Exception as exc:
            return fail_result("upbit_paper_missing_cycle_rerun_guard_validator", f"runtime missing cycle rerun guard artifact is not valid json: {rel(runtime_path)}: {exc}", paths, "SCHEMA_IDENTITY_MISMATCH")
        runtime_result = validate_upbit_paper_missing_cycle_rerun_guard_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                "upbit_paper_missing_cycle_rerun_guard_validator",
                f"runtime missing cycle rerun guard artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
...
def upbit_paper_post_rerun_ledger_rollup_reconciliation_validator() -> ValidatorResult:
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_ledger_rollup_reconciliation_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_ledger_rollup_reconciliation.py"
    staging_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_bounded_rerun_staging_executor.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_ledger_rollup_reconciliation.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob("*/paper_runtime/upbit_paper_post_rerun_ledger_rollup_reconciliation_report.json")
    )
    paths = [schema_path, module_path, staging_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_ledger_rollup_reconciliation_report.v1":
        return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun reconciliation schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun reconciliation schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", f"post-rerun reconciliation schema missing required field: {field}", paths, "SCHEMA_IDENTITY_MISMATCH")
    ...
    with TemporaryDirectory() as tmp:
        ...
        if result.status != "PASS":
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", f"valid post-rerun reconciliation failed: {result.message}", paths, result.blocker_code or "UNKNOWN_BLOCKED")
        if (
            report.get("post_rerun_ledger_rollup_status") != "PASS"
            or report.get("post_rerun_reconciliation_status") != "BLOCKED"
            ...
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun reconciliation did not produce staged-only candidate rollup while staying live-blocked", paths, "MEASUREMENT_MISSING")
        if current_ledger_path.exists():
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun reconciliation wrote current ledger JSONL", paths, "LIVE_FINAL_GUARD_FAILED")
        candidate_path = root.joinpath(*report["items"][0]["candidate_rollup_artifact_path"].split("/"))
        if "/rerun_candidates_post_rollup/" not in report["items"][0]["candidate_rollup_artifact_path"] or not candidate_path.exists():
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun candidate rollup artifact missing or outside post-rollup namespace", paths, "SNAPSHOT_SCOPE_MISMATCH")
        second = build_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(root=root, staging_executor_report=staging)
        second_result = validate_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(second)
        if second_result.status != "PASS" or second.get("candidate_rollup_reused_existing_count") != 1:
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun candidate rollup is not idempotent for matching artifacts", paths, second_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")
        written_path = write_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(root=root, report=report)
        if not written_path.exists():
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun reconciliation writer did not create report", paths, "MEASUREMENT_MISSING")
        ...
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun count tamper was not rejected", paths, count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH")
        ...
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", "post-rerun live mutation was not blocked", paths, live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED")

    for runtime_path in runtime_report_paths:
        try:
            runtime_report = load_json(runtime_path)
        except Exception as exc:
            return fail_result("upbit_paper_post_rerun_ledger_rollup_reconciliation_validator", f"runtime post-rerun reconciliation artifact is not valid json: {rel(runtime_path)}: {exc}", paths, "SCHEMA_IDENTITY_MISMATCH")
        runtime_result = validate_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_ledger_rollup_reconciliation_validator",
                f"runtime post-rerun reconciliation artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        "upbit_paper_post_rerun_ledger_rollup_reconciliation_validator",
        "Upbit PAPER post-rerun ledger rollup reconciliation writes staged-only candidate rollups and blocks current evidence mutation",
        paths,
    )


def upbit_paper_post_rerun_current_evidence_promotion_guard_validator() -> ValidatorResult:
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_current_evidence_promotion_guard_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_current_evidence_promotion_guard.py"
    source_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_ledger_rollup_reconciliation.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_current_evidence_promotion_guard.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_current_evidence_promotion_guard_report.json"
        )
    )
    paths = [schema_path, module_path, source_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_current_evidence_promotion_guard_report.v1":
        return fail_result(
            "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
            "post-rerun current evidence promotion guard schema_id mismatch",
            paths,
            "SCHEMA_IDENTITY_MISMATCH",
        )
    if schema.get("additionalProperties") is not False:
        return fail_result(
            "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
            "post-rerun current evidence promotion guard schema must be strict",
            paths,
            "SCHEMA_IDENTITY_MISMATCH",
        )
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                f"post-rerun current evidence promotion guard schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    with TemporaryDirectory() as tmp:
        ...
        source_result = validate_upbit_paper_post_rerun_ledger_rollup_reconciliation_report(source_report)
        if source_result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                f"valid source post-rerun reconciliation failed: {source_result.message}",
                paths,
        ...
        if result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                f"valid post-rerun current evidence promotion guard failed: {result.message}",
                paths,
                result.blocker_code or "UNKNOWN_BLOCKED",
            )
        if (
            ...
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "post-rerun current evidence promotion guard did not verify candidates while staying write/live blocked",
                paths,
                "MEASUREMENT_MISSING",
            )
        source_path.unlink()
        ...
        if current_ledger_path.exists():
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "post-rerun current evidence promotion guard wrote current ledger JSONL",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        guard_item = report["items"][0]
        if (
            guard_item.get("promotion_review_status") != "REVIEW_READY_WRITE_BLOCKED"
            or guard_item.get("item_blocker_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or guard_item.get("current_evidence_write_allowed")
            or guard_item.get("candidate_current_evidence_usable")
        ):
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "verified promotion item did not remain write-blocked",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        ...
        if not written_path.exists():
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "post-rerun current evidence promotion guard writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )

        count_tamper = json.loads(json.dumps(report))
        count_tamper["current_evidence_write_allowed_count"] = 1
        count_tamper["promotion_guard_hash"] = upbit_paper_post_rerun_current_evidence_promotion_guard_hash(count_tamper)
        count_result = validate_upbit_paper_post_rerun_current_evidence_promotion_guard_report(count_tamper)
        if count_result.status != "BLOCKED" or count_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "current evidence write count tamper was not blocked",
                paths,
                count_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
            )
        ...
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "post-rerun current evidence promotion guard live mutation was not blocked",
                paths,
        ...
        path_escape["promotion_guard_hash"] = upbit_paper_post_rerun_current_evidence_promotion_guard_hash(path_escape)
        path_result = validate_upbit_paper_post_rerun_current_evidence_promotion_guard_report(path_escape)
        if path_result.status != "BLOCKED" or path_result.blocker_code != "SNAPSHOT_SCOPE_MISMATCH":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                "post-rerun promotion guard path escape was not blocked",
                paths,
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                f"runtime post-rerun current evidence promotion guard artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_current_evidence_promotion_guard_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
                f"runtime post-rerun current evidence promotion guard artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        "upbit_paper_post_rerun_current_evidence_promotion_guard_validator",
        "Upbit PAPER post-rerun current evidence promotion guard verifies candidate rollups while keeping current evidence writes and live/scale blocked",
        paths,
    )


def upbit_paper_post_rerun_operator_reconciliation_queue_validator() -> ValidatorResult:
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_operator_reconciliation_queue_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_operator_reconciliation_queue.py"
    source_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_current_evidence_promotion_guard.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_operator_reconciliation_queue.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_operator_reconciliation_queue_report.json"
        )
    )
    paths = [schema_path, module_path, source_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_operator_reconciliation_queue_report.v1":
        return fail_result(
            "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
            "post-rerun operator reconciliation queue schema_id mismatch",
            paths,
            "SCHEMA_IDENTITY_MISMATCH",
        )
    if schema.get("additionalProperties") is not False:
        return fail_result(
            "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
            "post-rerun operator reconciliation queue schema must be strict",
            paths,
            "SCHEMA_IDENTITY_MISMATCH",
        )
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                f"post-rerun operator reconciliation queue schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    with TemporaryDirectory() as tmp:
        ...
        guard_result = validate_upbit_paper_post_rerun_current_evidence_promotion_guard_report(promotion_guard)
        if guard_result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                f"valid source promotion guard failed: {guard_result.message}",
                paths,
                guard_result.blocker_code or "UNKNOWN_BLOCKED",
            )
        source_guard_path = write_upbit_paper_post_rerun_current_evidence_promotion_guard_report(root=root, report=promotion_guard)
        ...
        if result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                f"valid post-rerun operator reconciliation queue failed: {result.message}",
                paths,
                result.blocker_code or "UNKNOWN_BLOCKED",
            )
        if (
            report.get("queue_status") != "BLOCKED"
            or report.get("primary_blocker_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or report.get("source_promotion_guard_file_load_status") != "PASS"
            or report.get("source_promotion_guard_file_hash") != report.get("source_promotion_guard_hash")
            ...
            or report.get("review_ready_reconciliation_item_count") != 1
            or report.get("current_evidence_write_allowed_count") != 0
            or report.get("candidate_current_evidence_usable_count") != 0
            or report.get("current_evidence_write_allowed")
            or report.get("current_ledger_jsonl_write_allowed")
            ...
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue did not preserve review-only write-blocked state",
                paths,
                "MEASUREMENT_MISSING",
            )
        source_guard_path.unlink()
        missing_source_report = build_upbit_paper_post_rerun_operator_reconciliation_queue_report(
            root=root,
            promotion_guard_report=promotion_guard,
        )
        missing_source_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(missing_source_report)
        if (
            missing_source_result.status != "BLOCKED"
            or missing_source_result.blocker_code != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or missing_source_report.get("source_promotion_guard_file_load_status") != "MISSING"
        ):
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "missing source promotion guard artifact was not blocked",
                paths,
                missing_source_result.blocker_code or POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE,
            )
        ...
        if current_ledger_path.exists():
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue wrote current ledger JSONL",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        queue_item = report["items"][0]
        if (
            queue_item.get("operator_reconciliation_status") != "REVIEW_READY_RECONCILIATION_REQUIRED"
            or queue_item.get("source_guard_item_blocker_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or queue_item.get("current_evidence_write_allowed")
            or queue_item.get("candidate_current_evidence_usable")
        ):
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue item did not remain reconciliation-blocked",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        written_path = write_upbit_paper_post_rerun_operator_reconciliation_queue_report(root=root, report=report)
        if not written_path.exists():
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )

        count_tamper = json.loads(json.dumps(report))
        count_tamper["operator_reconciliation_required_count"] = 0
        count_tamper["queue_hash"] = upbit_paper_post_rerun_operator_reconciliation_queue_hash(count_tamper)
        count_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(count_tamper)
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "operator reconciliation count tamper was not rejected",
                paths,
        ...
        live_mutation = json.loads(json.dumps(report))
        live_mutation["live_order_allowed"] = True
        live_mutation["queue_hash"] = upbit_paper_post_rerun_operator_reconciliation_queue_hash(live_mutation)
        live_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(live_mutation)
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue live mutation was not blocked",
                paths,
                live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
            )
        ...
        path_escape = json.loads(json.dumps(report))
        path_escape["items"][0]["planned_current_ledger_jsonl_path"] = "system/runtime/upbit/krw_spot/live/bad.paper_ledger_events.jsonl"
        path_escape["queue_hash"] = upbit_paper_post_rerun_operator_reconciliation_queue_hash(path_escape)
        path_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(path_escape)
        if path_result.status != "BLOCKED" or path_result.blocker_code != "SNAPSHOT_SCOPE_MISMATCH":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                "post-rerun operator queue path escape was not blocked",
                paths,
                path_result.blocker_code or "SNAPSHOT_SCOPE_MISMATCH",
            )
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                f"runtime post-rerun operator queue artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
                f"runtime post-rerun operator queue artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        "upbit_paper_post_rerun_operator_reconciliation_queue_validator",
        "Upbit PAPER post-rerun operator reconciliation queue exposes review-ready candidates while keeping current evidence writes and live/scale blocked",
        paths,
    )


def upbit_paper_post_rerun_reconciliation_decision_audit_validator() -> ValidatorResult:
    validator_id = "upbit_paper_post_rerun_reconciliation_decision_audit_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_reconciliation_decision_audit_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_reconciliation_decision_audit.py"
    source_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_operator_reconciliation_queue.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_reconciliation_decision_audit.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_reconciliation_decision_audit_report.json"
        )
    )
    paths = [schema_path, module_path, source_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_reconciliation_decision_audit_report.v1":
        return fail_result(validator_id, "post-rerun reconciliation decision audit schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "post-rerun reconciliation decision audit schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                validator_id,
                f"post-rerun reconciliation decision audit schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    with TemporaryDirectory() as tmp:
        ...
        queue_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(operator_queue)
        if queue_result.status != "PASS":
            return fail_result(
                validator_id,
                f"valid source operator queue failed: {queue_result.message}",
                paths,
        ...
        if result.status != "PASS":
            return fail_result(
                validator_id,
                f"valid post-rerun reconciliation decision audit failed: {result.message}",
                paths,
                result.blocker_code or "UNKNOWN_BLOCKED",
            )
        if (
            ...
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit did not preserve write-denied live-blocked state",
                paths,
                "MEASUREMENT_MISSING",
            )
        ...
        if current_ledger_path.exists():
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit wrote current ledger JSONL",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        decision_item = report["items"][0]
        if (
            decision_item.get("decision_status") != "WRITE_DENIED_RECONCILIATION_REQUIRED"
            or decision_item.get("decision_reason_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or decision_item.get("current_evidence_write_authorized")
            or decision_item.get("current_evidence_write_allowed")
            or decision_item.get("candidate_current_evidence_usable")
        ):
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit item did not remain write-denied",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        written_path = write_upbit_paper_post_rerun_reconciliation_decision_audit_report(root=root, report=report)
        if not written_path.exists():
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )

        count_tamper = json.loads(json.dumps(report))
        count_tamper["write_denied_count"] = 0
        count_tamper["decision_audit_hash"] = upbit_paper_post_rerun_reconciliation_decision_audit_hash(count_tamper)
        count_result = validate_upbit_paper_post_rerun_reconciliation_decision_audit_report(count_tamper)
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result(
                validator_id,
                "decision audit write-denied count tamper was not rejected",
                paths,
                count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH",
            )
        ...
        live_mutation = json.loads(json.dumps(report))
        live_mutation["live_order_allowed"] = True
        live_mutation["decision_audit_hash"] = upbit_paper_post_rerun_reconciliation_decision_audit_hash(live_mutation)
        live_result = validate_upbit_paper_post_rerun_reconciliation_decision_audit_report(live_mutation)
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit live mutation was not blocked",
                paths,
                live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
            )
        ...
        path_escape = json.loads(json.dumps(report))
        path_escape["items"][0]["planned_current_ledger_jsonl_path"] = "system/runtime/upbit/krw_spot/live/bad.paper_ledger_events.jsonl"
        path_escape["decision_audit_hash"] = upbit_paper_post_rerun_reconciliation_decision_audit_hash(path_escape)
        path_result = validate_upbit_paper_post_rerun_reconciliation_decision_audit_report(path_escape)
        if path_result.status != "BLOCKED" or path_result.blocker_code != "SNAPSHOT_SCOPE_MISMATCH":
            return fail_result(
                validator_id,
                "post-rerun reconciliation decision audit path escape was not blocked",
                paths,
                path_result.blocker_code or "SNAPSHOT_SCOPE_MISMATCH",
            )
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                validator_id,
                f"runtime post-rerun reconciliation decision audit artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_reconciliation_decision_audit_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime post-rerun reconciliation decision audit artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        validator_id,
        "Upbit PAPER post-rerun reconciliation decision audit denies current evidence writes while keeping live/scale blocked",
        paths,
    )


def upbit_paper_post_rerun_reconciliation_blocker_rollup_validator() -> ValidatorResult:
    validator_id = "upbit_paper_post_rerun_reconciliation_blocker_rollup_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_reconciliation_blocker_rollup_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_reconciliation_blocker_rollup.py"
    source_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_reconciliation_decision_audit.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_reconciliation_blocker_rollup.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_reconciliation_blocker_rollup_report.json"
        )
    )
    paths = [schema_path, module_path, source_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_reconciliation_blocker_rollup_report.v1":
        return fail_result(validator_id, "post-rerun reconciliation blocker rollup schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "post-rerun reconciliation blocker rollup schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                validator_id,
                f"post-rerun reconciliation blocker rollup schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    with TemporaryDirectory() as tmp:
        ...
        if decision_result.status != "PASS":
            return fail_result(
                validator_id,
                f"valid source decision audit failed: {decision_result.message}",
                paths,
        ...
        if result.status != "PASS":
            return fail_result(
                validator_id,
                f"valid post-rerun reconciliation blocker rollup failed: {result.message}",
                paths,
                result.blocker_code or "UNKNOWN_BLOCKED",
            )
        if (
            ...
            or report.get("unique_blocker_count", 0) < 1
            or report.get("current_evidence_write_authorized_count") != 0
            or report.get("current_evidence_write_allowed_count") != 0
            or report.get("candidate_current_evidence_usable_count") != 0
            or report.get("current_evidence_write_allowed")
            ...
            return fail_result(
                validator_id,
                "post-rerun reconciliation blocker rollup did not preserve blocked state",
                paths,
                "MEASUREMENT_MISSING",
            )
        if current_ledger_path.exists():
            return fail_result(
                validator_id,
                "post-rerun reconciliation blocker rollup wrote current ledger JSONL",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        rollup_item = report["items"][0]
        if (
            rollup_item.get("item_rollup_status") != "BLOCKED_CURRENT_EVIDENCE_WRITE_DENIED"
            or rollup_item.get("primary_item_blocker_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
            or rollup_item.get("current_evidence_write_authorized")
            or rollup_item.get("current_evidence_write_allowed")
            or rollup_item.get("candidate_current_evidence_usable")
        ):
            return fail_result(
                validator_id,
                "post-rerun reconciliation blocker rollup item did not remain blocked",
                paths,
                "LIVE_FINAL_GUARD_FAILED",
            )
        written_path = write_upbit_paper_post_rerun_reconciliation_blocker_rollup_report(root=root, report=report)
        if not written_path.exists():
            return fail_result(
                validator_id,
                "post-rerun reconciliation blocker rollup writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )
        ...
        if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
            return fail_result(
                validator_id,
                "blocker rollup primary count tamper was not rejected",
                paths,
                count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH",
            )
        ...
        if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
            return fail_result(
                validator_id,
                "post-rerun reconciliation blocker rollup live mutation was not blocked",
                paths,
                live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
            )
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                validator_id,
                f"runtime post-rerun reconciliation blocker rollup artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_reconciliation_blocker_rollup_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime post-rerun reconciliation blocker rollup artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
...
def upbit_paper_post_rerun_operator_resolution_audit_validator() -> ValidatorResult:
    validator_id = "upbit_paper_post_rerun_operator_resolution_audit_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_operator_resolution_audit_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_operator_resolution_audit.py"
    guidance_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_operator_reconciliation_review_guidance.py"
    decision_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_reconciliation_decision_audit.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_operator_resolution_audit.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_operator_resolution_audit_report.json"
        )
    )
    paths = [schema_path, module_path, guidance_module_path, decision_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_operator_resolution_audit_report.v1":
        return fail_result(validator_id, "post-rerun operator resolution audit schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "post-rerun operator resolution audit schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                validator_id,
                f"post-rerun operator resolution audit schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    paths.extend([guidance_path, decision_path])
    if not guidance_path.exists() or not decision_path.exists():
        return blocked_result(
            validator_id,
            "post-rerun operator resolution audit source reports are missing",
            paths,
    ...
    if guidance_result.status != "PASS":
        return fail_result(
            validator_id,
            f"source review guidance failed validation: {guidance_result.message}",
            paths,
            guidance_result.blocker_code or "UNKNOWN_BLOCKED",
        )
    decision_result = validate_upbit_paper_post_rerun_reconciliation_decision_audit_report(decision)
    if decision_result.status != "PASS":
        return fail_result(
            validator_id,
            f"source decision audit failed validation: {decision_result.message}",
            paths,
            decision_result.blocker_code or "UNKNOWN_BLOCKED",
        )
    ...
    if result.status != "PASS":
        return fail_result(
            validator_id,
            f"valid post-rerun operator resolution audit failed: {result.message}",
            paths,
            result.blocker_code or "UNKNOWN_BLOCKED",
        )
    if (
        ...
        or report.get("resolution_controls_satisfied_count") != 0
        or report.get("current_evidence_write_authorized_count") != 0
        or report.get("current_evidence_write_allowed_count") != 0
        or report.get("candidate_current_evidence_usable_count") != 0
        or report.get("current_evidence_write_allowed")
        ...
        return fail_result(
            validator_id,
            "post-rerun operator resolution audit did not preserve unresolved blocked state",
            paths,
            "LIVE_FINAL_GUARD_FAILED",
        )
    if not report.get("items"):
        return fail_result(validator_id, "post-rerun operator resolution audit produced no items", paths, "MEASUREMENT_MISSING")
    item = report["items"][0]
    if (
        item.get("resolution_status") != "UNRESOLVED_CURRENT_EVIDENCE_BLOCKED"
        or item.get("resolution_evidence_present")
        or item.get("resolution_evidence_accepted")
        ...
        return fail_result(
            validator_id,
            "post-rerun operator resolution audit item did not remain unresolved and blocked",
            paths,
            "LIVE_FINAL_GUARD_FAILED",
        )

    with TemporaryDirectory() as tmp:
        written_path = write_upbit_paper_post_rerun_operator_resolution_audit_report(root=Path(tmp), report=report)
        if not written_path.exists():
            return fail_result(
                validator_id,
                "post-rerun operator resolution audit writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )
    ...
    count_tamper["resolution_audit_hash"] = upbit_paper_post_rerun_operator_resolution_audit_hash(count_tamper)
    count_result = validate_upbit_paper_post_rerun_operator_resolution_audit_report(count_tamper)
    if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
        return fail_result(
            validator_id,
            "post-rerun operator resolution count tamper was not rejected",
            paths,
            count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH",
        )
    ...
    if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(
            validator_id,
            "post-rerun operator resolution audit live mutation was not blocked",
            paths,
            live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
        )

    resolution_drift = json.loads(json.dumps(report))
    resolution_drift["items"][0]["resolution_evidence_present"] = True
    resolution_drift["resolution_audit_hash"] = upbit_paper_post_rerun_operator_resolution_audit_hash(resolution_drift)
    drift_result = validate_upbit_paper_post_rerun_operator_resolution_audit_report(resolution_drift)
    if drift_result.status != "BLOCKED" or drift_result.blocker_code != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE:
        return fail_result(
            validator_id,
            "post-rerun operator resolution evidence drift was not blocked",
            paths,
            drift_result.blocker_code or POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE,
        )
    ...
    if path_result.status != "BLOCKED" or path_result.blocker_code != "SNAPSHOT_SCOPE_MISMATCH":
        return fail_result(
            validator_id,
            "post-rerun operator resolution audit path escape was not blocked",
            paths,
            path_result.blocker_code or "SNAPSHOT_SCOPE_MISMATCH",
        )
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                validator_id,
                f"runtime post-rerun operator resolution audit artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_operator_resolution_audit_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime post-rerun operator resolution audit artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
...
def upbit_paper_post_rerun_resolution_current_evidence_closure_validator() -> ValidatorResult:
    validator_id = "upbit_paper_post_rerun_resolution_current_evidence_closure_validator"
    schema_path = ROOT / "contracts" / "schema" / "upbit_paper_post_rerun_resolution_current_evidence_closure_report.schema.json"
    module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_resolution_current_evidence_closure.py"
    source_module_path = ROOT / "trader1" / "runtime" / "paper" / "upbit_paper_post_rerun_operator_resolution_audit.py"
    test_path = ROOT / "tests" / "runtime" / "test_upbit_paper_post_rerun_resolution_current_evidence_closure.py"
    runtime_report_paths = sorted(
        (ROOT / "system" / "runtime" / "upbit" / "krw_spot" / "paper").glob(
            "*/paper_runtime/upbit_paper_post_rerun_resolution_current_evidence_closure_report.json"
        )
    )
    paths = [schema_path, module_path, source_module_path, test_path, *runtime_report_paths]
    schema = load_json(schema_path)
    if schema.get("$id") != "trader1.upbit_paper_post_rerun_resolution_current_evidence_closure_report.v1":
        return fail_result(validator_id, "post-rerun resolution current-evidence closure schema_id mismatch", paths, "SCHEMA_IDENTITY_MISMATCH")
    if schema.get("additionalProperties") is not False:
        return fail_result(validator_id, "post-rerun resolution current-evidence closure schema must be strict", paths, "SCHEMA_IDENTITY_MISMATCH")
    required = set(schema.get("required", []))
    for field in (
        ...
        if field not in required:
            return fail_result(
                validator_id,
                f"post-rerun resolution current-evidence closure schema missing required field: {field}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
    ...
    if not source_path.exists():
        return blocked_result(
            validator_id,
            "post-rerun resolution current-evidence closure source audit is missing",
            paths,
            "POST_RERUN_RECONCILIATION_REQUIRED",
        )
    source = load_json(source_path)
    source_result = validate_upbit_paper_post_rerun_operator_resolution_audit_report(source)
    if source_result.status != "PASS":
        return fail_result(
            validator_id,
            f"source post-rerun operator resolution audit failed validation: {source_result.message}",
            paths,
            source_result.blocker_code or "UNKNOWN_BLOCKED",
        )
    report = build_upbit_paper_post_rerun_resolution_current_evidence_closure_report(
        ...
        source_resolution_audit_path=rel(source_path),
    )
    result = validate_upbit_paper_post_rerun_resolution_current_evidence_closure_report(report)
    if result.status != "PASS":
        return fail_result(
            validator_id,
            f"valid post-rerun resolution current-evidence closure failed: {result.message}",
            paths,
            result.blocker_code or "UNKNOWN_BLOCKED",
        )
    if (
        report.get("closure_status") != "CURRENT_EVIDENCE_CLOSED_RESOLUTION_UNRESOLVED"
        or report.get("primary_blocker_code") != POST_RERUN_RECONCILIATION_REQUIRED_BLOCKER_CODE
        or report.get("source_resolution_audit_file_load_status") != "PASS"
        or report.get("source_resolution_audit_file_hash_match") is not True
        or report.get("source_unresolved_item_count") != source.get("unresolved_item_count")
        or report.get("source_resolved_item_count") != 0
        or report.get("closed_item_count") != source.get("unresolved_item_count")
        or report.get("current_evidence_closed_count") != source.get("unresolved_item_count")
        ...
        or report.get("live_order_allowed")
        or report.get("scale_up_allowed")
        or POST_RERUN_RESOLUTION_CURRENT_EVIDENCE_CLOSURE_REQUIRED not in set(report.get("blocker_codes") or [])
    ):
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure did not preserve closed blocked state",
            paths,
            "LIVE_FINAL_GUARD_FAILED",
        )
    if not report.get("closure_items"):
        return fail_result(validator_id, "post-rerun resolution current-evidence closure produced no items", paths, "MEASUREMENT_MISSING")
    item = report["closure_items"][0]
    if (
        item.get("closure_status") != "CLOSED_CURRENT_EVIDENCE_BLOCKED"
        or item.get("source_resolution_evidence_present")
        or item.get("source_resolution_evidence_accepted")
        or item.get("current_evidence_write_authorized")
        or item.get("current_evidence_write_allowed")
        or item.get("candidate_current_evidence_usable")
        or item.get("live_order_allowed")
        or item.get("scale_up_allowed")
        ...
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure item did not remain closed and blocked",
            paths,
            "LIVE_FINAL_GUARD_FAILED",
        )

    with TemporaryDirectory() as tmp:
        written_path = write_upbit_paper_post_rerun_resolution_current_evidence_closure_report(root=Path(tmp), report=report)
        if not written_path.exists():
            return fail_result(
                validator_id,
                "post-rerun resolution current-evidence closure writer did not create report",
                paths,
                "MEASUREMENT_MISSING",
            )
    ...
    with TemporaryDirectory() as tmp:
        root = Path(tmp)
        source_relative_path = rel(source_path)
        target = root / source_relative_path
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(json.dumps(source, sort_keys=True), encoding="utf-8")
        target.unlink()
        missing_report = build_upbit_paper_post_rerun_resolution_current_evidence_closure_report(
            root=root,
            resolution_audit_report=source,
            source_resolution_audit_path=source_relative_path,
        )
        missing_result = validate_upbit_paper_post_rerun_resolution_current_evidence_closure_report(missing_report)
        if (
            missing_report.get("source_resolution_audit_file_load_status") != "MISSING"
            or missing_result.status != "BLOCKED"
            or missing_result.blocker_code != POST_RERUN_RESOLUTION_CLOSURE_SOURCE_BINDING_REQUIRED
        ):
            return fail_result(
                validator_id,
                "post-rerun resolution current-evidence closure missing source file was not blocked",
                paths,
                missing_result.blocker_code or POST_RERUN_RESOLUTION_CLOSURE_SOURCE_BINDING_REQUIRED,
            )

    count_tamper = json.loads(json.dumps(report))
    count_tamper["closed_item_count"] = count_tamper["closed_item_count"] + 1
    count_tamper["closure_hash"] = upbit_paper_post_rerun_resolution_current_evidence_closure_hash(count_tamper)
    count_result = validate_upbit_paper_post_rerun_resolution_current_evidence_closure_report(count_tamper)
    if count_result.status != "FAIL" or count_result.blocker_code != "SCHEMA_IDENTITY_MISMATCH":
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure count tamper was not rejected",
            paths,
            count_result.blocker_code or "SCHEMA_IDENTITY_MISMATCH",
        )
    ...
    if live_result.status != "BLOCKED" or live_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure live mutation was not blocked",
            paths,
            live_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
        )
    ...
    if drift_result.status != "BLOCKED" or drift_result.blocker_code != "LIVE_FINAL_GUARD_FAILED":
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure resolution drift was not blocked",
            paths,
            drift_result.blocker_code or "LIVE_FINAL_GUARD_FAILED",
        )

    path_escape = json.loads(json.dumps(report))
    path_escape["closure_items"][0]["planned_current_ledger_jsonl_path"] = "system/runtime/upbit/krw_spot/live/bad.paper_ledger_events.jsonl"
    ...
    if path_result.status != "BLOCKED" or path_result.blocker_code != "SNAPSHOT_SCOPE_MISMATCH":
        return fail_result(
            validator_id,
            "post-rerun resolution current-evidence closure path escape was not blocked",
            paths,
            path_result.blocker_code or "SNAPSHOT_SCOPE_MISMATCH",
        )
    ...
    for runtime_path in runtime_report_paths:
        try:
            ...
            return fail_result(
                validator_id,
                f"runtime post-rerun resolution current-evidence closure artifact is not valid json: {rel(runtime_path)}: {exc}",
                paths,
                "SCHEMA_IDENTITY_MISMATCH",
            )
        runtime_result = validate_upbit_paper_post_rerun_resolution_current_evidence_closure_report(runtime_report)
        if runtime_result.status != "PASS":
            return fail_result(
                validator_id,
                f"runtime post-rerun resolution current-evidence closure artifact failed validation: {rel(runtime_path)}: {runtime_result.message}",
                paths,
                runtime_result.blocker_code or "UNKNOWN_BLOCKED",
            )
    ...
    return pass_result(
        validator_id,
        "Upbit PAPER post-rerun resolution current-evidence closure keeps unresolved resolution out of current evidence/live/scale",
        paths,
    )
...
VALIDATOR_FUNCTIONS: dict[str, Callable[[], ValidatorResult]] = {
    ...
    "upbit_paper_post_rerun_ledger_rollup_reconciliation_validator": upbit_paper_post_rerun_ledger_rollup_reconciliation_validator,
    "upbit_paper_post_rerun_current_evidence_promotion_guard_validator": upbit_paper_post_rerun_current_evidence_promotion_guard_validator,
