...
class ReadOnlyDashboardTest(unittest.TestCase):
    def test_dashboard_shell_is_display_only_and_live_blocked(self):
        ...
        self.assertEqual(refresh["source"], "runner_status.json")
        self.assertEqual(refresh["auto_refresh_interval_seconds"], 2)
        self.assertEqual(refresh["stale_after_seconds"], 300)
        self.assertTrue(refresh["client_stale_guard_enabled"])
        self.assertEqual(refresh["refresh_mode"], "LOCAL_STATUS_CHANNEL")
    ...
    def test_dashboard_marks_scorecard_previous_scope_when_runner_candidate_rotates(self):
        ...
        self.assertIn('data-primary-question="next-action"', html)
        self.assertIn('aria-label="operator next action quick answer"', html)
        self.assertIn("What should I do now?", html)
    ...
    def test_dashboard_running_header_ignores_stop_request_superseded_by_start(self):
        session_id = "test_read_only_dashboard_runner_ops"
        stop_report = {
            "schema_id": "trader1.root_stop_request_report.v1",
            "generated_at_utc": utc_now(),
            ...
            "mode": "PAPER",
            "session_id": session_id,
            "stop_request_status": "SUPERSEDED_BY_OPERATOR_START",
            "stop_request_method": "UPBIT_PAPER_STOP_FILE",
            "stop_confirmed": False,
            "stop_result_summary": "Previous UPBIT PAPER stop request was superseded by a later operator start.",
            "runner_status_after": "RUNNING",
            "runner_running_after": True,
            "dashboard_refresh_requested": True,
            "dashboard_should_show_stopped": False,
        ...
        self.assertEqual(result.status, "PASS", result.message)
        self.assertEqual(dashboard["primary_status_text"], "PAPER RUNNING - READ ONLY, LIVE ORDERS BLOCKED")
        self.assertEqual(dashboard["operator_stop_status"]["status"], "NOT_REQUESTED")
        self.assertEqual(
            dashboard["operator_stop_status"]["stop_request_status"],
            "SUPERSEDED_BY_OPERATOR_START",
        )
        self.assertIn("superseded by a later operator start", html)
        self.assertFalse(dashboard["live_order_allowed"])
    ...
    def test_dashboard_primary_status_separates_current_profile_from_unproven_runner(self):
        ...
        operator_action = dashboard["operator_action_summary"]
        self.assertEqual(operator_action["primary_action_label"], "Continue PAPER/SHADOW collection")
        self.assertEqual(operator_action["workflow_step"], "COLLECT_EVIDENCE")
        self.assertIn("PAPER cycles remaining", operator_action["one_line_blocker"])
        html = render_dashboard_html(dashboard)
        self.assertIn("Continue PAPER/SHADOW collection", html)
        self.assertIn("PAPER profile is current; LIVE_READY is blocked", html)
        self.assertIn("Remaining: PAPER cycles=", html)
        self.assertFalse(dashboard["live_order_ready"])
        self.assertFalse(dashboard["live_order_allowed"])
        self.assertFalse(dashboard["can_live_trade"])
        self.assertFalse(dashboard["scale_up_allowed"])
    ...
    def test_dashboard_projects_candidate_scorecard_as_display_only(self):
        ...
        self.assertEqual(
            maturity["candidate_scorecard_realized_vs_expected_edge_status"],
    ...
    def test_dashboard_html_has_no_order_controls(self):
        ...
        self.assertIn("data-quick-next-label", html)
        ...
        self.assertIn("/api/runner-status", html)
        self.assertIn("runnerChannelLastSeenAtMs", html)
        self.assertIn("Runner channel live", html)
        self.assertIn("Stop requested", html)
        self.assertIn("Static dashboard file age is fallback only", html)
        self.assertIn("data-dashboard-age", html)
        self.assertIn("Fallback Refresh", html)
        ...
        self.assertIn("data-refresh-seconds=\"2\"", html)
