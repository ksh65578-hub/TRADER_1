...
class ReadOnlyDashboardTest(unittest.TestCase):
    ...
    def test_dashboard_html_has_no_order_controls(self):
        ...
        self.assertIn(".operator-answer-grid { display: grid; gap: 16px; grid-template-columns: minmax(300px, 0.95fr) minmax(360px, 1.25fr) minmax(280px, 0.9fr);", html)
        self.assertIn(".operator-answer-card { background: white; border: 1px solid var(--line); border-radius: 8px; padding: 16px;", html)
    ...
    def test_dashboard_first_screen_prioritizes_operator_questions(self):
        ...
        answer_html = html[answer_start:freshness_start]

        self.assertLess(answer_start, freshness_start)
        self.assertLess(freshness_start, detail_start)
