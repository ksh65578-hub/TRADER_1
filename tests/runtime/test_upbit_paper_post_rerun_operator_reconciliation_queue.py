...
class UpbitPaperPostRerunOperatorReconciliationQueueTest(unittest.TestCase):
    ...
    def _promotion_guard_with_missing_current_ledger(self) -> tuple[Path, dict, Path]:
    ...
    def test_builds_operator_queue_without_current_evidence_mutation(self):
        ...
        item = report["items"][0]
        self.assertEqual(item["operator_reconciliation_status"], "REVIEW_READY_RECONCILIATION_REQUIRED")
        self.assertTrue(item["requires_operator_reconciliation"])
        self.assertEqual(item["path_scope_status"], "MATCH")
        ...
        path = write_upbit_paper_post_rerun_operator_reconciliation_queue_report(root=root, report=report)
    ...
    def test_blocks_live_and_current_write_mutation(self):
        ...
        write_count["queue_hash"] = upbit_paper_post_rerun_operator_reconciliation_queue_hash(write_count)
        write_result = validate_upbit_paper_post_rerun_operator_reconciliation_queue_report(write_count)
    ...
    def test_rejects_count_tamper_and_path_escape(self):
        ...
        path_escape["items"][0]["planned_current_ledger_jsonl_path"] = "system/runtime/upbit/krw_spot/live/bad.jsonl"
