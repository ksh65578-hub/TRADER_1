...
STAGE_GATE_PATH = (
    ...
    / "evidence"
    / "stage_gates"
    / "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK.stage_gate_result.json"
)
DEPTH_REPORT_PATH = (
    ROOT
    / "system"
    / "evidence"
    / "audit_reports"
    / "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK.json"
)
CONTRACT_GAP_PATH = (
    ROOT
    / "system"
    / "evidence"
    / "contract_gaps"
    / "POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED.contract_gap.json"
)
RUNTIME_BASE = (
    ROOT
    / "system"
...
class PostRerunCurrentEvidenceWriteBlockedImplementationDepthRecheckTest(unittest.TestCase):
    def test_depth_recheck_keeps_review_ready_candidates_write_blocked(self):
        ...
        self.assertEqual(
            ...
            "MVP4_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_IMPLEMENTATION_DEPTH_RECHECK_20260504_001",
        )
        self.assertEqual(patch_result["next_task_class"], NEXT_TASK_CLASS)
        self.assertEqual(
            depth_report["status"],
            "PASS_DEPTH_5_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_LIVE_BLOCKING",
        )
        self.assertEqual(
            stage_gate["stage_gate_status"],
            "PASS_POST_RERUN_CURRENT_EVIDENCE_WRITE_BLOCKED_DEPTH_RECHECK_LIVE_BLOCKING",
        )

        self.assertEqual(depth_report["promotion_guard_status"], "BLOCKED")
        self.assertEqual(depth_report["promotion_review_ready_count"], 8)
        self.assertEqual(depth_report["promotion_candidate_verified_count"], 8)
        ...
        self.assertEqual(depth_report["resolution_audit_status"], "UNRESOLVED_RECONCILIATION_REVIEW_ONLY")
        self.assertEqual(depth_report["closure_status"], "CURRENT_EVIDENCE_CLOSED_RESOLUTION_UNRESOLVED")
        ...
        self.assertEqual(depth_report["current_evidence_write_authorized_count"], 0)
    ...
    def test_runtime_artifact_chain_remains_fail_closed(self):
        ...
        for artifact_name in artifact_names:
            with self.subTest(artifact_name=artifact_name):
                ...
                self.assertIn(GAP_ID, report["blocker_codes"])
    ...
    def test_state_routes_forward_after_write_blocked_depth_recheck(self):
        ...
        if (state["last_patch_id"].startswith("MVP4_OPEN_GAP_CURRENT_BLOCKER_CLASSIFICATION_") or state["last_patch_id"].startswith("MVP4_DASHBOARD_RESIDUAL_BLOCKER_SUMMARY_") or state["last_patch_id"].startswith("MVP4_RESIDUAL_OPEN_GAP_OPERATOR_ACTION_PLAN_")):
