...
STAGE_GATE_PATH = (
    ...
    / "evidence"
    / "stage_gates"
    / "MVP4_STALE_LOOP_REGENERATION_EXECUTION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK.stage_gate_result.json"
)
DEPTH_REPORT_PATH = (
    ROOT
    / "system"
    / "evidence"
    / "audit_reports"
    / "MVP4_STALE_LOOP_REGENERATION_EXECUTION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK.json"
)
REQUIRED_GAP_PATH = (
    ROOT / "system" / "evidence" / "contract_gaps" / "STALE_LOOP_REGENERATION_REQUIRED.contract_gap.json"
)
EXECUTION_GAP_PATH = (
    ROOT / "system" / "evidence" / "contract_gaps" / "STALE_LOOP_REGENERATION_EXECUTION_REQUIRED.contract_gap.json"
)
GUARD_PATH = (
    ROOT
    / "system"
...
class StaleLoopRegenerationExecutionRequiredRecheckTest(unittest.TestCase):
    ...
    def test_depth_recheck_closes_execution_gap_and_routes_to_post_regeneration_reconciliation(self):
        if not PATCH_PATH.exists():
            self.skipTest("stale loop regeneration execution depth recheck patch has not been generated yet")
        ...
        self.assertEqual(
            ...
            "MVP4_STALE_LOOP_REGENERATION_EXECUTION_REQUIRED_IMPLEMENTATION_DEPTH_RECHECK_20260505_001",
        )
        self.assertEqual(patch_result["next_task_class"], NEXT_TASK)
        self.assertEqual(
            stage_gate["stage_gate_status"],
            "PASS_DEPTH_5_STALE_LOOP_REGENERATION_EXECUTION_RECONCILIATION_BLOCKED",
        )
        self.assertEqual(
            depth_report["status"],
            "PASS_DEPTH_5_STALE_LOOP_REGENERATION_EXECUTION_RECONCILIATION_BLOCKED",
        )
        self.assertEqual(depth_report["contract_gap_id"], "STALE_LOOP_REGENERATION_EXECUTION_REQUIRED")
        ...
        self.assertFalse(depth_report["new_runtime_regeneration_performed"])
    ...
    def test_resolved_contract_gap_projections_keep_next_reconciliation_open(self):
        required_gap = load_json(REQUIRED_GAP_PATH)
        execution_gap = load_json(EXECUTION_GAP_PATH)
        state = load_json(STATE_PATH)

        self.assertEqual(required_gap["status"], "RESOLVED")
        self.assertEqual(execution_gap["status"], "RESOLVED")
        self.assertTrue(required_gap["live_affecting"])
