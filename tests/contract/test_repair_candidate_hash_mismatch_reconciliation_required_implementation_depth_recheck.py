...
AFTER_REGENERATED_CURRENT_DEPTH_RECHECK_NEXT_TASK = (
...
class RepairCandidateHashMismatchImplementationDepthRecheckTest(unittest.TestCase):
    ...
    def test_state_routes_forward_after_hash_mismatch_depth_recheck(self):
        ...
        if (state["last_patch_id"].startswith("MVP4_OPEN_GAP_CURRENT_BLOCKER_CLASSIFICATION_") or state["last_patch_id"].startswith("MVP4_DASHBOARD_RESIDUAL_BLOCKER_SUMMARY_") or state["last_patch_id"].startswith("MVP4_RESIDUAL_OPEN_GAP_OPERATOR_ACTION_PLAN_")):
            ...
            self.assertEqual(state["next_allowed_task_class"], AFTER_REGENERATED_CURRENT_DEPTH_RECHECK_NEXT_TASK)
        elif BLOCKED_REPAIR_PLAN_DEPTH_RECHECK_REQUIREMENT_ID in state["completed_requirement_ids"]:
