...
class UpbitPaperRuntimeCycleTest(unittest.TestCase):
    ...
    def test_vwap_exit_policy_survives_trend_exhaustion_tightening(self):
        ...
        self.assertAlmostEqual(exit_plan["hard_stop"], 99.25)
        self.assertAlmostEqual(exit_plan["tp1"], 100.65)
        self.assertAlmostEqual(exit_plan["tp2"], 101.0)
        self.assertAlmostEqual(exit_plan["trailing_start"], 101.0)
        self.assertAlmostEqual(exit_plan["trailing_distance"], 0.5)
        self.assertEqual(evaluation["final_decision"], "EXIT_POSITION")
        self.assertEqual(evaluation["reason_code"], "VWAP_REVERSION_COMPLETE")
        self.assertEqual(evaluation["strategy_exit_action"], "FULL_EXIT")
    ...
    def test_breakout_exit_policy_survives_trend_exhaustion_tightening(self):
        ...
        self.assertEqual(exit_plan["time_stop_candles"], 4)
        self.assertEqual(exit_plan["trend_exhaustion_exit_adjustment"], "TIGHTENED_TRAILING_AND_TIME_STOP")
        self.assertAlmostEqual(exit_plan["hard_stop"], 99.2)
        self.assertAlmostEqual(exit_plan["tp1"], 101.0)
        self.assertAlmostEqual(exit_plan["tp2"], 101.8)
        self.assertAlmostEqual(exit_plan["trailing_start"], 100.9)
        self.assertAlmostEqual(exit_plan["trailing_distance"], 0.55)
        self.assertEqual(evaluation["final_decision"], "EXIT_POSITION")
        self.assertEqual(evaluation["reason_code"], "BREAKOUT_LEVEL_LOST")
        self.assertEqual(evaluation["strategy_exit_action"], "FULL_EXIT")
    ...
    def test_overextended_uptrend_exhaustion_blocks_new_entry_and_tightens_exit_plan(self):
        ...
        entry_price = float(report["exit_plan"]["entry_price"])
        atr_proxy = float(report["exit_plan"]["atr_proxy"])
        self.assertAlmostEqual(report["exit_plan"]["hard_stop"], entry_price - 0.8 * atr_proxy)
        self.assertAlmostEqual(report["exit_plan"]["tp1"], entry_price + 1.0 * atr_proxy)
        self.assertAlmostEqual(report["exit_plan"]["tp2"], entry_price + 1.8 * atr_proxy)
        self.assertAlmostEqual(report["exit_plan"]["trailing_start"], entry_price + 0.9 * atr_proxy)
        self.assertAlmostEqual(report["exit_plan"]["trailing_distance"], 0.55 * atr_proxy)
        self.assertFalse(report["live_order_allowed"])
