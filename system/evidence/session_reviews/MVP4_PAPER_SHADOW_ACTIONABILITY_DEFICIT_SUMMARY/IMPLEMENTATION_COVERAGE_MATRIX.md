...
| 4 | paper / shadow / replay / micro-live / live | Critical | Patched PAPER/SHADOW evidence report only. | No micro-live/live path touched. |
...
| 9 | order lifecycle / execution quality / partial fill | High | No order-capable path touched. | Actionability is report-only. |
...
| 13 | dashboard / USER_STATUS_SUMMARY / user simplicity | Medium | Report now gives user-level next action. | Dashboard wiring can consume this later. |
| 14 | validator / schema / registry / acceptance artifacts | Medium | Schema, validator, fixtures, and evidence updated. | Validators pass. |
