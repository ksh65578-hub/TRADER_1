...
|---|---|---|---|---|
| 1 | strategy / regime / entry / exit | High | Strategy formulas exist; PAPER/SHADOW runtime binding remains blocking. | Dashboard now points to missing runtime binding dimensions. |
...
| 4 | paper / shadow / replay / micro-live / live | Critical | Patched PAPER/SHADOW runtime binding dashboard display only. | No micro-live/live path touched. |
...
| 9 | order lifecycle / execution quality / partial fill | High | No order-capable path touched. | Runtime binding is dashboard-only. |
...
| 12 | concurrency / race condition / restart recovery | Medium | No writer ownership changed. | Runtime source binding remains non-live and display-only. |
| 13 | dashboard / USER_STATUS_SUMMARY / user simplicity | Medium | Dashboard now gives user-level runtime source binding state. | Runtime Binding is visible. |
