...
| 4 | paper / shadow / replay / micro-live / live | Critical | Patched PAPER/SHADOW evidence ladder display. | Short-window and bounded profile evidence cannot become live permission. |
...
| 11 | data health / stale data / gap / duplicate / clock drift | High | Ladder makes stale/current distinction clearer. | TTL and long-run floors unchanged. |
| 12 | concurrency / race condition / restart recovery | Medium | No writer ownership changed. | Continuous writer remains blocked. |
| 13 | dashboard / USER_STATUS_SUMMARY / user simplicity | High | Patched this session. | First screen now shows evidence ladder. |
| 14 | validator / schema / registry / acceptance artifacts | Medium | Schema and tests extended. | Patch result and validators must pass. |
| 15 | testing / pytest / paper run proof / live block proof | High | Tests prove ladder invariants; no new PAPER run was started. | Runtime proof not claimed. |
